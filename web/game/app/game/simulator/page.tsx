...
export default function SimulatorPage() {
    ...
    const defenderSkillGroups = result
        ? result.defendersSkills
            .map((skills, idx) => ({ idx, entries: Object.entries(skills) }))
            ...
            {result && (
                <GameCard>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-sm)', marginBottom: 'var(--space-md)' }}>
                        <StatusBadge variant={result.conquerCity ? 'gold' : 'muted'}>
                            {result.conquerCity ? '공성 성공' : '교전 종료'}
                        </StatusBadge>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 'var(--space-md)', marginBottom: 'var(--space-md)', fontSize: 'var(--text-sm)' }}>
                        <div>
                            ...
                            <div>교전 수: <strong>{result.avgWar.toLocaleString()}</strong></div>
                            <div>페이즈: <strong>{result.phase.toLocaleString()}</strong></div>
                            <div>잔여 병력: <strong>{result.attackerCrew.toLocaleString()}</strong></div>
                        </div>
                        <div>
                            <h3 style={{ color: 'var(--jade)', fontWeight: 600, marginBottom: 'var(--space-xs)' }}>피해 집계</h3>
                            <div>살상: <strong>{result.killed.toLocaleString()}</strong></div>
                            <div>전사: <strong>{result.dead.toLocaleString()}</strong></div>
                            <div>살상 범위: <strong>{result.minKilled.toLocaleString()}~{result.maxKilled.toLocaleString()}</strong></div>
                            <div>전사 범위: <strong>{result.minDead.toLocaleString()}~{result.maxDead.toLocaleString()}</strong></div>
                        </div>
                        <div>
                            <h3 style={{ color: 'var(--text-secondary)', fontWeight: 600, marginBottom: 'var(--space-xs)' }}>성 상태</h3>
                            <div>방어: <strong>{result.defenderCityDefense.toLocaleString()}</strong></div>
                            <div>성벽: <strong>{result.defenderCityWall.toLocaleString()}</strong></div>
                            <div>반복: <strong>{result.repeatCnt.toLocaleString()}</strong></div>
                        </div>
                    </div>
                                ...
                                {attackerSkillEntries.length > 0 && (
                                    <div>
                                        ...
                                        {attackerSkillEntries.map(([name, count]) => (
                                            <div key={name} style={{ color: 'var(--text-secondary)' }}>{name}: {count.toLocaleString()}</div>
                                        ))}
                                    </div>
                                )}
