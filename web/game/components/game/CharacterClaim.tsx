...
export default function CharacterClaim({ global, onClaimed }: { global: FrontGlobalInfo; onClaimed: () => void }) {
  ...
  const joinHref = serverId ? resolveServerGamePath(undefined, serverId, '/game', 'join') : '/game/join';
  const selectPoolHref = serverId ? resolveServerGamePath(undefined, serverId, '/game', 'select-pool') : '/game/select-pool';
  ...
  const claim = useCallback(
    async (generalId: number) => {
      ...
      try {
        ...
        const terminal = await pollCommandResult(res.requestId);
        if (terminal == null) {
          setError('빙의 처리가 지연되고 있습니다. 잠시 후 다시 시도하세요.');
          return;
        }
        if (!terminal.ok) {
    ...
    <div className="page-content claim-screen">
      ...
      <div className="claim-mode-grid">
        ...
        <div className={`claim-mode-card game-card ${canSelectNpc ? '' : 'is-disabled'}`}>
          ...
          <span className={canSelectNpc ? 'claim-mode-active' : 'claim-mode-blocked'}>{canSelectNpc ? '현재 서버 모드' : `현재 npcmode=${npcMode}`}</span>
        </div>
      ...
      {canSelectNpc ? (
            ...
            <div className="claim-grid">
              {candidates.map((c) => (
                <div key={c.generalId} className="claim-card game-card">
                  ...
                  <button className="claim-btn" disabled={claiming !== null} onClick={() => void claim(c.generalId)}>
        ...
        <div className="claim-mode-note">이 서버는 NPC 빙의 모드가 아닙니다. 위의 등록 방식을 사용하세요.</div>
      )}
