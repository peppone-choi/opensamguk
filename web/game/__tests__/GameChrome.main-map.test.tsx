...
describe('GameChrome main map', () => {
  beforeEach(() => {
    ...
    mocks.claim.mockReset().mockResolvedValue({ result: true, generalId: 9, reason: null, requestId: 'req-claim-9' });
    mocks.pollCommandResult.mockReset().mockResolvedValue({
      status: 'RESOLVED',
      requestId: 'req-claim-9',
      ok: true,
      type: 'claimNpc',
      result: {},
    });
  ...
  it('passes front-info into function children for the main record zone', () => {
    ...
    render(<GameChrome>{(loadedFrontInfo) => <div data-testid="record-probe-filled">{loadedFrontInfo.recentRecord.global[0]?.[1]}</div>}</GameChrome>);
