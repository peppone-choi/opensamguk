...
test('dispatch is separate single command and reservation never claims execution',async()=>{
 render(<HwihaCourtForm generalId={1}/>);await selectDestination();fireEvent.click(screen.getByRole('button',{name:'발령 접수'}));
...
test('recipient NOT_LORD can reply and terminal status outranks failure code',async()=>{
 ...
 render(<HwihaCourtForm generalId={2}/>);fireEvent.click(await screen.findByRole('button',{name:'수락'}));
...
test('blocked county uses server reason and cannot submit',async()=>{
 ...
 render(<HwihaCourtForm generalId={1}/>);await selectDestination();
...
test('stale read response cannot replace a newer refresh',async()=>{
 ...
 const view=render(<HwihaCourtForm generalId={1} refreshKey={0}/>);
 view.rerender(<HwihaCourtForm generalId={1} refreshKey={1}/>);
...
test('read failure remains explicit with retry and no optimistic empty success',async()=>{
 ...
 render(<HwihaCourtForm generalId={1}/>);
...
test('queued issuer is disabled and terminal read shows accepted without response controls',async()=>{
 ...
 render(<HwihaCourtForm generalId={2}/>);
...
test('refusal passes an opaque dispatch ID and execution rejection preserves the form',async()=>{
 ...
 render(<HwihaCourtForm generalId={2}/>);fireEvent.click(await screen.findByRole('button',{name:'거절'}));
...
test('parent callback identity changes cannot starve queued result polling',async()=>{
 ...
 try {
  const view=render(<HwihaCourtForm generalId={1} onReserved={()=>{}}/>);
  await act(async()=>{});
  for(let i=0;i<3;i++){
   await act(async()=>{await vi.advanceTimersByTimeAsync(1000);});
   view.rerender(<HwihaCourtForm generalId={1} onReserved={()=>{}}/>);
  }
