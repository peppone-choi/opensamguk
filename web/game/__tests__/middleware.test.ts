...
describe('game middleware server path selection', () => {
    ...
    it.each(['pep', 'a1', 's1'])('rewrites the configured public ID %s and sets its cookie', (serverId) => {
    ...
    it('uses canonical IDs for query selection and ignores uppercase paths and markers', () => {
        middleware(makeRequest('/game/join?server=a1'));
        expect(nextServerMocks.next).toHaveBeenCalledTimes(1);
        expect(nextResponse.cookies.set).toHaveBeenCalledWith('sam_server', 'a1', {
            path: '/',
            sameSite: 'lax',
            maxAge: 7 * 24 * 60 * 60,
        });
        ...
        nextServerMocks.next.mockClear();
        middleware(makeRequest('/game/pep/join'));
        expect(nextServerMocks.next).toHaveBeenCalledTimes(1);
        expect(nextServerMocks.rewrite).not.toHaveBeenCalled();
    });
});
