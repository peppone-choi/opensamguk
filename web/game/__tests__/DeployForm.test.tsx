...
test('requires both choices sends only canonical personal args and distinguishes reservation',async()=>{
    render(<DeployForm {...props}/>);expect(screen.getByRole('button',{name:'출병 예약'})).toBeDisabled();await choose();
...
test.each([{available:false,reason:'소유권 없음'},{order:{orderId:'order',destinationProvinceId:'p1',stop:'ARRIVED'}}])('server denial or existing order cannot submit %j',async change=>{
    vi.mocked(api.deployOptions).mockResolvedValue({...options,...change});render(<DeployForm {...props}/>);
...
test('late actor response is discarded',async()=>{
    ...
    const view=render(<DeployForm {...props}/>);view.rerender(<DeployForm {...props} generalId={2}/>);
...
test('refresh resets selections and ignores stale submission completion',async()=>{
    ...
    const view=render(<DeployForm {...props}/>);await choose();fireEvent.click(screen.getByRole('button',{name:'출병 예약'}));
    expect(submitCommandAndAwaitResult).toHaveBeenCalledOnce();view.rerender(<DeployForm {...props} refreshKey={1}/>);
...
test('multiple units sort and double clicks submit once while rejection stays visible',async()=>{
    ...
    render(<DeployForm {...props}/>);fireEvent.click(await screen.findByLabelText('아홉 · 10명'));await choose();
