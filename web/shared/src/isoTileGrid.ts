...
/**
* 원본 지형 격자를 타일 격자로 줄이는 배수. 세 매니페스트가 rasterGroup: 2 로 고정했다.
*
* 4 였다. 그러면 타일 하나가 원본 16 셀이라 城 두 곳이 한 타일에 겹쳐 앉는 일이 잦았다
* — 실측 39 타일에 城 79 곳(4 곳이 한 칸에 앉은 경우까지 있었다). 2 로 내리면 그 충돌이
* 4 타일 / 城 8 곳으로 줄고(패·광척, 임읍·치평, 단부·성무, 양추·추평), 남은 넷은 원본
* 셀 자체가 한두 칸 차이라 격자로는 더 갈라지지 않는다.
*
* 대가는 그린다. 같은 화면 배율에서 보이는 타일이 네 배다(32,064 → 128,256).
* 렌더러는 행 범위를 잘라 그리므로 전체가 아니라 화면에 든 만큼만 늘어난다.
*/
export const RASTER_GROUP = 2;
...
* 강 3,421셀(0.67%) · 호수 1,079셀(0.21%) · 구릉 129셀(0.03%). 블록 다수결에서
* 이 셋은 한 타일도 남지 않는다. 그래서 희소한 순서로 우선권을 준다.
*
* 강은 한 셀만 걸려도 강으로 친다. 선형 지형이라 끊기면 강이 아니게 되기 때문이다.
* 이 규칙은 강 비중을 0.67% → 1.46% 로 부풀리지만, 원본 강 자체가 580조각으로
* 끊겨 있던 것이 125조각으로 이어진다. 화면에서 물줄기로 읽히는 쪽을 택했다.
* 호수·구릉은 면 지형이라 두 셀 이상을 요구해 잡티를 거른다.
*
* 위 수치는 rasterGroup 2(2×2 블록) 실측이다. 4×4 였을 때는 강 2.97% · 38조각으로
* 더 굵고 더 이어졌다 — 격자를 촘촘히 할수록 「한 셀이라도」 규칙이 덜 번지므로
* 물줄기는 원본 비중에 가까워지고 대신 조각은 늘어난다.
*/
...
* rasterGroup 2): han-tiles 城 1,189 곳 중 **70 곳이 물 타일 위**에 섰다
* (강 65·호수 2·바다 3). 네 셀 중 셋이 뭍인데 타일이 통째로 강이 되어, 건업현(建業)·
* 합비현·파양현 같은 城 이 물 위에 떠 보였다. 진짜 섬(주호·대마국 따위)은 다수결에서
* 섬이 통째로 지워진 쪽이다.
*
...
*
* 원본 지형 격자는 후한 판도만 분류해 놨고 그 밖은 전부 OUT_OF_SCOPE 다. 블록이
* 통째로 OUT_OF_SCOPE 일 때만 타일이 OUT_OF_SCOPE 가 되므로, 남은 타일(rasterGroup 2
* 실측 27,052 · 4 였을 때 6,395)에는
 ...
 *
 * downsampleOwner 는 원본 셀 블록의 다수결로 칸 주인을 정한다. 그래서 縣 경계에 바짝 붙어
 * 선 治所는 제 칸을 이웃에게 넘긴다 — 원본 셀 해상도에서는 998/998 이 제 縣 땅 위에 서
 * 있는데(실측), 줄이고 나면 남의 색 위에 서는 城 이 생긴다. han-tiles 1,189 城 기준
 * rasterGroup 4 에서 245 곳, 2 에서 55 곳이다(격자를 촘촘히 해도 0 은 아니다).
