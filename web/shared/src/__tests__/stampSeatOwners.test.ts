...
describe('stampSeatOwners', () => {
  it('다수결만 쓰면 게임 城 67 곳이 남의 縣 색 위에 선다 — 이것이 고치는 대상이다', () => {
    ...
    // 2026-09-23 결손 縣 60곳을 세우며 67 → 87. 도장 찍기가 고치는 대상이 늘어난 것이고, 찍은 뒤는 32 다.
    expect(displaced(downsampleOwner(source, srcCols, cols, rows, RASTER_GROUP))).toHaveLength(87);
  });
  ...
  it('治所 칸을 되돌리면 68 → 27 로 줄고, 남는 27 은 전부 칸을 나눠 쓰는 城 이다', () => {
    ...
    // 2026-09-23 결손 縣 60곳을 세우며 27 → 32. 새 縣이 제 실제 자리에 서면서 이웃 縣과 같은 2×2
    // 타일에 드는 자리가 다섯 늘었다 — 아래 단언이 그 다섯도 전부 治所가 둘 이상인 칸임을 본다.
