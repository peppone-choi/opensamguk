...
object HanStrategicTopologyJson {
    ...
    fun load(mapName: String, readArtifact: (String) -> ByteArray): HanStrategicRouteProjection =
        loadVersion(mapName, 1228, readArtifact)
     ...
     */
    private val landCountByRoster = mapOf(832 to 1520, 835 to 1520, 846 to 1520, 848 to 1520, 1098 to 1594, 1133 to 1331, 1141 to 1336, 1341 to 1742, 1194 to 1558, 1168 to 1374, 1228 to 1434)
    ...
    private fun routeBindings(docs: Map<String, JsonNode>, hashes: Map<String, String>, provinces: List<JsonNode>, landIds: List<String>,
        ...
        fun identities(rows: List<JsonNode>, idField: String): Set<Identity> {
            // 1168·1228 판은 명부 id 가 연속이 아니다 — 은퇴 id 26 개를 되쓰지 않으므로
            // 최댓값이 명부 수보다 크다. 그 판은 등록부에서 실제 id 집합을 읽는다.
            val expectedIds = if (cityCount in setOf(1168, 1228)) opensamguk.logic.world.CityConstRegistry.hanWorld(
