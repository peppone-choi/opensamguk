...
internal object Archive1224Artifacts {
    private const val CATALOG_SHA256 = "047668fa9bfcc5de9efe6d82d40b67c1942d8534f426616837058e2f02729d15"
    ...
    fun load(root: Path): ResolvedWorldArtifacts {
        val variant = WorldMapVariant.V3_1224
        val directory = root.resolve("data/map/han-world-v3-1224-artifacts-v1")
        val raw = Files.readAllBytes(directory.resolve("catalog.json"))
        require(sha(raw) == CATALOG_SHA256) { "1224 release catalog hash mismatch" }
        ...
        require(catalog.path("schemaVersion").asInt() == 1 &&
            ...
            catalog.path("cityCount").asInt() == variant.cityCount) { "1224 release identity mismatch" }
        ...
        require(entries.size == paths.size && entries.map { it.path("path").asText() }.toSet() == paths) {
            "1224 release artifact path set mismatch"
        }
        val bytes = entries.associate { entry ->
            ...
            require(sha(compressed) == entry.path("compressedSha256").asText()) { "1224 compressed artifact hash mismatch" }
            ...
            require(data.size == length && sha(data) == hash) { "1224 artifact hash/length mismatch" }
