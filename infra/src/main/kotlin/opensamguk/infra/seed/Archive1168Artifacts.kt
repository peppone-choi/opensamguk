...
internal object Archive1168Artifacts {
    private const val CATALOG_SHA256 = "675e28ae2cfa49e52e8e0c45a0e28193dd22725465fb0e71902c8d0418a60f61"
    ...
    fun load(root: Path): ResolvedWorldArtifacts {
        val variant = WorldMapVariant.V3_1168
        val directory = root.resolve("data/map/han-world-v3-1168-artifacts-v1")
        val raw = Files.readAllBytes(directory.resolve("catalog.json"))
        require(sha(raw) == CATALOG_SHA256) { "1168 release catalog hash mismatch" }
        ...
        require(catalog.path("schemaVersion").asInt() == 1 &&
            ...
            catalog.path("cityCount").asInt() == variant.cityCount) { "1168 release identity mismatch" }
        ...
        require(entries.size == paths.size && entries.map { it.path("path").asText() }.toSet() == paths) {
            "1168 release artifact path set mismatch"
        }
        val bytes = entries.associate { entry ->
            ...
            require(sha(compressed) == entry.path("compressedSha256").asText()) { "1168 compressed artifact hash mismatch" }
            ...
            require(data.size == length && sha(data) == hash) { "1168 artifact hash/length mismatch" }
