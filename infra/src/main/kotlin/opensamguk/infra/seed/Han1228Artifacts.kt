...
internal object Han1228Artifacts {
    private const val CATALOG_SHA256 = "b64eaa689bde91c5090364909b01322a9e964900c4c4afc831d3c06458078fa7"
    ...
    fun load(root: Path): ResolvedHanWorldArtifacts {
        val variant = HanWorldVariant.V3_1228
        val directory = root.resolve("data/map/han-world-v3-1228-artifacts-v1")
        val raw = Files.readAllBytes(directory.resolve("catalog.json"))
        require(sha(raw) == CATALOG_SHA256) { "1228 release catalog hash mismatch" }
        ...
        require(catalog.path("schemaVersion").asInt() == 1 &&
            ...
            catalog.path("cityCount").asInt() == variant.cityCount) { "1228 release identity mismatch" }
        ...
        require(entries.size == paths.size && entries.map { it.path("path").asText() }.toSet() == paths) {
            "1228 release artifact path set mismatch"
        }
        val bytes = entries.associate { entry ->
            ...
            require(sha(compressed) == entry.path("compressedSha256").asText()) { "1228 compressed artifact hash mismatch" }
            ...
            require(data.size == length && sha(data) == hash) { "1228 artifact hash/length mismatch" }
