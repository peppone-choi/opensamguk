...
internal object Archive1341Artifacts {
    private const val CATALOG_SHA256 = "a3d9536d99fd3c204a2d801cbf1fac7f9db8e2de7253b71b564944d882e9a9c1"
    ...
    fun load(root: Path): ResolvedWorldArtifacts {
        val variant = WorldMapVariant.V3_1341
        val directory = root.resolve("data/map/han-world-v3-1341-artifacts-v1")
        val raw = Files.readAllBytes(directory.resolve("catalog.json"))
        require(sha(raw) == CATALOG_SHA256) { "1341 release catalog hash mismatch" }
        ...
        require(catalog.path("schemaVersion").asInt() == 1 &&
            ...
            catalog.path("cityCount").asInt() == variant.cityCount) { "1341 release identity mismatch" }
        ...
        require(entries.size == paths.size && entries.map { it.path("path").asText() }.toSet() == paths) {
            "1341 release artifact path set mismatch"
        }
        val bytes = entries.associate { entry ->
            ...
            require(sha(compressed) == entry.path("compressedSha256").asText()) { "1341 compressed artifact hash mismatch" }
            ...
            require(data.size == length && sha(data) == hash) { "1341 artifact hash/length mismatch" }
