 ...
 *
 * Asserts the V3 seed counts (`world_state`=1, `nation`=2, `city`=1224, `general`=230,
...
class ScenarioImporterIT {
    ...
    fun `importAll seeds the A-minimal world with the expected row counts`() {
        ...
        // V3 projects scenario ownership through stable place identities: Han 123, Yellow 117, neutral 608
        ...
        assertEquals(1224, count("city"))
        // 2026-09-23: 결손 縣 56곳이 1342–1397 을 받았다(예약 번호 1195–1341 은 비워 둔다).
        assertEquals((1..1194).filterNot { it in setOf(1143, 1148, 1157, 1159, 1160, 1161, 1162, 1163, 1164, 1178, 1179, 1180, 1181, 1182, 1183, 1184, 1185, 1186, 1187, 1188, 1189, 1190, 1191, 1192, 1193, 1194) } + (1342..1397), jdbc.queryForList("SELECT id FROM city ORDER BY id", Int::class.java))
        ...
        // 「군현제 안에선 뭐뭐군 뭐뭐현으로 표기해」(2026-09-12)다.
        assertEquals("제남국 역성현", jdbc.queryForObject("SELECT name FROM city WHERE id = 781 AND nation_id = 0", String::class.java))
        // 실효 지배지만 시나리오 소유로 칠하고, 나머지는 공백지로 둔다.
        // 596 → 599 는 城 없던 郡 3곳의 治所(833–835)가 들어오면서다 — 1010 지배표에 없어
        // 셋 다 공백지다. 599 → 606 은 w1 11곳 중 지배표에 없는 7곳이 공백지로 들어오면서다
        // (836은 後漢, 838·839·842는 黃巾 지배표에 있어 소유로 간다).
        ...
        // 공백 → 후한. 868 → 911 · 161 → 167 · 139 → 146.
        assertEquals(911, jdbc.queryForObject("SELECT count(*) FROM city WHERE nation_id = 0", Int::class.java))
        assertEquals(167, jdbc.queryForObject("SELECT count(*) FROM city WHERE nation_id = 1", Int::class.java))  // + 1098 平陰(河南尹, 후한)
        assertEquals(146, jdbc.queryForObject("SELECT count(*) FROM city WHERE nation_id = 2", Int::class.java))
        ...
        // 京師라도 雒陽縣이다 — name 컬럼은 표기라 「하남윤 낙양현」이 들어간다
        // (2026-09-11; 郡을 앞에 세우는 것은 2026-09-12 지시다).
        assertEquals("하남윤 낙양현", sso(nak["name"]))
    ...
    fun `importAll seeds scenario_1030 (군웅할거, 21 nations) without error`() {
        ...
        assertEquals(1224, counts.city)            // Versioned Han V3 catalog, owned and neutral cities.
