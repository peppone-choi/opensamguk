 ...
 *
 * Asserts the V3 seed counts (`world_state`=1, `nation`=2, `city`=1194, `general`=230,
...
class ScenarioImporterIT {
    ...
    fun `importAll seeds the A-minimal world with the expected row counts`() {
        ...
        assertEquals(1194, count("city"))
        assertEquals((1..1194).toList(), jdbc.queryForList("SELECT id FROM city ORDER BY id", Int::class.java))
    ...
    fun `importAll seeds scenario_1030 (군웅할거, 21 nations) without error`() {
        ...
        assertEquals(1194, counts.city)            // Versioned Han V3 catalog, owned and neutral cities.
