 ...
 *
 * Asserts the V3 seed counts (`world_state`=1, `nation`=2, `city`=848, `general`=230,
...
class ScenarioImporterIT {
    ...
    fun `importAll seeds the A-minimal world with the expected row counts`() {
        ...
        assertEquals(848, count("city"))
        assertEquals((1..848).toList(), jdbc.queryForList("SELECT id FROM city ORDER BY id", Int::class.java))
        ...
        assertEquals(608, jdbc.queryForObject("SELECT count(*) FROM city WHERE nation_id = 0", Int::class.java))
        assertEquals(123, jdbc.queryForObject("SELECT count(*) FROM city WHERE nation_id = 1", Int::class.java))
        assertEquals(117, jdbc.queryForObject("SELECT count(*) FROM city WHERE nation_id = 2", Int::class.java))
    ...
    fun `importAll seeds scenario_1030 (군웅할거, 21 nations) without error`() {
        ...
        assertEquals(848, counts.city)             // Versioned Han V3 catalog, owned and neutral cities.
