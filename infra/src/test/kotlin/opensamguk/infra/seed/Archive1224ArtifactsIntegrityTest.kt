...
class Archive1224ArtifactsIntegrityTest {
    @TempDir lateinit var temporary: Path

    @Test fun `new release loads placed gap counties and matching economy in both runtime paths`() {
        val release = Archive1224Artifacts.load(Path.of(".."))
        val previous = Archive1168Artifacts.load(Path.of("..")).cityConst.all().keys
        ...
        for (id in added) {
            ...
            // 1168 판이 더한 취락은 정확 배정(statScale 1)이라 같은 값이었다 — 경로가 다르다.
    ...
    @Test fun `changed catalog and corrupt or missing blob never fall back to current files`() {
        val relative = "data/map/han-world-v3-1224-artifacts-v1"
        ...
        for (mutation in listOf("catalog", "blob", "missing")) {
            ...
            Archive1224Artifacts.load(root) // Establish a valid fixture before introducing the fault.
            when (mutation) {
            ...
            if (mutation == "missing") {
                assertFailsWith<java.nio.file.NoSuchFileException> { Archive1224Artifacts.load(root) }
            } else {
                assertFailsWith<IllegalArgumentException>(mutation) { Archive1224Artifacts.load(root) }
            }
        }
    }
}
