...
class Han1447ArtifactsIntegrityTest {
    @TempDir lateinit var temporary: Path

    @Test fun `new release loads placed gap counties and matching economy in both runtime paths`() {
        val release = Han1447Artifacts.load(Path.of(".."))
        assertEquals(1447, release.cityConst.all().size)
        val previous = Han1224Artifacts.load(Path.of("..")).cityConst.all().keys
        assertEquals(1224, previous.size)
    ...
    @Test fun `changed catalog and corrupt or missing blob never fall back to current files`() {
        ...
        for (mutation in listOf("catalog", "blob", "missing")) {
            ...
            Han1447Artifacts.load(root) // Establish a valid fixture before introducing the fault.
            when (mutation) {
            ...
            if (mutation == "missing") {
                assertFailsWith<java.nio.file.NoSuchFileException> { Han1447Artifacts.load(root) }
            } else {
                assertFailsWith<IllegalArgumentException>(mutation) { Han1447Artifacts.load(root) }
            }
        }
    }
}
