...
class Archive846ArtifactsIntegrityTest {
    ...
    @Test fun `changed catalog and corrupt or missing blob never fall back to current files`() {
        ...
        for (mutation in listOf("catalog", "blob", "missing")) {
            ...
            Archive846Artifacts.load(root) // Establish a valid fixture before introducing the fault.
            when (mutation) {
            ...
            if (mutation == "missing") {
                assertFailsWith<java.nio.file.NoSuchFileException> { Archive846Artifacts.load(root) }
            } else {
                assertFailsWith<IllegalArgumentException>(mutation) { Archive846Artifacts.load(root) }
            }
        }
    }
}
