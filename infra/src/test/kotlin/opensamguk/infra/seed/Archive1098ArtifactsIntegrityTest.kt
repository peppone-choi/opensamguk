...
class Archive1098ArtifactsIntegrityTest {
    ...
    @Test fun `changed catalog and corrupt or missing blob never fall back to current files`() {
        ...
        for (mutation in listOf("catalog", "blob", "missing")) {
            ...
            Archive1098Artifacts.load(root) // Establish a valid fixture before introducing the fault.
            when (mutation) {
            ...
            if (mutation == "missing") {
                assertFailsWith<java.nio.file.NoSuchFileException> { Archive1098Artifacts.load(root) }
            } else {
                assertFailsWith<IllegalArgumentException>(mutation) { Archive1098Artifacts.load(root) }
            }
        }
    }
}
