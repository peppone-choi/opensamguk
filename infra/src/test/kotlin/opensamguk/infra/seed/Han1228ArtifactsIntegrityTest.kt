...
class Han1228ArtifactsIntegrityTest {
    @TempDir lateinit var temporary: Path

    @Test fun `new release loads placed gap counties and exact economy in both runtime paths`() {
        val release = Han1228Artifacts.load(Path.of(".."))
        assertEquals(1228, release.cityConst.all().size)
        ...
        // 결손 縣 60곳만 새로 들어왔고, 1168 판의 城 정체성은 하나도 바뀌지 않았다.
        val added = cities.keys.filter { it !in previous }
        assertEquals(60, added.size)
    ...
    @Test fun `changed catalog and corrupt or missing blob never fall back to current files`() {
        val relative = "data/map/han-world-v3-1228-artifacts-v1"
        ...
        for (mutation in listOf("catalog", "blob", "missing")) {
            ...
            Han1228Artifacts.load(root) // Establish a valid fixture before introducing the fault.
            when (mutation) {
            ...
            if (mutation == "missing") {
                assertFailsWith<java.nio.file.NoSuchFileException> { Han1228Artifacts.load(root) }
            } else {
                assertFailsWith<IllegalArgumentException>(mutation) { Han1228Artifacts.load(root) }
            }
        }
    }
}
