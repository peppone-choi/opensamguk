...
class WorldArtifactsResolverTest {
    private val resolver = WorldArtifactsResolver(Path.of("..").toAbsolutePath().normalize())
    ...
    @Test fun `current generated release resolves complete identities and rejects historical pins`() {
        ...
        val selected = resolver.artifacts(WorldMapVariant.V3_1447_MAP4)
        ...
        assertFailsWith<IllegalArgumentException> {
            resolver.resolve(ids, listOf(WorldTopologyPin("province_control", prior.topologyRevision, prior.contentHash)))
        }
        val currentPin = selected.projection.topology.let {
            WorldTopologyPin("province_control", it.topologyRevision, it.contentHash)
        }
        assertEquals(WorldMapVariant.V3_1447_MAP4, resolver.resolve(ids, listOf(currentPin)).variant)
        assertEquals(WorldMapVariant.V3_1447, resolver.resolve(ids, emptyList()).variant)
        val oldTopology = resolver.artifacts(WorldMapVariant.V3_1447).projection.topology
        assertEquals(WorldMapVariant.V3_1447, resolver.resolve(ids,
            listOf(WorldTopologyPin("province_control", oldTopology.topologyRevision, oldTopology.contentHash))).variant)
    }
    ...
    @Test fun `complete old and current rosters select distinct verified bundles without persisted pins`() {
        ...
        assertEquals(WorldMapVariant.V3_832, old.variant)
        assertEquals(WorldMapVariant.V3_835, current.variant)
    ...
    @Test fun `frozen 846 and 848 rosters select distinct pinned bundles`() {
        ...
        assertEquals(WorldMapVariant.V3_846, selected846.variant)
        val selected848 = resolver.resolve((1..848).toList(), emptyList())
        assertEquals(WorldMapVariant.V3_848, selected848.variant)
    ...
    @Test fun `1098 roster selects its own pinned bundle with strategic-site provinces`() {
        ...
        assertEquals(WorldMapVariant.V3_1098, selected.variant)
    ...
    @Test fun `1133 roster selects its own pinned bundle where every province has a city jurisdiction`() {
        ...
        assertEquals(WorldMapVariant.V3_1133, selected.variant)
    ...
    @Test fun `every persisted pin must match the selected variant`() {
        ...
        val pins = listOf("water_zone_control", "province_control", "general_spatial_position").map {
            WorldTopologyPin(it, old.topologyRevision, old.contentHash)
        }
        assertEquals(WorldMapVariant.V3_832, resolver.resolve((1..832).toList(), pins).variant)
