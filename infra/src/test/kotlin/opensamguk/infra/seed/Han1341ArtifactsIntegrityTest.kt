...
class Han1341ArtifactsIntegrityTest {
    @TempDir lateinit var temporary: Path

    @Test fun `new release loads expanded settlements and exact economy in both runtime paths`() {
        val release = Han1341Artifacts.load(Path.of(".."))
        assertEquals(1341, release.cityConst.all().size)
    ...
    @Test fun `changed catalog and corrupt or missing blob never fall back to current files`() {
        val relative = "data/map/han-world-v3-1341-artifacts-v1"
        ...
        for (mutation in listOf("catalog", "blob", "missing")) {
            ...
            Han1341Artifacts.load(root) // Establish a valid fixture before introducing the fault.
            when (mutation) {
            ...
            if (mutation == "missing") {
                assertFailsWith<java.nio.file.NoSuchFileException> { Han1341Artifacts.load(root) }
            } else {
                assertFailsWith<IllegalArgumentException>(mutation) { Han1341Artifacts.load(root) }
            }
        }
    }
}
