...
class Archive1141ArtifactsIntegrityTest {
    ...
    @Test fun `new release loads eight settlements and exact economy in both runtime paths`() {
        ...
        val gok = listOf(977, 1134, 1135, 1136, 1137).map { cities.getValue(it) }
    ...
    @Test fun `changed catalog and corrupt or missing blob never fall back to current files`() {
        val relative = "data/map/han-world-v3-1141-artifacts-v1"
        ...
        for (mutation in listOf("catalog", "blob", "missing")) {
            ...
            Archive1141Artifacts.load(root) // Establish a valid fixture before introducing the fault.
            when (mutation) {
            ...
            if (mutation == "missing") {
                assertFailsWith<java.nio.file.NoSuchFileException> { Archive1141Artifacts.load(root) }
            } else {
                assertFailsWith<IllegalArgumentException>(mutation) { Archive1141Artifacts.load(root) }
            }
        }
    }
}
