...
class ProvinceCellJsonTest {
    ...
    private fun load(raw: String = fixture): ProvinceCellIndex {
        val bytes = raw.toByteArray(); return ProvinceCellJson.load(topology(bytes),bytes)
    }

    @Test fun `stable province identities preserve disconnected and owned water cells in row major order`() {
        val index = load()
        assertEquals(listOf(ProvinceCell(0,0,'1'),ProvinceCell(0,1,'0'),ProvinceCell(2,1,'1')),index.cellsOf("B"))
        assertEquals(listOf(ProvinceCell(2,0,'0')),index.cellsOf("A"))
    ...
    @Test fun `selected tile bytes must match exact topology pin`() {
        assertFailsWith<IllegalArgumentException> {
            ProvinceCellJson.load(topology(fixture.toByteArray()),(fixture+" ").toByteArray())
        }
    }
    ...
    @Test fun `model copies collections and rejects duplicate cells ordering and bounds`() {
        val cells = mutableListOf(ProvinceCell(0,0,'1'))
        val legend = mutableMapOf('1' to "PLAIN")
        val provinces = mutableMapOf<String,List<ProvinceCell>>("A" to cells)
        val index = ProvinceCellIndex("qa","a".repeat(64),"b".repeat(64),2,2,legend,provinces)
        cells.clear(); legend.clear(); provinces.clear()
        assertEquals(listOf(ProvinceCell(0,0,'1')),index.cellsOf("A"))
        assertEquals("PLAIN",index.terrainLegend['1'])
        assertFailsWith<UnsupportedOperationException> { (index.cellsOf("A") as MutableList<ProvinceCell>).clear() }
        ...
        for (bad in listOf(mapOf("A" to listOf(ProvinceCell(0,0,'1'),ProvinceCell(0,0,'1'))),
            mapOf("A" to listOf(ProvinceCell(1,0,'1'),ProvinceCell(0,0,'1'))),
            mapOf("A" to listOf(ProvinceCell(2,0,'1'))),mapOf("A" to listOf(ProvinceCell(0,0,'9'))),
            mapOf("A" to listOf(ProvinceCell(0,0,'1')), "B" to listOf(ProvinceCell(0,0,'1'))))) {
            assertFailsWith<IllegalArgumentException> {
                ProvinceCellIndex("qa","a".repeat(64),"b".repeat(64),2,2,mapOf('1' to "PLAIN"),bad)
            }
        }
    }
    ...
    @Test fun `actual selected han bundle exposes a province cell inventory without current file fallback`() {
        ...
        assertEquals(ProvinceCell(448,138,'1'),cells.first())
        assertEquals(ProvinceCell(449,146,'1'),cells.last())
