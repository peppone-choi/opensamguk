...
class Han1133ArtifactsIntegrityTest {
    ...
    @Test fun `changed catalog and corrupt or missing blob never fall back to current files`() {
        ...
        for (mutation in listOf("catalog", "blob", "missing")) {
            ...
            Han1133Artifacts.load(root) // Establish a valid fixture before introducing the fault.
            when (mutation) {
            ...
            if (mutation == "missing") {
                assertFailsWith<java.nio.file.NoSuchFileException> { Han1133Artifacts.load(root) }
            } else {
                assertFailsWith<IllegalArgumentException>(mutation) { Han1133Artifacts.load(root) }
            }
        }
    }
}
