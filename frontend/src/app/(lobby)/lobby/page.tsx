...
import { Plus, Globe, UserPlus, Users, Bot, LogIn, Loader2 } from "lucide-react";
...
export default function LobbyPage() {
  ...
  const { worlds, currentWorld, loading: worldsLoading, fetchWorlds, setCurrentWorld, createWorld } = useWorldStore();
  ...
  const handleCreateWorld = async () => {
    ...
    setCreating(true);
    try {
      const world = await createWorld(selectedScenario);
      setCurrentWorld(world);
      clearMyGeneral();
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_1fr]">
      ...
      <div className="space-y-4">
        ...
        {worldsLoading ? (
          ...
          <p className="text-sm text-muted-foreground py-4">생성된 월드가 없습니다.</p>
        ) : (
            ...
            {worlds.map((w) => (
              ...
              >
                <CardContent className="flex items-center justify-between py-3">
                  <div>
                    <p className="font-medium">{w.scenarioCode}</p>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-sm">월드 생성</CardTitle>
          </CardHeader>
        ...
        {!currentWorld ? (
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {currentWorld.scenarioCode} &middot; {currentWorld.currentYear}년 {currentWorld.currentMonth}월
                    </p>
                  </div>
                </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-3">
              <Card className="cursor-pointer hover:border-primary/50 transition-colors" onClick={() => router.push("/lobby/join")}>
                <CardContent className="flex items-center gap-4 py-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">새로운 장수를 만들어 시작합니다.</p>
                  </div>
                </CardContent>
              </Card>
              <Card className="cursor-pointer hover:border-primary/50 transition-colors" onClick={() => router.push("/lobby/select-pool")}>
                <CardContent className="flex items-center gap-4 py-4">
                  <div className="flex items-center justify-center size-10 rounded-lg bg-primary/10 text-primary">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">등록된 장수 중 하나를 선택합니다.</p>
                  </div>
                </CardContent>
              </Card>
              <Card className="cursor-pointer hover:border-primary/50 transition-colors" onClick={() => router.push("/lobby/select-npc")}>
                <CardContent className="flex items-center gap-4 py-4">
                  <div className="flex items-center justify-center size-10 rounded-lg bg-primary/10 text-primary">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">빈 NPC 장수를 인수하여 플레이합니다.</p>
                  </div>
                </CardContent>
              </Card>
            </div>
