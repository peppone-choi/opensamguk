...
function UnitBuilder({
    ...
    (data: Record<string, unknown>) => {
      if (data.objType === "general" && data.data) {
        ...
        onChange({
          ...
          crewType:
            (d.crewtype as number) ?? (d.crewType as number) ?? unit.crewType,
          ...
          specialCode:
            (d.specialWar as string) ??
            (d.special2 as string) ??
            (d.specialCode as string) ??
            "",
          personalCode:
            (d.personal as string) ?? (d.personalCode as string) ?? "",
          injury: (d.injury as number) ?? 0,
          rice: (d.rice as number) ?? 10000,
          defenceTrain:
            (d.defenceTrain as number) ?? (d.defence_train as number) ?? 80,
          officerLevel:
            (d.officerLevel as number) ?? (d.officer_level as number) ?? 1,
          dex1:
            (d.dex1 as number) ?? (d.dex as Record<string, number>)?.["1"] ?? 0,
          dex2:
            (d.dex2 as number) ?? (d.dex as Record<string, number>)?.["2"] ?? 0,
          dex3:
            (d.dex3 as number) ?? (d.dex as Record<string, number>)?.["3"] ?? 0,
          dex4:
            (d.dex4 as number) ?? (d.dex as Record<string, number>)?.["4"] ?? 0,
          dex5:
            (d.dex5 as number) ?? (d.dex as Record<string, number>)?.["5"] ?? 0,
  ...
  const handleLoadGeneral = async (targetId: number) => {
    ...
    try {
      const { data } = await simulatorExportApi.exportGeneral(
        myGeneralId,
        targetId,
      );
    ...
    >
          ...
          {generals.length > 0 && myGeneralId && (
            ...
            >
              <option value="" disabled>
                {loadingGeneral ? "로딩..." : "서버에서 불러오기"}
              </option>
              {generals.map((g) => (
                <option
                  key={g.id}
                  value={g.id}
                  style={g.npcState > 0 ? { color: "#999" } : undefined}
                >
                  {g.name}
                </option>
              ))}
      ...
      <CardContent className="space-y-2">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <NumberField
            label="수비훈"
            value={unit.defenceTrain}
            onChange={(v) => set("defenceTrain", v)}
            min={0}
            max={100}
          />
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          <SelectField
            ...
            options={SPECIAL_CODES}
          />
          <SelectField
            label="성격"
            value={unit.personalCode}
            onChange={(v) => set("personalCode", v)}
            options={["", "의리", "냉정", "호탈"]}
          />
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          <NumberField
            label="관직"
            value={unit.officerLevel}
            onChange={(v) => set("officerLevel", v)}
            min={0}
            max={12}
          />
          <NumberField
        ...
        <div>
          ...
          {showInherit && (
            <div className="mt-1 grid grid-cols-2 gap-1">
              <NumberField
                label="회피"
                value={unit.inheritBuff.warAvoidRatio}
                onChange={(v) =>
                  set("inheritBuff", { ...unit.inheritBuff, warAvoidRatio: v })
                }
              />
              <NumberField
                label="회피저"
                value={unit.inheritBuff.warAvoidRatioOppose}
                onChange={(v) =>
                  set("inheritBuff", {
                    ...unit.inheritBuff,
                    warAvoidRatioOppose: v,
                  })
                }
              />
              <NumberField
                label="필살"
                value={unit.inheritBuff.warCriticalRatio}
                onChange={(v) =>
                  set("inheritBuff", {
                    ...unit.inheritBuff,
                    warCriticalRatio: v,
              ...
              <NumberField
                label="필살저"
                value={unit.inheritBuff.warCriticalRatioOppose}
                onChange={(v) =>
                  set("inheritBuff", {
                    ...unit.inheritBuff,
                    warCriticalRatioOppose: v,
                  })
                }
              />
              <NumberField
                label="계략"
                value={unit.inheritBuff.warMagicTrialProb}
                onChange={(v) =>
                  set("inheritBuff", {
                    ...unit.inheritBuff,
                    warMagicTrialProb: v,
              ...
              <NumberField
                label="계략저"
                value={unit.inheritBuff.warMagicTrialProbOppose}
                onChange={(v) =>
                  set("inheritBuff", {
                    ...unit.inheritBuff,
                    warMagicTrialProbOppose: v,
                  })
                }
              />
            </div>
...
export default function BattleSimulatorPage() {
  ...
  const [defenders, setDefenders] = useState<UnitFormState[]>([
    defaultUnit("방어측"),
  ]);
  ...
  const handleImportConfig = () => {
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        if (config.nationContext !== undefined)
          setNationContext(config.nationContext);
      } catch {
        ...
        <CardContent className="space-y-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              value={nationContext ?? ""}
              onChange={(e) =>
                setNationContext(e.target.value ? Number(e.target.value) : null)
              }
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div className="border border-gray-700 rounded p-2 text-center">
                    ...
                    <div className="text-[10px] text-muted-foreground">
                      {result.repeatSummary.attackerWins}승
                    </div>
                  </div>
                  <div className="border border-gray-700 rounded p-2 text-center">
                    <div className="text-[10px] text-muted-foreground">
                    ...
                    <div className="text-lg font-bold text-red-400 tabular-nums">
                      {(
                        (1 -
                          result.repeatSummary.attackerWinRate -
                          result.repeatSummary.draws /
                            result.repeatSummary.totalRuns) *
                        100
                      ).toFixed(1)}
                      %
                    </div>
                    <div className="text-[10px] text-muted-foreground">
                      {result.repeatSummary.defenderWins}승
                    </div>
                  </div>
                  <div className="border border-gray-700 rounded p-2 text-center">
                    <div className="text-[10px] text-muted-foreground">
                    ...
                    <div className="text-lg font-bold text-gray-400 tabular-nums">
                      {result.repeatSummary.draws}
                    </div>
                  </div>
                  <div className="border border-gray-700 rounded p-2 text-center">
                    ...
                    <div className="text-lg font-bold tabular-nums">
                      {result.repeatSummary.avgRounds.toFixed(1)}
                    </div>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="border border-gray-700 rounded p-2 text-center">
                    ...
                    <div className="text-sm font-bold tabular-nums">
                      {Math.round(
                        result.repeatSummary.avgAttackerRemaining,
                      ).toLocaleString()}
                    </div>
                  </div>
                  <div className="border border-gray-700 rounded p-2 text-center">
                    ...
                    <div className="text-sm font-bold tabular-nums">
                      {Math.round(
                        result.repeatSummary.avgDefenderRemaining,
                      ).toLocaleString()}
                    </div>
                  </div>
                </div>
                ...
                <div className="h-4 rounded-full overflow-hidden flex bg-gray-800">
                  <div
                    ...
                    style={{
                      width: `${result.repeatSummary.attackerWinRate * 100}%`,
                    }}
                  ...
                  <div
                    ...
                    style={{
                      width: `${(result.repeatSummary.draws / result.repeatSummary.totalRuns) * 100}%`,
                    }}
