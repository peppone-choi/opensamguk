...
export default function BattleCenterPage() {
  ...
  const orderedGenerals = useMemo(() => {
    ...
    list.sort((a, b) => {
      switch (orderBy) {
        ...
        case "name": {
          return `${a.npcState} ${a.name}`.localeCompare(
            `${b.npcState} ${b.name}`,
          );
        }
        default:
  ...
  const compareGeneral = useMemo(
    () =>
      compareGeneralId
        ? (orderedGenerals.find((g) => g.id === compareGeneralId) ?? null)
        : null,
  ...
  const compareNation = compareGeneral
    ? nationMap.get(compareGeneral.nationId)
    : null;
          ...
          {orderedGenerals.map((g) => {
            ...
            return (
              <option
                key={g.id}
                value={g.id}
                style={npcColor ? { color: npcColor } : undefined}
              >
      ...
      <div className="flex items-center gap-2">
        ...
        {compareMode && (
          <select
            ...
            onChange={(e) =>
              setCompareGeneralId(Number(e.target.value) || null)
            }
            className="h-8 flex-1 border border-gray-600 bg-[#111] px-2 text-xs text-white rounded min-w-0"
          >
      ...
      <CardContent className="text-xs space-y-4">
        ...
        <div className="grid grid-cols-3 gap-2 text-center font-bold border-b border-gray-700 pb-2">
          <div className="text-left flex items-center gap-1">
            <span style={{ color: getNPCColor(generalA.npcState) }}>
              {generalA.name}
            </span>
            {nationA && (
              <NationBadge name={nationA.name} color={nationA.color} />
            )}
          </div>
          <div className="text-muted-foreground">항목</div>
          <div className="text-right flex items-center justify-end gap-1">
            {nationB && (
              <NationBadge name={nationB.name} color={nationB.color} />
            )}
            <span style={{ color: getNPCColor(generalB.npcState) }}>
              {generalB.name}
            </span>
          </div>
        </div>
        ...
        {stats.map((s) => (
          <div
            key={s.label}
            className="grid grid-cols-3 gap-2 text-center items-center"
          >
            <div
              className={`text-left tabular-nums ${s.a > s.b ? "text-green-400 font-bold" : s.a < s.b ? "text-red-400" : ""}`}
            >
              {s.a.toLocaleString()}
            </div>
            <div className="text-muted-foreground">{s.label}</div>
            <div
              className={`text-right tabular-nums ${s.b > s.a ? "text-green-400 font-bold" : s.b < s.a ? "text-red-400" : ""}`}
            >
              {s.b.toLocaleString()}
            </div>
          </div>
        ...
        <div className="border-t border-gray-700 pt-2">
          ...
          {(["weaponCode", "bookCode", "horseCode", "itemCode"] as const).map(
            (key) => {
...
function GeneralBasicCard({
  ...
  const nameDisplay =
    general.officerLevel > 4 ? `*${general.name}*` : general.name;
    ...
    <Card>
      ...
      <CardContent className="space-y-3 text-xs">
        ...
        <div className="grid grid-cols-3 gap-x-4 gap-y-1">
          ...
          <div>
            <span className="text-muted-foreground">훈련:</span> {general.train}
          </div>
          <div>
            <span className="text-muted-foreground">사기:</span> {general.atmos}
          </div>
          <div>
            <span className="text-muted-foreground">수비훈:</span>{" "}
            {general.defenceTrain}
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-x-4 gap-y-1">
          ...
          <div>
            <span className="text-green-400">승리:</span> {general.killnum ?? 0}
          </div>
          <div>
            <span className="text-red-400">패배:</span> {general.deathnum ?? 0}
          </div>
          <div>
            <span className="text-green-400">살상:</span>{" "}
            {(general.killcrew ?? 0).toLocaleString()}
          </div>
          <div>
            <span className="text-red-400">피해:</span>{" "}
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-1 border-t border-muted/30 pt-2">
          ...
          {general.commandEndTime && (
            <div>
              ...
              <span className="tabular-nums">
                {general.commandEndTime.slice(-8)}
              </span>
            </div>
          )}
      ...
      <CardContent className="text-xs space-y-1">
        <div className="grid grid-cols-2 gap-x-4 gap-y-1">
          <div>
            ...
            {general.personalCode || "-"}
          </div>
          <div>
            <span className="text-muted-foreground">전투특기:</span>{" "}
            {general.special2Code || "-"}
          </div>
          <div>
            <span className="text-muted-foreground">내정특기:</span>{" "}
            {general.specialCode || "-"}
          </div>
          <div>
            <span className="text-muted-foreground">경험:</span>{" "}
            {general.experience.toLocaleString()} (Lv.{general.expLevel})
          </div>
          <div>
            <span className="text-muted-foreground">공헌:</span>{" "}
            {general.dedication.toLocaleString()} (Lv.{general.dedLevel})
          </div>
          <div>
          ...
          <div>
            <span className="text-muted-foreground">나이:</span> {general.age}세
          </div>
          <div>
    ...
    <Card>
      <CardHeader className="pb-1">
        <div className="flex items-center justify-between">
          <CardTitle className={`text-sm ${titleColor ?? ""}`}>
            {title}
          </CardTitle>
