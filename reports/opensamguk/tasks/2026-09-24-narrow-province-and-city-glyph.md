...
- 4방향 마른땅 접경 후보 4,284개 중 초기 건설 도로는 1,941개, 개척 가능 도로는 2,331개, 두 현의 치소에서 접경까지 갈 수 없는 후보는 12개다. 마지막 12개는 공사 대상으로 제시하지 않는다. 본토 건설 도로는 하나의 연결 성분이다. 전체 육로 성분 9개 중 나머지는 주로 섬이다.
...
- 전체 줌에서는 이어진 간선 656개와 간선 간의 이미 건설된 우회로, 역사 통로를 강조한다. 가까이서는 지방길을 그린다. 멀리서의 곡선은 표시만 부드럽게 한 것이며 통행 판정은 원본 격자다.
