...
Run from a fresh isolated Compose stack with caller-supplied ephemeral JWT keys, `INTERNAL_SERVICE_TOKEN`, and unused host ports, as in the court fixture instructions. The Playwright case persists nine screen captures, their API responses, a read-only siege/warehouse DB snapshot, and the phase outcome in the runner artifact directory. Its only post-seed SQL write raises the new human general's `killturn` to 96 so the long QA loop does not delete that account. It restarts the isolated engine after that write: the running engine holds a world snapshot and otherwise overwrites the direct DB adjustment on its next save. The observed appointment route is about 762 km at 30 km per turn, so its arrival wait is 40 minutes within the 50-minute test timeout. The test also asserts that the engine log has no `tick failed` entry.
