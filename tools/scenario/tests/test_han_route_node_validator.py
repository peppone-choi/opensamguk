...
class HanRouteNodeValidatorTest(unittest.TestCase):
    ...
    def test_production_v2_append_is_versioned_and_fail_closed(self) -> None:
        ...
        self.assertEqual(1447, report.approved_count)
        append = documents.migration["appendedRows"]
        self.assertEqual(667, len(append))
        self.assertEqual(781, append[0]["newCityId"])
        self.assertEqual(1620, append[-1]["newCityId"])
        self.assertEqual(320, sum(1 for row in append if "sourceClaimId" in row))
