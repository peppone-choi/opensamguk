...
class HanWorldOwnershipOverrideTest(unittest.TestCase):
    ...
    def test_world_v3_loader_verifies_manifest_and_has_all_832_nodes(self) -> None:
        ...
        self.assertEqual(1228, len({city for group in by_jun.values() for city in group}))  # 2026-09-23: 결손 縣 60곳(城 1195–1254) 편입.
    ...
    def test_all_15_scenarios_migrate_references_and_licheng_owner_from_source(self) -> None:
        ...
        by_jun, id_of, seat_of = apply_han_world.load_world("han-world-v3")
        known = set(active_numeric_ids(1228))
        ...
        self.assertEqual(448, len(migration_doc["appendedRows"]))  # 2026-09-23: 결손 縣 60곳(城 1195–1254) 편입.
        self.assertEqual(
            {
        ...
        self.assertEqual(
            [i for i in active_numeric_ids(1228) if i > 780],  # 1098 = 河南尹 平陰(w4), 1099–1133 = 郡國 밖 취락(w5)
