...
class HanWorldOwnershipOverrideTest(unittest.TestCase):
    ...
    def test_world_v3_loader_verifies_manifest_and_has_all_832_nodes(self) -> None:
        ...
        self.assertEqual(1194, len({city for group in by_jun.values() for city in group}))
    ...
    def test_all_15_scenarios_migrate_references_and_licheng_owner_from_source(self) -> None:
        ...
        self.assertEqual(414, len(migration_doc["appendedRows"]))
        self.assertEqual(
            {
        ...
        self.assertEqual(
            list(range(781, 1195)),  # 1098 = 河南尹 平陰(w4), 1099–1133 = 郡國 밖 취락(w5)
