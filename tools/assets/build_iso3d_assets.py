...
**타일 단위.** 지형 래스터 768×669 를 2×2 로 묶어 384×334 타일이다(사용자 결정 2026-09-11).
처음엔 4×4(192×167, 3만 2천 타일)였다. 한 칸이 지도상 약 12km 라 城 두 곳 이상이 한 칸에
겹쳐 앉는 자리가 39 칸 / 城 79 곳이나 됐다 — 화면에서 대현·맹현 아이콘이 포개져 보였다.
2×2 면 한 칸이 약 6km 이고 겹침이 4 칸 / 城 8 곳으로 줄되 타일은 12만 8천으로 네 배다.
렌더러가 화면에 든 행만 그리므로 전체 인스턴스 수가 아니라 시야 안 타일 수만 늘어난다.
...
#: 래스터 몇 칸을 한 타일로 묶는가. 사용자 결정(2026-09-11) — 2×2(전 4×4).
#: web/shared/src/isoTileGrid.ts 의 RASTER_GROUP 과 반드시 같아야 한다. 매니페스트의
#: rasterGroup/tileGrid 가 그 계약을 런타임에 전달하는 통로다.
RASTER_GROUP = 2
