...
| `che_견문` | sightseeing | **Non-deterministic primary draw: `Util::randF()` → `mt_rand()` (Mersenne Twister), NOT `LiteHashDRBG`.** `SightseeingMessage::pickAction()` (`hwe/sammo/TextDecoration/SightseeingMessage.php:107`) calls `Util::choiceRandomUsingWeightPair()` and `Util::choiceRandom()` which both use `self::randF()` → `mt_rand()` — PHP's Mersenne Twister, seeded per-process by PHP itself (not by the `hiddenSeed`-derived `LiteHashDRBG` stream). The `$rng` (LiteHashDRBG) passed to `run()` is used ONLY for the conditional `Wounded`/`HeavyWounded` injury amounts (`nextRangeInt(10,20)` / `nextRangeInt(20,50)`). Verified by two capture runs on the SAME install (same `seedString`, same `hiddenSeed`): run A logLine `"지나가는 행인에게서 쌀을 <C>300</> 받았습니다."` (draw_count=0), run B logLine `"어느 명사와 설전을 벌여 멋지게 이겼습니다."` (draw_count=0) — different PHP process → different `mt_rand()` seed → different outcome. The capture cannot produce byte-identical fixtures across two independent runs, violating parity rule 5. Fix path: `SightseeingMessage::pickAction()` must be refactored to accept the `LiteHashDRBG`-backed `$rng` argument (two draws: `choiceUsingWeightPair` then `choice`) — this is a PHP oracle change, which is out of scope for the faithful-port. Until then, `che_견문` cannot be golden-captured. |

## Docker-blocked (capture script written, fixture pending manual run)

_(empty — all previously Docker-blocked commands have now been attempted and resolved below)_
