<?php
/**
 * capture_event_원융노병연구.php — single-command C3-event golden capture
 * (devsam-core PHP, grand truth). ONE-SHOT, MANUAL HOST — NEVER CI.
 *
 * Standalone sibling of capture_command_c3.php (which is being co-edited for the OTHER
 * event-research commands — kept DISJOINT to avoid co-widening the same file). Mirrors that
 * harness's structure EXACTLY for the ONE target hwe/sammo/Command/Nation/event_원융노병연구.php:
 ...
 *     so the +120 exp/ded gain keeps explevel/dedlevel unchanged),
 ...
 *
 * event_원융노병연구 PARITY SURFACE (per the manifest_c3 event entry):
 *   - actor action line (general_record action) + general-history line (general_record history),
 *   - national-history line (world_history) "<Y>{name}</>{josaYi} <M>원융노병 연구</> 완료",
 *   - nation gold -= 100000, rice -= 100000, aux[can_원융노병사용]=1 (APPENDED last → insertion order),
 *   - actor exp/ded each += 5*(getPreReqTurn()+1) = 5*24 = 120,
 ...
 *   - ReqNationGold(basegold(0)+100000)=100000 / ReqNationRice(baserice(2000)+100000)=102000:
 *     scenario_1010 후한 starts gold/rice=10000 (short) → bumped to a large round number.
 *   - ReqNationAuxValue(can_원융노병사용,0,"<",1): the flag is UNSET on this install → reachable; the
 ...
 *   php tools/php-golden/capture_event_원융노병연구.php [--out-dir=logic/src/test/resources/golden/p2]
 */
...
// event_원융노병연구.php: NationCommand (ctor 'nation'), pushGeneralActionLog writes exactly ONE
...
// nation treasury bump + clear the can_원융노병사용 flag.
$auxRow = $db->queryFirstField('SELECT aux FROM nation WHERE nation=%i', $nid);
$aux = is_string($auxRow) ? ($auxRow === '' ? [] : Json::decode($auxRow)) : [];
if (!is_array($aux)) { $aux = []; }
unset($aux[NationAuxKey::can_원융노병사용->value]);
$db->update('nation', [
    'gold' => $db->sqleval('GREATEST(gold, %i)', 1000000),   // >= basegold(0)+100000
    'rice' => $db->sqleval('GREATEST(rice, %i)', 1000000),   // >= baserice(2000)+100000
    'aux'  => Json::encode($aux),
], 'nation=%i', $nid);
...
hardAssert($auxKey === NationAuxKey::can_원융노병사용->value, RAW . ": auxType != can_원융노병사용 ({$auxKey})");
