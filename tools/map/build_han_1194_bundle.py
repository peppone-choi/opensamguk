#!/usr/bin/env python3
"""Build/check the 1194-city release; older frozen releases are never rewritten.
...
BUNDLE = ROOT / 'data/map/han-world-v3-1194-artifacts-v1'
...
def outputs():
    ...
    catalog.update(artifactId='han-world-v3-1194', cityCount=1194,
    ...
    constants['variantId'] = 'han-world-v3-1194'
    for entry in constants['files']:
        entry['snapshot'] = entry['snapshot'].replace('1133','1194')
        source = (ROOT/entry['source']).read_text()
        snapshot = ('// Frozen 1194-identity release snapshot. Future generators must not overwrite.\n'+source.replace(Path(entry['source']).stem, Path(entry['snapshot']).stem)).encode()
...
def main():
    ...
    if not a.write:
        if pins['catalog'] not in (ROOT/'infra/src/main/kotlin/opensamguk/infra/seed/Archive1194Artifacts.kt').read_text():problems.append('1194 Kotlin catalog pin')
        if pins['constants'] not in (ROOT/'infra/src/test/kotlin/opensamguk/infra/seed/ArchiveRuntimeConstantsIntegrityTest.kt').read_text():problems.append('1194 constants test pin')
