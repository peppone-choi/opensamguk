...
def main():
    ...
    if not a.write:
        if pins['catalog'] not in (ROOT/'infra/src/main/kotlin/opensamguk/infra/seed/Han1168Artifacts.kt').read_text():problems.append('1168 Kotlin catalog pin')
        if pins['constants'] not in (ROOT/'infra/src/test/kotlin/opensamguk/infra/seed/HanRuntimeConstantsIntegrityTest.kt').read_text():problems.append('1168 constants test pin')
