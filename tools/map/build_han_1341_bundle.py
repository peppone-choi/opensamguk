#!/usr/bin/env python3
"""Build/check the 1341-city release; older frozen releases are never rewritten.
...
BUNDLE = ROOT / 'data/map/han-world-v3-1341-artifacts-v1'
...
def outputs():
    ...
    catalog.update(artifactId='han-world-v3-1341', cityCount=1341,
    ...
    constants['variantId'] = 'han-world-v3-1341'
    for entry in constants['files']:
        entry['snapshot'] = entry['snapshot'].replace('1133','1341')
        source = (ROOT/entry['source']).read_text()
        snapshot = ('// Frozen 1341-identity release snapshot. Future generators must not overwrite.\n'+source.replace(Path(entry['source']).stem, Path(entry['snapshot']).stem)).encode()
...
def main():
    ...
    if not a.write:
        if pins['catalog'] not in (ROOT/'infra/src/main/kotlin/opensamguk/infra/seed/Archive1341Artifacts.kt').read_text():problems.append('1341 Kotlin catalog pin')
