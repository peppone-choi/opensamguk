#!/usr/bin/env python3
"""Build/check the 1224-city release; older frozen releases are never rewritten.
...
BUNDLE = ROOT / 'data/map/han-world-v3-1224-artifacts-v1'
PREVIOUS = ROOT / 'data/map/han-world-v3-1168-artifacts-v1'
...
def outputs():
    ...
    catalog.update(artifactId='han-world-v3-1224', cityCount=1224,
                   sourceBaseCommit='eeb826925353fef8c0dbaa221459826b0a562c70')
    out = {}
    ...
    constants['variantId'] = 'han-world-v3-1224'
    for entry in constants['files']:
        entry['snapshot'] = entry['snapshot'].replace('1168','1224')
        source = (ROOT/entry['source']).read_text()
        snapshot = ('// Frozen 1224-identity release snapshot. Future generators must not overwrite.\n'+source.replace(Path(entry['source']).stem, Path(entry['snapshot']).stem)).encode()
...
def main():
    ...
    if not a.write:
        if pins['catalog'] not in (ROOT/'infra/src/main/kotlin/opensamguk/infra/seed/Archive1224Artifacts.kt').read_text():problems.append('1224 Kotlin catalog pin')
        if pins['constants'] not in (ROOT/'infra/src/test/kotlin/opensamguk/infra/seed/ArchiveRuntimeConstantsIntegrityTest.kt').read_text():problems.append('1224 constants test pin')
