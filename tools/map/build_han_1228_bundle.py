#!/usr/bin/env python3
"""Build/check the 1228-city release; older frozen releases are never rewritten.
...
BUNDLE = ROOT / 'data/map/han-world-v3-1228-artifacts-v1'
...
def outputs():
    ...
    catalog.update(artifactId='han-world-v3-1228', cityCount=1228,
                   sourceBaseCommit='9d78596788ebc4c312bac364c903d010d3bc31ba')
    out = {}
    ...
    constants['variantId'] = 'han-world-v3-1228'
    for entry in constants['files']:
        entry['snapshot'] = entry['snapshot'].replace('1168','1228')
        source = (ROOT/entry['source']).read_text()
        snapshot = ('// Frozen 1228-identity release snapshot. Future generators must not overwrite.\n'+source.replace(Path(entry['source']).stem, Path(entry['snapshot']).stem)).encode()
...
def main():
    ...
    if not a.write:
        if pins['catalog'] not in (ROOT/'infra/src/main/kotlin/opensamguk/infra/seed/Han1228Artifacts.kt').read_text():problems.append('1228 Kotlin catalog pin')
        if pins['constants'] not in (ROOT/'infra/src/test/kotlin/opensamguk/infra/seed/HanRuntimeConstantsIntegrityTest.kt').read_text():problems.append('1228 constants test pin')
