...
CONFIRMED_NOTE_INTERNAL_REFERENCE = frozenset((
    ...
    ('河南尹', '有高都'),   # 新城 註 「有高都城」. 高都 는 옛 城 이름(後漢書 卷109
                            ...
                            # CHGIS 매칭에 우연히 걸린다.
    # 南陽郡 「育陽」 註 「有小長安」(後漢書 卷112 郡國四 원문, data/corpus/
    # hhs-112.txt:216 — 「育陽，邑。有小長安，{{*|漢軍為甄阜所破處。}}有東陽
    # 聚。」). 「小長安」은 育陽 안의 별칭 지명이지 縣이 아니다. 「長安」이
    # cnty_xy 에 京兆尹 도성 長安으로 실존하고 anchor 거리 363.8km 로
    # MAX_KM(400km) 안에 들어 near() 를 통과해 재동기화가 진짜 縣으로
    # 오인했다 — 남양군·경조윤이 동일 좌표를 공유하는 것으로 드러났다
    # (105군국 좌표중복 감사에서 발견).
    ('南陽郡', '有小長安'),
))
