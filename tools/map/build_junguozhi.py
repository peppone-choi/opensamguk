...
# 註 문법: 이 글자들은 **뒤따르는 지명을 목적어로 취한다** — 「故屬琅邪」(옛날
# 琅邪에 속했다) · 「脩武故南陽」(脩武는 옛 이름이 南陽) · 「秦為信都」 ·
# 「春秋時曰祝其」 · 「厭次本富平」 · 「永元十六年分海昏置」 · 「建初四年徙宋公
# 於此」. 목적어 자리의 지명은 **다른 행정단위를 가리키는 참조**이지 새 縣
# 항목이 아니다. 그런데 재동기화 스캔은 CHGIS 사전에 있기만 하면 縣으로 삼켰다.
#
# 이건 개별 사고가 아니라 한 클래스다 — 같은 상투구가 여러 郡에서 같은 가짜를
# 만든다(「故屬信都」 3郡: 清河國·趙國·勃海郡, 「故屬琅邪」 3郡: 東海郡·北海國·
# 東萊郡). 그래서 (郡, 절) 화이트리스트로 하나씩 막지 않는다.
#
# 목록은 지어내지 않고 실측에서 뽑았다: 재동기화 54건의 직전 글자를 전부 찍어
# 屬 35 · 故 7 · 分 2 · 為/本/曰/徙 각 1 이 나왔고, 그 46건을 원문에서 하나씩
# 읽어 **전부 참조**임을 확인했다. 「名」은 같은 문장(趙國 「項羽更名信都」)의
# 같은 구문이라 함께 넣는다. 나머지 8건(직전 글자 「丹」 등)은 진짜 縣이
# 이어지는 자리라 건드리지 않는다 — 巴郡 「涪陵出丹墊江安漢…」의 墊江이 그것이다.
#
# **파싱 규칙이라 파괴적이다.** 이 글자 바로 뒤에서 진짜 縣이 시작하는 郡이
# 있으면 그 縣이 조용히 사라진다. 그런 사례가 원문 전체에 없다는 것이 이
# 규칙의 전제이고, 위 실측이 그 근거다.
NOTE_GOVERNOR = frozenset('屬故本為曰名分徙更')


# 「…山」 뒤에 出/在/， 가 오면 그 山 은 註 안 지물의 머리다 — 縣名의 끝이 아니다.
# 郡國志의 註는 「X山出銅」「X山在西北」「X山，Y水出」처럼 산을 주어로 세우는데,
# CHGIS 결손 구간에서는 이 절이 통째로 DP 분절에 떨어져 산 이름 조각이 縣이 됐다
# (실측 EXTRA: 泰山郡 「岱山」· 益州郡 「裝山」「石室」· 隴西郡 「積石」· 下邳國
# 「葛嶧」「山，」· 南陽郡 「桐柏」「大復」「山，」· 江夏郡 「立章」「山，」· 梁國
# 「山」· 犍為屬國 「山」).
#
# **파괴적인 규칙이라 반대 방향을 먼저 쟀다.** 파서 밖 축(위키문헌 원문)에서 山으로
# 끝나는 진짜 縣은 105郡 전체에 다섯뿐이다 — 梁國 碭山 · 下邳國 髙山 · 南郡 很山 ·
# 桂陽郡 陰山 · 郁林郡 布山. 이 다섯 중 뒤에 出/在/，가 오는 것은 「碭|山出文石」
# 하나뿐이고, 그건 縣名이 아니다: 後漢書 卷53 注[四]「梁國有碭縣」· 讀史方輿紀要
# 卷029「秦置碭郡及碭縣…漢改郡曰梁國，碭縣屬焉」— 縣은 「碭」이고 「山出文石」의
# 山은 芒碭山이다(史記「高祖隱於芒、碭山澤巖石之閒」). 明代 碭山縣은 딴 곳이다
# (讀史方輿紀要 卷050「碭山縣，漢下邑縣也」). 나머지 넷은 뒤가 出/在/，가 아니다
# (南郡 「很山故屬武陵」의 故, 下邳國 「盱台高山潘旌」의 縣 나열).
#
# 수식어(산 이름에서 山 앞부분) 폭은 실측한 3자까지만 본다. 넓히면 수식어가 縣名을
# 가로질러 진짜 縣을 삼킨다 — 4자로 늘리면 益州郡 「律高石室山出錫」에서 律高가,
# 有를 안 빼면 右扶風 「汧有吳嶽山」에서 汧이 통째로 사라진다(둘 다 실측). 「有」는
# NOTE_START 쪽 일반 규칙이 이미 절 단위로 잡으므로 여기서 뺀다.
#
# 이 검사는 절 경계·매칭 직후(q) 자리에서만 본다. run 한복판에서도 보면 수식어
# 시작점을 알 수 없어 같은 방식으로 縣을 가로챈다(益州郡 「勝休俞元裝山出銅」에서
# 勝休·俞元 소실 — 실측). 그래서 run 안의 「裝山」「石室山」「岷山」은 아직 안 잡힌다.
MOUNTAIN_FEATURE = re.compile(r'[^。，有]{0,3}山(?=[出在，])')
...
CONFIRMED_JUN_NAMES_NO_CHGIS = frozenset((
    ...
    ('右扶風', '槐里'),
    # 隴西郡 「安故」・「氐道」. 위 山 규칙이 가짜 「積石」(積石山)을 걷어내자 城數
    # 11 에 하나가 모자라 隴西郡이 FAIL 로 돌아섰다 — 거짓 PASS 가 정직한 FAIL 이
    # 된 것이고, 모자란 자리는 원래 이 두 縣이었다. 원문 「狄道安故氐道養水出此。」
    # 에서 CHGIS 는 全稱 대신 짧은 「安」만 갖고 있어 부분일치가 먼저 먹고, 남은
    # 「故」가 NOTE_GOVERNOR 라 재동기화까지 막혀 「氐道」가 통째로 사라졌다.
    # 사료: 後漢書 卷87 西羌傳 「徙留何等於狄道、安故，至臨洮、氐道、羌道縣」 —
    # 狄道·安故·臨洮·氐道 가 나란히 隴西의 縣으로 나온다. 讀史方輿紀要 卷060
    # 「安故城…漢縣，屬隴西」. 반대로 「積石」은 縣이 아니다 — 元和郡縣圖志 卷39
    # 隴右道 「積石山，一名唐述山…在縣西北七十里」.
    # **이건 증상 봉쇄다.** 원인은 CHGIS 부분일치가 全稱을 가로채는 분절 부류
    # (西安陽→西安 · 西安平→西安 · 北宜春→北宜 · 湔氐道→湔氐)이고, 그 일반 규칙이
    # 서면 이 두 줄은 지워야 한다.
    ('隴西郡', '安故'),
    ('隴西郡', '氐道'),
    ...
    for b in blocks:
        ...
        for s0 in starts:
            ...
            while q < n:
                ...
                # CHGIS 는 그 全稱을 안 갖고 앞 2자만 갖는 자리가 있다(湔氐道·
                # 汶江道·綿虒道 셋 다 cnty_xy 에 없고 湔氐/汶江/綿虒 만 있다).
                # 그러면 위 hit 이 道를 떼어 놓고 가고, 그 道가 다음 縣의 첫
                # 글자로 붙어 가짜 縣이 된다(蜀郡 「道岷」 실측).
                #
                # 두 가지를 막아야 한다. 둘 다 실측으로 잡힌 것이지 예방적
                # 추측이 아니다 — 가드 없이 돌리면 발화 3자리 중 1자리가 파괴였다.
                # (1) 道로 **시작하는** 진짜 縣이 있다 — 代郡 道人. 그 자리에서
                #     道를 앞에 붙이면 代郡이 조용히 깨진다. 뒤 2자가 그 자체로
                #     cnty_xy 에 있으면(=道人) 붙이지 않는다. 현재 산출에서 道로
                #     시작하는 縣은 「道岷」(가짜)과 「道人」(진짜) 둘뿐이다.
                # (2) 「道亭」은 縣이 아니라 亭이다. 郡國志가 자기 안에서 두 번
                #     못박는다: 汝南郡 「陽安，有道亭，故國」(卷110)·京兆尹
                #     「霸陵有枳道亭」(卷109). 우리가 파싱하는 ctext HTML 은
                #     陽安 자리의 「有」를 흘려 「陽安」+「道亭，故國。」 로 끊어
                #     놓기 때문에 그 「有」에 기댈 수 없다. 亭 을 직접 막는다.
                if hit and rest[q+len(hit):q+len(hit)+1] == '道' \
                        and rest[q+len(hit)+1:q+len(hit)+2] != '亭' \
                        and rest[q+len(hit):q+len(hit)+2] not in cnty_xy:
                    # 全稱에 좌표를 물려준다. CHGIS 의 「汶江」 점은 汶江道의
                    # 縣治 그 자체다 — 이름만 全稱으로 고치고 좌표는 그대로다.
                    # (안 물려주면 아래 cnty_xy[nm] 조회가 KeyError 로 죽고,
                    # ok=False 로 떨어뜨리면 멀쩡한 좌표를 버린다.)
                    cnty_xy.setdefault(hit + '道', cnty_xy[hit])
                    hit += '道'
                if hit:
                ...
                if rest[q] == '。':
                    ...
                    q += 1
                    continue
                if MOUNTAIN_FEATURE.match(rest, q):
                    # 「…山出/在/，」— 註 안 지물 절이다. 다음 「。」부터 다시 본다
                    # (starts 가 그 자리를 새 절 시작으로 이미 갖고 있다).
                    break
                if NOTE_START.match(rest, q) or REIGN_NOTE_START.match(rest, q):
                    ...
                    if rest[q] == '有' or any(
                        ...
                        while p < n and rest[p] != '。':
                            ...
                            if hit_len and rest[p-1] not in NOTE_GOVERNOR:
                                resync = p
                                break
                            p += 1
                    if resync is None:
