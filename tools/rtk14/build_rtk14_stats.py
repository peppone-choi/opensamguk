...
def build_one(path, out_dir, rtk, scenario_root=None, dry_run=False):
    ...
    # The synthetic campaign slice has invented NPC lords. RTK14 historic officer
    # matching must leave its reviewed game-design roster intact.
    if relative_path.name == "scenario_990002.json" and scenario.get("worldFormat") == "GENERAL_RETAINER_CAMPAIGN":
        out_path = Path(out_dir) / relative_path
        if not dry_run:
            out_path.parent.mkdir(parents=True, exist_ok=True)
            shutil.copyfile(path, out_path)
        return str(out_path), 0, 0, _empty_assignment_stats(), _inactive_detail(schema, "untouched_synthetic_campaign")
