...
class ClaimNpcHandlerTest {
    @Test fun `possession preserves reduced renown instead of reseeding initial capacity`() {
        ...
        val world = InMemoryTurnWorld(WorldSnapshot(
            state = TurnWorldState(1, 200, 3, 3600, t0, config = mapOf("mapName" to "han-world-v3", "ruleProfile" to "HWIHA")),
            worldId = opensamguk.common.world.WorldId(1), generals = listOf(npc(meta = meta)),
            generalPositionSnapshot = positions, cityLandProvinceById = mapOf(5 to "p")))
