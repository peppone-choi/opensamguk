...
class MakeGeneralHandlerTest {
    @Test fun `HWIHA creation adds truthful policy without changing the five stat draw`() {
        ...
        val budget = assertIs<opensamguk.engine.hwiha.EnlistmentPolicyResult.Ready>(
            opensamguk.engine.hwiha.EnlistmentPolicyReader(hwiha).current(opensamguk.logic.input.EnlistmentRequest(b.generalId, opensamguk.logic.input.EnlistmentMode.RANDOM)))
