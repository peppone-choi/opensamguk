...
class WorldSnapshotLoaderArchiveIT {
    ...
    private lateinit var archiveHistoryReader: ArchiveHistoryReader
    private lateinit var statisticSnapshotReader: StatisticSnapshotReader
    ...
    fun setUp() {
        ...
        archiveHistoryReader = ArchiveHistoryReader(NamedParameterJdbcTemplate(dataSource))
        statisticSnapshotReader = StatisticSnapshotReader(NamedParameterJdbcTemplate(dataSource))
        ...
        jdbc.update(
            ...
            VALUES (7, 1, 'active owner nation', '#777777')
            """.trimIndent(),
        )
        jdbc.update(
            """
            ...
            VALUES (7, 1, 'active owner city', 1, 7, 100, 1000, 10, 1000, 10, 1000, 10, 1000, 10, 1000, 10, 1000, 1)
            """.trimIndent(),
        )
        jdbc.update(
            """
            INSERT INTO general (id, world_id, name, nation_id, city_id, turn_time, user_id)
            VALUES (7, 1, 'active owner general', 7, 7, now(), '77')
            """.trimIndent(),
        )
        jdbc.update(
            """
        ...
        jdbc.update(
            """
            INSERT INTO log_entry (world_id, scope, category, year, month, text, nation_id)
            SELECT 1, 'NATION', 'HISTORY', 200, 1, '국가 추가 기록 ' || gs, 7
        ...
        jdbc.update(
            """
            INSERT INTO log_entry (world_id, scope, category, year, month, text, general_id)
            SELECT 1, 'GENERAL', 'HISTORY', 200, 1, '장수 추가 기록 ' || gs, 7
              FROM generate_series(1, 65) gs
            """.trimIndent(),
        )
        jdbc.update(
        ...
        jdbc.update(
            """
            INSERT INTO log_entry (world_id, scope, category, year, month, text)
            SELECT 1, 'SYSTEM', 'ACTION', 200, 1, '세계 추가 기록 ' || gs
              FROM generate_series(1, 65) gs
            """.trimIndent(),
        )
        jdbc.update(
            ...
            VALUES
              ...
              (NULL, 'inheritance', 'inheritance_88', 'previous', '[4400,null]'::jsonb),
    ...
    fun `archived nation ids are scoped to the current server`() {
        ...
        assertFalse(snapshot.state.meta.containsKey("statisticRows"))
        assertFalse(snapshot.state.meta.containsKey("nationHistory"))
        assertFalse(snapshot.state.meta.containsKey("generalHistory"))
        assertFalse(snapshot.state.meta.containsKey("globalLogs"))
        assertEquals(2, statisticSnapshotReader.snapshotRows(WorldId(1)).size)
        val nation7History = archiveHistoryReader.nationHistory(WorldId(1), 7)
        assertEquals(67, nation7History.size)
        assertEquals("국가 추가 기록 65", nation7History.first())
        assertEquals("과거 기록", nation7History.last())
        val general7History = archiveHistoryReader.generalHistory(WorldId(1), 7)
        assertEquals(67, general7History.size)
        assertEquals("장수 추가 기록 65", general7History.first())
        assertEquals("장수 과거 기록", general7History.last())
        val globalLogTexts = archiveHistoryReader.globalLogs(WorldId(1), "action", 200, 1)
        assertEquals(66, globalLogTexts.size)
        ...
        assertFalse(88 in inheritancePoints.keys)
        val activeOwner = assertNotNull(inheritancePoints[77] as? Map<*, *>)
        val previous = assertNotNull(activeOwner["previous"] as? List<*>)
        assertEquals(1200.0, (previous[0] as Number).toDouble())
        assertEquals(null, previous[1])
        val activeAction = assertNotNull(activeOwner["active_action"] as? List<*>)
        assertEquals(9, (activeAction[0] as Number).toInt())
        assertEquals(mapOf("source" to "old"), activeAction[1])
    }
    ...
    fun `configured world only loads its world-scoped state cohorts`() {
        ...
        assertEquals(listOf(7, 101), snapshot.nations.map { it.id })
        assertEquals(listOf(7, 102), snapshot.cities.map { it.id })
        assertEquals(listOf(7, 103), snapshot.generals.map { it.id })
    }
}
