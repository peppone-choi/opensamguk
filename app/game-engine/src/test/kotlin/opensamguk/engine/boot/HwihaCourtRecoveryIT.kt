...
class HwihaCourtRecoveryIT {
    ...
    @Test fun `same flush admission and issue roll back together and retained retry writes both result sequences once`() {
        ...
        assertNull(HwihaQueuedDispatch.read(after.generals.single { it.id == 10 }.meta))
        val dispatch = assertNotNull(HwihaDispatchState.read(after.generals.single { it.id == 1 }.meta))
    ...
    @Test fun `queued issuer ownership change rejects once after cold reload without issuing target order`() {
        ...
        assertNotNull(HwihaQueuedDispatch.read(fixture.load(id).generals.single { it.id == 10 }.meta))
        ...
        assertNull(HwihaDispatchState.read(after.generals.single { it.id == 1 }.meta))
    ...
    @Test fun `tick expires new phase dispatch before processing false reply from stale starting phase`() {
        ...
        assertEquals(HwihaPhase(200, 5, 1), HwihaDispatchState.read(before.generals.single { it.id == 1 }.meta)!!.dueAt)
        val published = mutableListOf<String>()
        ...
        assertEquals(DispatchStatus.ACCEPTED, HwihaDispatchState.read(target.meta)!!.status)
        assertNotNull(HwihaCountyAssignment.read(target.meta))
        assertEquals(before.retainers, after.retainers)
        assertEquals(HwihaPersonPolicyState.read(before.generals.single { it.id == 1 }.meta),
            HwihaPersonPolicyState.read(target.meta))
    ...
    @Test fun `tick queues after issuer personal action and executes only in next eligible phase`() {
        ...
        assertNotNull(HwihaQueuedDispatch.read(queued.generals.single { it.id == 10 }.meta))
        assertNull(HwihaDispatchState.read(queued.generals.single { it.id == 1 }.meta))
        ...
        val after = fixture.load(id)
        assertNull(HwihaQueuedDispatch.read(after.generals.single { it.id == 10 }.meta))
        assertEquals(request, HwihaDispatchState.read(after.generals.single { it.id == 1 }.meta)!!.dispatchId)
        assertEquals(listOf("reservationAccepted", "executionApplied"), jdbc.queryForList(
            "SELECT result_type FROM command_result WHERE world_id=? ORDER BY result_seq", String::class.java, id))
        assertEquals(listOf(request, request), published)
    }
