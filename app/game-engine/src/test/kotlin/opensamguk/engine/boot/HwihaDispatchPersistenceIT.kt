...
class HwihaDispatchPersistenceIT {
    ...
    @Test fun `pending dispatch cold reload acceptance and repeated reply preserve personal state`() {
        ...
        assertEquals(DispatchStatus.PENDING,HwihaDispatchState.read(pending.generals.single { it.id==1 }.meta)!!.status)
        ...
        assertEquals(county,HwihaCountyAssignment.read(actor.meta)!!.countyId)
