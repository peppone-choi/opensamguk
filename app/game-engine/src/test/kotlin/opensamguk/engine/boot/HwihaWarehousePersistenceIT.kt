...
import opensamguk.engine.hwiha.HwihaWarehouseSettlement
import opensamguk.engine.hwiha.HwihaWarehouseSettlement.Result.*
...
class HwihaWarehousePersistenceIT {
    ...
    @Test fun `five resource settlement flush reload retries ownership and rollback preserve stock`() {
        ...
        val stock = HwihaResources(100, 200, 300, 400, 500)
        val initial = HwihaCountyWarehouse(county, 0, stock)
        fun stored(w: InMemoryTurnWorld) = HwihaCountyWarehouse.read(w.getCityById(county)!!.meta, county)
        var recorder = ChangeRecorder()
        assertEquals(NOT_READY, HwihaWarehouseSettlement(world, recorder).settle(county, 0, 0, HwihaResources()))
        ...
        jdbc.update("UPDATE city SET meta=?::jsonb WHERE world_id=701 AND id=?",
            MetaJson.encode(mapOf("keep" to 17, HwihaCountyWarehouse.META_KEY to initial.toMetaValue())), county)
        world = cold(701)
        val original = world.getCityById(county)!!
        val people = world.listGenerals()
        val cost = HwihaResources(1, 2, 3, 4, 5)
        recorder = ChangeRecorder()
        val settlement = HwihaWarehouseSettlement(world, recorder)
        assertEquals(INSUFFICIENT_STOCK, settlement.settle(county, 0, 0, HwihaResources(horses=501), stock))
        ...
        assertEquals(STALE_REVISION, HwihaWarehouseSettlement(world, ChangeRecorder()).settle(county, 0, 0, cost))
        jdbc.update("UPDATE city SET nation_id=1 WHERE world_id=701 AND id=?", county)
        world = cold(701)
        assertEquals(OWNER_CHANGED, HwihaWarehouseSettlement(world, ChangeRecorder()).settle(county, 0, 1, cost))
        val committed = stored(world)
        recorder = ChangeRecorder()
        assertEquals(APPLIED, HwihaWarehouseSettlement(world, recorder).settle(county, 1, 1, cost))
        ...
        world = cold(701)
        assertEquals(INVALID_STATE, HwihaWarehouseSettlement(world, ChangeRecorder()).settle(county, 1, 2, cost))
    }
}
