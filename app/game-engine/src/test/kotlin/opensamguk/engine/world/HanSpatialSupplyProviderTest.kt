...
class HanSpatialSupplyProviderTest {
    ...
    @Test
    fun `canonical Han topology has valid symmetric province adjacency`() {
        val network = provider().network(1020, emptyList())
        assertEquals(network.provinceOwners.size, network.provinceAdjacency.size)
        assertTrue(network.provinceAdjacency.any(IntArray::isNotEmpty))
        network.provinceAdjacency.forEachIndexed { a, neighbors ->
            assertEquals(neighbors.size, neighbors.toSet().size, "duplicate neighbor at province $a")
            neighbors.forEach { b ->
                assertTrue(b in network.provinceAdjacency.indices && b != a, "invalid neighbor $a->$b")
                assertTrue(a in network.provinceAdjacency[b], "asymmetric edge $a->$b")
            }
        }
    }
