...
class HanStrategicSupplyProviderTest {
    ...
    @Test fun `V3 uses pinned dry land and keeps water out of political province ownership`() {
        ...
        assertEquals(1430, network.provinceOwners.size)  // 2026-09-21 임시 거점 정리(#848): 1,558 → 1,374; 2026-09-23 결손 縣 56곳: → 1,430
        ...
        // 2026-09-23 결손 縣 56곳: 縣 인접 3,767 중 물을 건너는 같은 51 간선을 뺀 마른땅 3,716.
        assertEquals(3716, network.provinceAdjacency.sumOf(IntArray::size) / 2)
        assertNotNull(network.strategicSupply)
        assertEquals(3767, provider.network("han-world-v2", 1020, emptyList()).provinceAdjacency.sumOf(IntArray::size) / 2)  // v3 와 같은 han-tiles (물 걸러내기 없음) — 2026-09-23 결손 縣 56곳 뒤 실측
