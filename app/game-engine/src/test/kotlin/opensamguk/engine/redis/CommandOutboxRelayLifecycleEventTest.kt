...
/**
 * OPENSAM-45 (1-b·1-c·1-d) — outbox 릴레이가 커밋된 결과마다 깨움 신호를 낸다.
 *
 * 고정하는 것은 세 가지다: (1) 폴링 키를 쓴 **뒤에** 신호가 나간다(신호가 자기 결과를 앞지르면
 * FE가 빈 키를 읽고 PENDING으로 물러난다), (2) 신호에는 requestId 말고 아무것도 실리지 않는다
 * (월드 전역 브로드캐스트라 결과 내용을 실으면 남의 결과가 샌다), (3) 발행이 실패해도 결과 처리는
 * 유지된다(신호는 보조 경로다).
 */
class CommandOutboxRelayLifecycleEventTest {
    ...
    @Test
    fun `커밋된 결과마다 requestId 만 담은 신호가 나간다`() {
        ...
        assertEquals(
            listOf("req-1", "req-2"),
            publisher.events.map { assertIs<CommandSettledEvent>(it).requestId },
        )
    }

    /** 신호가 자기 결과를 앞지르면 FE 는 아직 없는 키를 읽고 PENDING 으로 물러난다. */
    @Test
    fun `폴링 키를 쓴 뒤에 신호가 나간다`() {
        val publisher = RecordingPublisher()

        relay(rows("req-3"), publisher).publishPending()

        assertEquals(listOf("key:req-3", "event:commandSettled"), publisher.calls)
    }
