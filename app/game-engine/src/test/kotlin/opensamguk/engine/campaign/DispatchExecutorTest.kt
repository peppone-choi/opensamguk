...
class DispatchExecutorTest {
    ...
    private fun issue(world: InMemoryTurnWorld, recorder: ChangeRecorder) = assertIs<DispatchExecution.Applied>(
        DispatchExecutor(world,recorder).issue("dispatch-1",DispatchRequest(1,2,10))).dispatch
    @Test fun `owned NPC dispatch and acceptance preserve position time reservations and assets`() {
        ...
        val accepted=assertIs<DispatchExecution.Applied>(DispatchExecutor(world,recorder)
        ...
        assertEquals(10,CountyAssignment.read(after.meta)!!.countyId)
        ...
        assertEquals(DispatchFailure.ALREADY_RESOLVED,assertIs<DispatchExecution.Rejected>(
            DispatchExecutor(world,recorder).reply(DispatchReplyRequest(2,pending.dispatchId,false))).reason)
    ...
    @Test fun `refusal consumes loyalty at once and tallies renown for the monthly assessment`() {
        ...
        assertEquals(30,PersonPolicyState.read(world.getGeneralById(2)!!.meta)!!.renownCapacity)
    ...
    @Test fun `a second refusal in the same month costs loyalty again but tallies renown once`() {
        val world=world(); val recorder=ChangeRecorder(); val executor=DispatchExecutor(world,recorder)
    ...
    @Test fun `deadline is twelve world phases across year and late refusal accepts without cost`() {
        ...
        assertEquals(Phase(201,4,1),pending.dueAt)
        val executor=DispatchExecutor(world,recorder)
    ...
    @Test fun `pending dispatch cannot be overwritten and missing refusal policy makes no partial writes`() {
        ...
        val target=world.getGeneralById(2)!!; world.applyGeneralDirtyFree(target.copy(meta=target.meta-PersonPolicyState.META_KEY))
        val before=world.getGeneralById(2); val executor=DispatchExecutor(world,recorder)
    ...
    @Test fun `unanswered dispatch accepts at deadline once without changing personal time`() {
        ...
        assertEquals(10,CountyAssignment.read(world.getGeneralById(2)!!.meta)!!.countyId)
