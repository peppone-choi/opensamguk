...
class TransferHandlerTest {
    ...
    @Test fun `gift debits only the owner and credits a co-located recipient once`() {
        ...
        val handler = TransferHandler(world, ChangeRecorder(), DomesticContext())
        val json = """{"targetGeneralId":1102,"resource":"MONEY","amount":40}"""
        val applied = assertIs<TurnOutcome.Applied>(
        ...
        assertEquals(TransferFailure.ALREADY_PROCESSED.name,
            assertIs<TurnOutcome.Rejected>(handler.handle(TransferInput.GIFT,
