...
class EnlistmentHandler(
    private val world: InMemoryTurnWorld,
    recorder: ChangeRecorder,
    private val hiddenSeed: String,
    private val rngFactory: (String) -> RandUtil = { RandUtil(LiteHashDrbg(it)) },
) {
    private val executor = EnlistmentExecutor(world, recorder)

    fun handle(actorId: Int, argJson: String?, year: Int, month: Int,
        inputId: String = INPUT_ID): TurnOutcome {
        fun reject(reason: EnlistmentFailure) = TurnOutcome.Rejected(inputId, reason.name, reason.message)
        ...
        return when (val result = executor.execute(request) { count -> rng.nextInt(0, count) }) {
            is EnlistmentExecution.Applied -> TurnOutcome.Applied(inputId)
