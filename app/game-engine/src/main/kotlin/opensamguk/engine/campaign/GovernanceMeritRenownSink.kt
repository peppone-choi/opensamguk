 ...
 * 여기서 [DomesticMerit.risen](호구·전답·시장 가운데 하나가 상한의 2% 이상, 2026-09-23 확정)을 한 번 더 건다 — 두 경로의
 * 치적 기준을 하나로 둔다.
 *
...
class GovernanceMeritRenownSink(
    ...
    override fun onCountyIndicatorsRose(event: GovernanceMeritEvent) {
        ...
        val risen = DomesticMerit.risen(
            DomesticMerit.Indicators(event.previous.population, event.previous.agriculture, event.previous.commerce),
            DomesticMerit.Indicators(event.current.population, event.current.agriculture, event.current.commerce),
    ...
    companion object {
        ...
        fun meritStamp(comparedMonth: String): String? {
            val ordinal = try { RenownEvents.monthOrdinal(comparedMonth) } catch (_: IllegalArgumentException) { return null }
            if (ordinal <= 0) return null
            val previous = ordinal - 1
            return RenownEvents.stampOf(previous / 12, previous % 12 + 1)
        }
    }
}
