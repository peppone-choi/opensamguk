...
internal object HwihaNpcEnlistmentSelector {
    fun select(world: InMemoryTurnWorld, actorId: Int, reserved: ReservedTurn): ReservedTurn {
        ...
        if (actor.npcState != NpcType.NPC_LITE || actor.nationId != 0 ||
            actor.meta[HwihaLordStatus.META_KEY] != false ||
        ...
        if (HwihaEnlistmentPrecheck.assess(request, projection) !is EnlistmentAssessment.Eligible) return reserved
        return ReservedTurn(HwihaEnlistmentHandler.INPUT_ID, HwihaEnlistmentInput.canonicalJson(request),
