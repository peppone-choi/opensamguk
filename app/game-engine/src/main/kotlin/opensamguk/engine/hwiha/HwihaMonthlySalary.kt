 ...
 *
 * 인물 카드(장수가 붙은 휘하 카드)마다 그 카드를 **직접 거느린 장수**가 명망 코스트 × [HwihaS3Provisional.SALARY_MONEY_PER_RENOWN_COST]
 * 금을 카드가 있는 곳의 창고망([HwihaWarehouseNetwork])에서 낸다. 못 내면 한 푼도 빼지 않고 충성을 깎는다.
 * 기존 가신 월 유지비(30/30, RetainerMonthlyService)는 HWIHA 에서 꺼져 있다 — 두 재정을 함께 켜지 않는다.
 * 도장([STAMP_KEY])으로 한 달에 한 번만 돈다(징세와 같은 방식, 같은 flush).
 */
class HwihaMonthlySalary(private val world: InMemoryTurnWorld, private val recorder: ChangeRecorder) {
    ...
    fun pay(year: Int, month: Int): Outcome? {
        ...
        for (card in world.listRetainers().sortedBy { it.id }) {
            ...
            if (network.payMoney(master.nationId, network.countiesFor(master.nationId, person.cityId), amount)) {
                ...
                unpaid++
                val loyalty = (card.loyalty - HwihaS3Provisional.UNPAID_SALARY_LOYALTY_LOSS).coerceAtLeast(0)
