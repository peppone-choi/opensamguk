...
class HwihaDomesticTurn(
    ...
    fun beforeMovement(generalId: Int) {
        ...
        if (HwihaPlacementState.META_KEY in general.meta) activatePlacement(generalId)
        if (world.listGenerals().any { HwihaCorpsPolicies.META_KEY in it.meta }) activateCorpsPolicies(generalId)
    ...
    private fun seatCandidate(generalId: Int): Int? {
        ...
        val placed = try { HwihaPlacementState.read(meta)?.active?.order } catch (_: IllegalArgumentException) { null }
    ...
    private fun activatePlacement(generalId: Int) {
        ...
        val stored = try { HwihaPlacementState.read(card.meta) } catch (_: IllegalArgumentException) {
        ...
        if (pending != null) {
            when (val check = HwihaDomesticRules.assessPlacementOrder(generalId, pending, state)) {
                ...
                is DomesticAssessment.Eligible -> {
                    active = if (pending.post == PlacementPost.NONE) null else HwihaActivePlacement(pending, state.now, null)
    ...
    private fun activateCorpsPolicies(commanderId: Int) {
        ...
        for (owner in world.listGenerals().sortedBy { it.id }) {
            ...
            val next = if (!live) policies.with(entry.orderId, commanderId, HwihaPolicySlot(null, null))
    ...
    private fun activateCountyPolicy(countyId: Int, now: HwihaPhase) {
        ...
        val current = try { HwihaCountyPolicyState.read(city.meta) } catch (_: IllegalArgumentException) { return } ?: return
        if (current.slot.pending == null) return
        val slot = current.slot.activate(now)
        val next = if (slot.isEmpty && current.lastApplied == null) null else HwihaCountyPolicyState(slot, current.lastApplied)
        world.updateCityMeta(recorder, countyId, city.meta.withKey(HwihaCountyPolicyState.META_KEY, next?.toMetaValue()))
    }
...
internal class HwihaDomesticCountyEffects(
    ...
    fun apply(countyId: Int, state: HwihaDomesticProjection) {
        ...
        val city = world.getCityById(countyId) ?: return
        val stored = try { HwihaCountyPolicyState.read(city.meta) } catch (_: IllegalArgumentException) {
    ...
    private fun write(before: City, levels: HwihaCountyLevels, baseMeta: Map<String, Any?>, stored: HwihaCountyPolicyState?,
        ...
        if (application != null) meta = meta.withKey(HwihaCountyPolicyState.META_KEY,
            HwihaCountyPolicyState(stored?.slot ?: HwihaPolicySlot(null, null), application).toMetaValue())
