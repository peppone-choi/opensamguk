...
class HwihaSiegeService(
    ...
    private fun withGarrison(city: City, troops: Int, resetCondition: Boolean = false): City {
        val military = if (resetCondition) HwihaCityMilitaryState.INITIAL
            else HwihaCityMilitaryState.read(city.meta, city.defence.coerceAtLeast(0))
    ...
    fun startIfArrived(commanderId: Int): Boolean {
        ...
        if (garrison > 0 && HwihaSiegeRules.maintenance(units.sumOf { it.troops }, garrison,
        ...
        val siege = HwihaSiege(county, ACTIVE, commanderId, corps.ownerGeneralId, corps.orderId, corps.nationId,
            ...
            garrison = garrison, timeline = listOf(entry(now, "START", HwihaSiegeMorale.INITIAL_MORALE, garrison)))
    ...
    fun npcAct(commanderId: Int) {
        ...
        if (corpsTroops(corps).toLong() >= garrison.toLong() * HwihaS3Provisional.NPC_ASSAULT_MIN_RATIO) assault(commanderId)
    }
    ...
    fun assault(actorId: Int): Failure? {
        ...
        val defenceBonus = if (city.defenceMax <= 0) 0 else
            (city.defence.coerceIn(0, city.defenceMax).toLong() * HwihaS3Provisional.ASSAULT_MAX_DEFENCE_BONUS_PERCENT / city.defenceMax).toInt()
        val cityMilitary = try { HwihaCityMilitaryState.read(city.meta, city.defence.coerceAtLeast(0)) }
