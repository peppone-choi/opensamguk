 ...
 * - [close]: 다음 월 경계의 월간 사건 **전**에, 적어 둔 값과 지금 값을 비교해 [HwihaDomesticMerit.risen] 이면
 *   그 縣 의 관할 장수([HwihaRenownHooks.countyHolderIds], 지금 소유 세력 기준)에게 치적을 쌓는다.
 *
...
class HwihaCountyMeritWindow(private val world: InMemoryTurnWorld, private val recorder: ChangeRecorder) {
    ...
    fun close(year: Int, month: Int): List<Int> {
        ...
        if (HwihaRenownEvents.monthOrdinal(window.stamp) >= HwihaRenownEvents.monthOrdinal(HwihaRenownEvents.stampOf(year, month)))
            return emptyList()
        ...
        for ((countyId, open) in window.counties.toSortedMap()) {
            ...
            val close = HwihaDomesticMerit.Indicators(city.population, city.agriculture, city.commerce)
            val max = HwihaDomesticMerit.Indicators(city.populationMax, city.agricultureMax, city.commerceMax)
            if (!HwihaDomesticMerit.risen(open, close, max)) continue
            merited += HwihaRenownHooks.countyHolderIds(countyId, city.nationId, metaById)
        }
        val events = HwihaRenownEventRecorder(world, recorder)
        val recorded = merited.filter { events.record(it, HwihaRenownEventSource.COUNTY_INDICATOR_RISE, window.stamp) }
    ...
    fun open(year: Int, month: Int): Int {
        ...
        for (countyId in world.administrativeCountyIds.sorted()) {
            val city = world.getCityById(countyId) ?: continue
            if (city.nationId == 0 || HwihaRenownHooks.countyHolderIds(countyId, city.nationId, metaById).isEmpty()) continue
    ...
    private data class Window(val stamp: String, val counties: Map<Int, HwihaDomesticMerit.Indicators>)
    ...
    private fun read(): Window? {
        ...
        val stamp = (raw["stamp"] as? String)?.takeIf {
            runCatching { HwihaRenownEvents.monthOrdinal(it) }.isSuccess
        } ?: return null
        val counties = (raw["counties"] as? Map<*, *>)?.entries?.mapNotNull { (key, value) ->
            ...
            id to HwihaDomesticMerit.Indicators(values[0]!!, values[1]!!, values[2]!!)
        }?.toMap() ?: return null
