...
/**
 * 휘하 기록을 쓰는 한 곳 — 종류([HwihaRecordKind])와 식별자(`meta.refs`)를 같은 모양으로 싣는다.
 *
...
internal object HwihaRecords {
    ...
    /**
     * 세력 앞 기록(`scope=NATION`). 공개 사건([HwihaRecordKind.NATION_SUMMARY_KINDS])은 `HISTORY`,
    ...
    fun nation(world: InMemoryTurnWorld, nationId: Int, kind: String, text: String, refs: Map<String, Any?> = emptyMap()) =
        world.pushLog(
            LogEntryDraft(
                scope = "nation",
                category = if (kind in HwihaRecordKind.NATION_SUMMARY_KINDS) "history" else "summary",
    ...
    private fun meta(refs: Map<String, Any?>): Map<String, Any?> =
        linkedMapOf(HwihaRecordKind.REFS_META_KEY to LinkedHashMap(refs))
}
