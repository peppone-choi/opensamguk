...
/**
 * S3 전쟁 결과 경계([HwihaWarOutcomeListener]) → 기록 스트림의 월단평 사건([HwihaRenownEventRecorder]).
 *
 ...
 */
class HwihaWarOutcomeRenownListener(
    private val world: InMemoryTurnWorld,
    private val recorder: ChangeRecorder,
) : HwihaWarOutcomeListener {
    override fun onEncounterResolved(winnerIds: List<Int>, loserIds: List<Int>) {
        ...
        HwihaRenownEventRecorder(world, recorder).onEncounterResolved(winners, losers)
    }
    ...
    override fun onCountyCaptured(countyId: Int, previousNationId: Int, captorNationId: Int, capturerIds: List<Int>) {
        ...
        HwihaRenownEventRecorder(world, recorder).onCountyCaptured(countyId, previousNationId, captorNationId, capturerIds.distinct())
    }
}
