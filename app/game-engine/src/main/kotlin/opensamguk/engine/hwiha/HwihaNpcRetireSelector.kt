...
internal class HwihaNpcRetireSelector(private val context: HwihaDomesticContext,
    ...
    fun select(world: InMemoryTurnWorld, actorId: Int, reserved: ReservedTurn): ReservedTurn {
        if (world.ruleProfile != RuleProfile.HWIHA || reserved.rowExists ||
            ...
            catalog[HwihaRetireInput.INPUT_ID]?.deliveryState?.hasHandler != true) return reserved
        ...
        val successor = state.cards.filter { it.masterId == actorId && it.generalId != null }
            .mapNotNull { it.generalId }.distinct().sorted().firstOrNull {
                HwihaRetireRules.assess(HwihaRetireRequest(actorId, it), state) is HwihaRetireAssessment.Eligible
            } ?: return reserved
        return ReservedTurn(HwihaRetireInput.INPUT_ID,
            HwihaRetireInput.canonicalJson(HwihaRetireRequest(actorId, successor)),
            brief = HwihaRetireInput.INPUT_ID, rowExists = false)
    }
}
