...
class NpcDeploySelector(
    ...
    fun choose(world: InMemoryTurnWorld, actorId: Int): DeployInput? {
        ...
        if (!units.all { SiegeRules.besiegerFed(it.troops, it.provisions) }) {
        ...
        val hops = provinceHops(position, CampaignBalance.NPC_DEPLOY_MAX_EDGES)
        val near = hops.keys
        ...
        val targets = world.administrativeCountyIds.sorted().mapNotNull { countyId ->
            ...
            val garrison = try { CityMilitaryState.read(city.meta, city.defence.coerceAtLeast(0)).troops }
                catch (_: IllegalArgumentException) { return@mapNotNull null }
            if (!hostile || troops < garrison.toLong() * CampaignBalance.NPC_DEPLOY_MIN_RATIO) return@mapNotNull null
    ...
    private fun reliefTarget(world: InMemoryTurnWorld, nationId: Int, position: StrategicNodeRef.LandProvince, troops: Long,
        ...
        return world.listHwihaSieges().filter { it.status == SiegeService.ACTIVE }.sortedBy { it.countyId }.mapNotNull { siege ->
            ...
            if (troops * CampaignBalance.NPC_RELIEF_MIN_RATIO_PERCENT < besiegers * 100) return@mapNotNull null
    ...
    private fun route(from: StrategicNodeRef.LandProvince, to: StrategicNodeRef.LandProvince, edges: StrategicEdgeStateSnapshot) =
        (StrategicPathResolver.resolveLandMarch(topology, StrategicPathRequest(from, to, 1), edges, metrics)
            as? LandMarchPathResult.Resolved)?.path?.takeIf { it.edgeIds.size <= CampaignBalance.NPC_DEPLOY_MAX_EDGES }
