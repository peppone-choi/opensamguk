...
class HwihaTravelHandler(
    ...
    fun handle(inputId: String, actorId: Int, rawJson: String?, requestId: String?, ownerUserId: Int?): HwihaTurnOutcome {
        fun reject(reason: TravelFailure) = HwihaTurnOutcome.Rejected(inputId, reason.name, reason.message)
        ...
        return when (result) {
            ...
            HwihaTravelExecution.NoOrder -> reject(TravelFailure.STATE_UNAVAILABLE)
