...
class HwihaTravelHandler(
    ...
    fun handle(inputId: String, actorId: Int, rawJson: String?, requestId: String?, ownerUserId: Int?): HwihaTurnOutcome {
        ...
        val result = HwihaTravelExecutor(world, recorder, topology, metrics).start(requestId, request, destination, budget) { node ->
            // A lone traveler has no corps battle plan. Block hostile entry until an encounter authority exists.
        ...
        return when (result) {
            ...
            is HwihaTravelExecution.Applied -> {
                ...
                record(actorId, result, hostileEntryBlocked)
    ...
    companion object {
        internal fun record(world: InMemoryTurnWorld, actorId: Int, result: HwihaTravelExecution.Applied,
            hostileEntryBlocked: Boolean = false) {
            val text = when (result.state.checkpoint.stop) {
                ...
                LandMarchStop.ENCOUNTER_UNAVAILABLE -> if (hostileEntryBlocked)
                    "적 군단 또는 요격을 만나 진입 전에 이동을 멈췄습니다."
                else "진입할 省의 군사·반응 상태를 확인할 수 없어 이동을 멈췄습니다."
            ...
            HwihaRecords.general(world, actorId, HwihaRecordKind.MARCH_DIRECT, text,
                linkedMapOf("inputId" to result.state.inputId, "orderId" to result.state.orderId,
                    ...
                    "blocker" to if (hostileEntryBlocked) "HOSTILE_ENTRY" else null,
                    "fatigue" to result.condition?.fatigue,
                    "morale" to result.condition?.morale))
        }
    }

    private fun record(actorId: Int, result: HwihaTravelExecution.Applied, hostileEntryBlocked: Boolean) =
        record(world, actorId, result, hostileEntryBlocked)
}
