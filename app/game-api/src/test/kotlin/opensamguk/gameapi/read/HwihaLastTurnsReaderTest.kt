...
class HwihaLastTurnsReaderTest {
    ...
    @Test fun `12순 창을 최근 순부터, 빈 순도 칸으로, 같은 순은 기록 순으로 싣는다`() {
        ...
        `when`(records.personal(1, from, now)).thenReturn(listOf(
            row(5, 199, 12, 3, HwihaRecordKind.DISPATCH_RECEIVED, "발령이 도착했습니다.", """{"dispatchId":"d-1","countyId":10}"""),
            row(7, 200, 1, 2, HwihaRecordKind.MARCH_ASSIGNMENT, "발령지로 행군하고 있습니다.", """{"stop":"BUDGET_EXHAUSTED"}"""),
            row(8, 200, 1, 2, HwihaRecordKind.RENOWN_EVENT, "월단평 사건", "not json"),
        ))
        `when`(records.summary(3, HwihaRecordKind.NATION_SUMMARY_KINDS, HwihaRecordKind.WORLD_SUMMARY_KINDS, from, now))
            .thenReturn(listOf(
                row(3, 199, 11, 1, HwihaRecordKind.COUNTY_CAPTURED, "縣10을 점령했습니다.", """{"countyId":10}"""),
                row(6, 200, 1, 1, HwihaRecordKind.YUEDAN_ANNOUNCED, "【월단평】", """{"stamp":"0200-01","top":[1,2]}"""),
            ))
        ...
        assertEquals(listOf(HwihaRecordKind.MARCH_ASSIGNMENT, HwihaRecordKind.RENOWN_EVENT), out.turns.first().entries.map { it.kind })
        ...
        assertEquals(listOf(HwihaRecordKind.YUEDAN_ANNOUNCED, HwihaRecordKind.COUNTY_CAPTURED), out.nationSummary.map { it.kind },
            "요약도 최근 순부터다")
