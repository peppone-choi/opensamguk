...
class MapPreviewControllerTest {
    ...
    fun `returns the world-map snapshot with merged coords and contract shape`() {
        // city id 3 = 낙양 (x275,y180 in cities_1010.json); id 1 = 업 (x345,y130); id 999 absent → omitted.
        ...
        mockMvc().perform(get("/api/map/preview"))
            ...
            .andExpect(jsonPath("$.cities[0].x").value(345))
            .andExpect(jsonPath("$.cities[0].y").value(130))
            ...
            .andExpect(jsonPath("$.cities[1].x").value(275))
            .andExpect(jsonPath("$.cities[1].y").value(180))
