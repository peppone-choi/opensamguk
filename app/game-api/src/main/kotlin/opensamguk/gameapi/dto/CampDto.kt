 ...
 */
data class YuedanSelf(val generalId: Int, val renown: Int?, val retinueCost: Int?, val overCapacity: Boolean)

/**
 * 월단평 사유 한 줄 — 마지막 월단평이 그 장수에게 적용한 사건 **종류**와 건수·증감(§2.8 발표, 공개 정보).
 * 사건 원인(어느 조우·어느 縣)은 싣지 않는다(#343).
 */
data class RenownReasonDto(val kind: String, val label: String, val count: Int, val amount: Int)

/**
 * 본인에게만 보이는 대기 사건 — 아직 월단평이 적용하지 않은 집계. [stamp] 달의 사건이며, [amount] 는 지금
 * 곡선 기준 예상 증감이다. 원인([source]·[sourceLabel])은 본인 정보라 싣는다.
 */
data class RenownPendingEventDto(
    val kind: String,
    val label: String,
    val stamp: String,
    val source: String?,
...
data class YuedanRow(
    ...
    val reasons: List<RenownReasonDto> = emptyList(),
)

data class YuedanResponse(
    val status: String,
    val stamp: String? = null,
    val self: YuedanSelf? = null,
    val ranking: List<YuedanRow> = emptyList(),
    /** 본인의 다음 월단평 대기 사건(한 달에 종류당 한 건). */
    val selfPendingEvents: List<RenownPendingEventDto> = emptyList(),
)

data class StockDto(val money: Long, val grain: Long, val iron: Long, val timber: Long, val horses: Long)
...
data class FiveStatsDto(val leadership: Int, val strength: Int, val intel: Int, val politics: Int, val charm: Int)

data class AptitudesDto(val command: Int, val administration: Int, val strategy: Int, val envoy: Int)
...
data class PersonCardDto(
    ...
    val stats: FiveStatsDto?,
    val cost: Int?,
    val aptitudes: AptitudesDto?,
...
data class CampRetinueResponse(
    ...
    val people: List<PersonCardDto> = emptyList(),
    val units: List<RetinueBugokDto> = emptyList(),
)

/** 「지난 순」 기록 한 줄. [kind] 는 `RecordKind`, [refs] 는 종류마다 다른 식별자 묶음이다. */
data class RecordEntryDto(val kind: String, val text: String, val refs: Map<String, Any?>)

/** 지난 순 하나(연·월·순). 기록이 없던 순도 빈 [entries] 로 싣는다 — 명령 목록 12순과 같은 칸 수다. */
data class LastTurnDto(
    val year: Int,
    val month: Int,
    val phase: Int,
    val phaseLabel: String,
    val entries: List<RecordEntryDto>,
)

/** 세력 요약 한 줄 — 본인 세력의 공개 사건(縣 점령·상실)과 세계 공개 사건(월단평 발표)만. */
data class NationSummaryEntryDto(
    val year: Int,
    val month: Int,
    val phase: Int,
    val phaseLabel: String,
...
data class LastTurnsResponse(
    ...
    val nationSummary: List<NationSummaryEntryDto> = emptyList(),
)
