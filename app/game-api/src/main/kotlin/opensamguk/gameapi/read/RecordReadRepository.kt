...
/** 연·월·순 한 칸. 1년 36순. */
data class HwihaTurnStamp(val year: Int, val month: Int, val phase: Int) {
    init { require(month in 1..12 && phase in 1..3) { "invalid turn $year-$month-$phase" } }
    val ordinal: Int get() = year * 36 + (month - 1) * 3 + phase - 1

    companion object {
        fun ofOrdinal(ordinal: Int): HwihaTurnStamp = HwihaTurnStamp(ordinal / 36, (ordinal % 36) / 3 + 1, ordinal % 3 + 1)
    }
}
 ...
 */
@Repository
class HwihaRecordReadRepository(
    ...
    fun personal(generalId: Int, from: HwihaTurnStamp, to: HwihaTurnStamp): List<HwihaRecordRow> = jdbc.query(
        """
    ...
    fun summary(
        ...
        from: HwihaTurnStamp,
        to: HwihaTurnStamp,
    ): List<HwihaRecordRow> = jdbc.query(
        """
        SELECT id, year, month, phase, event_kind, text, meta -> 'refs' AS refs FROM log_entry
    ...
    private fun window(from: HwihaTurnStamp, to: HwihaTurnStamp): Map<String, Any> = mapOf(
        "world" to worldId,
    ...
    private companion object {
        val ROW = RowMapper { rs, _ ->
            HwihaRecordRow(rs.getLong("id"), rs.getInt("year"), rs.getInt("month"), rs.getInt("phase"),
