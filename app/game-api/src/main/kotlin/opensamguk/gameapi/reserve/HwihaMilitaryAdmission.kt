...
class HwihaMilitaryAdmission(private val reader: HwihaDomesticReader,
    ...
    fun canonicalArguments(inputId: String, actorId: Int, ownerUserId: Int?, turnIdx: Int, raw: String?): String {
        ...
        val request = HwihaMilitaryInput.parse(actorId, inputId, raw)
            ?: deny(HwihaMilitaryFailure.INVALID_INPUT.name, HwihaMilitaryFailure.INVALID_INPUT.message)
        if (inputId == HwihaMilitaryInput.MUSTER) {
            ...
            if (check is HwihaMusterAssessment.Rejected) deny(check.reason.name, check.reason.message)
        } else {
            ...
            if (check is HwihaCityMilitaryAssessment.Rejected) deny(check.reason.name, check.reason.message)
        }
        ...
        return HwihaMilitaryInput.canonicalJson(request)
    }
}
