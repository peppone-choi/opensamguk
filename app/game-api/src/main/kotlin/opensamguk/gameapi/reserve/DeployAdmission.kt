...
class DeployAdmission(private val precheck: DeployPrecheckService,
    ...
    fun canonicalArguments(generalId: Int, ownerUserId: Int?, turnIdx: Int, raw: String?): String {
        ...
        val request = DeployInputs.parse(generalId, raw)
            ?: deny(DeploymentFailure.INVALID_INPUT.name, DeployRules.reason(DeploymentFailure.INVALID_INPUT))
        val assessment = precheck.assess(request, ownerUserId.toLong())
        if (assessment is DeploymentAssessment.Rejected) deny(assessment.reason.name, DeployRules.reason(assessment.reason))
        if (catalog[DeployInputs.INPUT_ID]?.deliveryState?.hasHandler != true)
