...
/**
 * 전 서비스 실행 버전 수집기 — gateway 자신(BuildProperties) + game-api/game-engine `/actuator/info` fan-out.
 *
...
class VersionService(
    ...
    /** gateway 자신의 버전을 받아 game-api/game-engine 버전을 합쳐 반환한다. */
    fun collect(gateway: ServiceVersion): VersionResponse {
        val gameApi = fetch(gameApiUrl)
        val gameEngine = fetch(gameEngineUrl)
        val skew = listOf(gameApi, gameEngine).any { it.reachable && it.version != gateway.version }
        return VersionResponse(gateway = gateway, gameApi = gameApi, gameEngine = gameEngine, skew = skew)
    }
