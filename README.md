...
스스로 주공이 되어 휘하, 국가, 작전, 전쟁을 운영합니다. 豫州 S3 슬라이스는 통과했고 기본 경로·pep 컷오버는 진행 중입니다.
...
| 영역 | 상태 | 설명 |
|---|---|---|
| 기존 게임 기반 | 회귀 기준선 | 삼모 명령 엔진·골든은 동결 검증에 보존, 제품 경로에서는 제거 예정 |
| 명령 체계 | S3 구현, 컷오버 진행 | 입력 원장 12행 중 11행 HANDLER_READY. 제품 기본값·라우트·API 교체와 계책 입력은 남음 ([계약](docs/superpowers/specs/2026-09-17-input-registry-contract.md)) |
