...
3. **Selected historical Korean log comparison.** In an explicitly selected frozen-regression scope, compare Josa(조사), markup, prefixes, and ordering against the recorded historical evidence. This comparison does not constrain new product copy. Use `context.addLog(...)`, `context.addPlainLog(...)`, and `context.addGlobalActionLog(...)` for their current architectural roles.
