...
**Result-poll convention (OPENSAM-13/135 — the deny channel the FE polls).** `dispatch` returns a `TurnDaemonCommandResult?`; the handler for an immediate-intake command MUST return a `TurnDaemonCommandResult` carrying `ok`/`reason` for BOTH success AND a PHP-faithful engine deny (a deny is `ok=false`, NOT a thrown exception and NOT a bare `null`). `TurnRunService` pairs each non-null result with its `requestId` and publishes it to the durable result outbox (Redis `commandResultKey` + `CommandResultRepository`), which game-api serves at `GET /api/command/result/{requestId}` and the FE reads via `pollCommandResult`. Returning `null` (or only returning a result on success) makes an engine deny a silent no-op the UI can never surface — match the `ok`/`reason` shape the `AuctionBidHandler`/`PlaceBetHandler`/`MessageHandler` siblings already emit. (Turn-reserved `che_*` ring commands resolve on the general's turn and do not flow this immediate-result path.)
