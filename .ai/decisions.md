...
- Decision (사용자 확정):
  1. 웹게임 정체성만 남기고 새 게임으로 교체한다. 삼모 명령은 동결 회귀 기준선으로만 남긴다(ADR-LITE-042 기준선 불변).
  ...
  10. 공백지는 「난세 개막」 모드다. 세력·주공 없이 시작하고 역사 인물 전원을 본관 縣에 재야로 흩어 두어 등용·거병으로 다툰다.
      `han-world-v3` 최신 판(결정 당시 1098, 작성 시점 1133)으로 새로 만들고, 은퇴한 공백지 15개(현재 `han-world-v2` 780城 지도)는 회귀용으로 둔다. 추가는 ADR-LITE-043
      allowlist 검토를 거친다.
...
- Decision (사용자 위임·수용): 주공 판정 기준(다른 장수의 휘하 카드가 아니고 자기 휘하에 인물 카드가 있는 장수, 봉신 계약 허용)·
  휘하 중첩(주공은 장수 카드 코스트만, 장수의 개인 휘하는 장수 명망에서)·권한 분할(발령은 주공의 조정 결정이고 장수는 행동 소모 없이
  ...
  카드 구조, 보급망 = 재정망과 적 군단이 선 省의 보급 통과 차단, 1·2·3층 구분.
- Supersedes:
  - `docs/design/roadmap.md` 2026-08-27 판 「공개 단계」 표·「커맨드 완결 규칙」.
  - `2026-08-27-public-alpha-rebaseline-design.md` §1 전 카탈로그 알파 조건, §2 legacy 명령 범위, §6 명령 5계층(동시 봉인 WEGO
    battle round), §7 해결 순서, §10 단계 게이트, §11 첫 항. §3 delivery-state 파이프라인은 보존.
  - `2026-08-27-public-alpha-command-contract-freeze.md` P-7·P-13(P-4·P-10 의 sealed WEGO rounds 는 「공격 봉인 계획 + 방어 대응」으로 읽는다).
  - 알파 카탈로그의 제품 범위 정본 지위.
  - `2026-07-12-opensamguk-v2-product-spec.md` §6 명령 3계층·§9 v1 커맨드 진화 규칙.
  ...
  - V55 「기존 NPC 가신은 스스로 행동한다」 규칙(새 게임에서는 거느린 장수의 배치·방침을 따른다).
...
- Consequences:
  ...
  - `CLAUDE.md` 의 RTK14 「미매칭만 50/50」 서술은 현행 빌더와 달라 별도 정정 대상이다.
  - 수치는 게이트 임계값이 아니다. 실측 기준선과 시뮬레이션으로만 정한다.
- Reversal: 이 ADR 과 두 설계 문서를 withdrawn 으로 표시하고, 다음을 이 ADR 이전 판(origin/main `5ba61ad3`)으로 되돌린다 —
  ...
  Amendment(2026-09-17). 코드 변경이 없으므로 되돌릴 구현은 없다.
