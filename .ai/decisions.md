...
- Gates: 제품 소스의 정본 파일·클래스·패키지·저장 키·API/웹 경로에 `Hwiha`/`hwiha`가 남으면 CI를 실패시키고, 코드 식별자·패키지의 설계 버전 표기에도 명시한 예외만 허용한다. 허용 목록은 308 리다이렉트 전용 옛 경로, 세계 형식 가드의 옛 키·삼모 값 거절 목록과 적색 프로브 테스트, 수정하지 않는 옛 Flyway 파일의 정확한 경로·용도로 제한한다. 이 목록은 제품 실행 경로·새 저장 값으로 번지면 실패한다. 제품 소스·테스트·도구의 `SAMMO`·`che_`·`CommandRegistry`·알파 카탈로그 참조는 위 거절·리다이렉트용 정확한 허용 목록 밖에서 0건이어야 한다. `HwihaProbe`와 `che_` 명령 파일을 각각 주입하거나 세계 형식 가드를 제거하면 검사·테스트가 실제로 실패하는 적색 프로브를 남긴다. 옛 키·삼모·가드 누락 세계는 부팅과 API에서 모두 거절한다.
