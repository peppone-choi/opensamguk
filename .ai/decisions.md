...
- Consequences: V57 의 확장이 아니라 새 스키마가 필요하다(방어 측 계획, 조건 칸, 공개 시점, 발동 기록). V57 은 SAMMO 城 강공용으로 그대로
  둔다(SAMMO 무변경). 리플레이는 발동한 조건과 카드 공개 회차를 기록하고 `replay_hash` 에 넣어야 한다. 결정론은
...
- Consequences: 입력 원장의 GENERAL_ACTION 행이 크게 는다. `inputId` 는 기존 코드(`che_…`)를 못 쓰고(`HwihaInputRegistry.kt:142-143`) 표시
  이름만 잇는다. 한 기존 명령을 직접 행동 행과 위임 행이 함께 `replacesLegacy[]` 로 가리킨다 — 계약 문구 후속 정리 필요. 효과 크기·비용·합산
  규칙·직접 징병과 부대 카드의 관계·12순 슬롯 사용은 **미정**(정본 §16.3). SAMMO 무변경.
- Amendment(2026-09-18, 사용자 위임 「너가 판단해」 → 메인 세션 판정): 은퇴도 직접 행동(2단계)으로 남겨 **34개**가 된다. 기술연구·물자조달은 직접
  ...
  `replacesLegacy` 개명과 70개 검사 다대일 허용은 후속 이슈.
...
- Decision:
  ...
  4. 삼모 복귀 env 스위치는 **한 시즌**만 둔다. 기본은 꺼 두고 스위치 제거 [#891](https://github.com/peppone-choi/opensamguk/issues/891)을 시즌 종료에 연결한다. 롤백 검증 뒤 다시 끈다.
