...
## ADR-LITE-044 개정 2 — 지도 설계 층을 화면·규칙 공통 정본으로 삼는다

- Date: 2026-09-26
- Status: approved (산출물·검사만 도입, 소비자 배선은 후속)
- Context: 실제 지형을 그대로 칸에 옮기면 사료·전투에 나오는 강과 산이 드러나지 않는다. 강은 NE 50m를 4×4로
  굵게 새긴 실핏줄이 되고, 산은 어디나 고르게 울퉁불퉁하다. 과장한 강 폭에 城이 잠기기도 한다.
- Decision: `data/curated/han/map-design/`의 **설계 층**을 화면과 규칙이 함께 읽는 지도 정본으로 둔다.
  `data/map/han-tiles.json`은 실제 지형·행정 정체성의 입력이자 정본으로 그대로 남는다.
  설계 층은 han-tiles를 고치지 않고, 그 위에 판정 네 가지를 얹는다:
  강 선·폭, 城 위치 수정, 길 궤적, 해안·호수, 산 편집·산 높이, 땅 피복.
  설계 방법은 지도 일반화 다섯 가지(합치기·드러내기·과장하기·줄이기·생략하기)다.
  모든 값은 격자 칸 `(row, col)`로 정의하며, 렌더러 투영과는 무관하다.
- Limits:
  - **강**
    - 사료로 확인한 강만 넣고, 출처를 반드시 단다.
    - 선마다 4-연결 한 줄이어야 하고, 되돌이·2×2가 없어야 한다.
    - 폭은 위계와 경도로 정한다.
  - **위치 수정**
    - 제 省 안에서 12칸까지 옮긴다.
    - 省을 넘는 것은 口·津 나루 거점뿐이다(6칸 안 물가, `FERRY_MOUTH`).
    - 城 id와 省 배정은 바꾸지 않는다.
  - **산 편집**: 물·길·城(성내 + 1칸)·강 기슭은 편집하지 않는다. 이름난 산에는 출처가 필요하다.
  - **해안·호수**: 위상(땅·물 덩어리 수)과 省 이웃 쌍을 지키는 범위에서만 다듬는다. 城 둘레·길·강 칸은 바다가 되지 않고, 새 땅은 둘레가 한 省일 때만 그 省에 붙는다.
  - **길**: 간선·끝점·지나는 省은 규칙 길 그대로 두고 궤적만 땅을 따라 다시 긋는다. 제 省 안·물 없음·8-연결·끝 칸 보존을 지킨다. 규칙 배선 때 보급 절단(map4 결정 C)을 다시 확인한다.
  - **산 높이**
    - 커밋된 표고(NOAA ETOPO1)로 산 범위를 고친다: 평평한 산 분류는 줄이고, 크게 솟은 평지·분지 분류는 산으로 드러낸다. 고원은 기복이 아주 클 때(가장자리 산벽·안쪽 산맥)만 산이 된다.
    - 고원·사막 경계도 표고의 턱을 따라 다시 긋는다(경계 띠 안에서만). 설계 강 기슭은 사막이 아니다.
    - 지형 분류의 옛 강 칸(NE 50m)은 설계 강으로 대체됐으므로 둘레 땅 분류로 흡수한다.
    - 산 칸마다 1–3단(낮은 산·산·높은 산)을 둔다. 윗단은 아랫단 안쪽 1칸 이상이다.
    - 물·길·강 기슭·城은 산이 되지 않는다.
    - 격자는 커밋하지 않고 매개변수·입력 지문·결과 지문만 커밋한다.
  - **피복**: 커밋 입력에서 결정적으로 만든다. 격자는 커밋하지 않고 매개변수와 sha256 지문만 커밋한다.
  - **판정 입력**: `river-waypoints-v1.json`·`river-dodge-v1.json`은 사람이 판정해 고친다.
    산출물이 입력 지문과 어긋나면 적색이다.
- Gate: `tools/map/build_map_design.py --check` 가 두 가지를 돈다. 하나는 불변식이고, 다른 하나는 위치·산·피복의 결정적 재계산 대조다.
  이 명령은 han-tiles 결합 목록 `map-design-layer`에 올라 있어 CI contracts 잡이 돈다.
  적색 프로브는 `tools/map/tests/test_map_design.py`에 있다.
  강 선을 다시 새기려면 gitignored NE 10m이 필요하다. 그래서 `--build`는 로컬에서만 돌고, han-tiles가 바뀐 뒤에는 `--write-derived`를 쓴다.
- Preserved:
...
## ADR-LITE-067 — 시나리오별 행정 오버레이와 월드 판 고정 (2026-09-25)

- Date: 2026-09-25
- Status: approved for design by the user's 2026-09-25 request; implementation deferred until the code-cleanup freeze ends and #905 merges. This ADR is not evidence that the feature exists.
- Context: the same 城 id 130 is 許縣 in 190, an imperial seat at 許 from 196, and renamed 許昌縣 in 黃初二年 (221). The present 1447-city map gives it `level=10` in every scenario and marks 陽翟 id 122 as 潁川郡 seat. Both physical 城 scale and county jurisdiction need time-specific values. The fixed city roster and topology pins must survive scenario selection and later events. The historical date correction to 221 was confirmed by the user; the original 220 example is not the history-mode rename date.
- Decision:
  1. Use immutable, id-and-content-hash-pinned **administrative delta** files. A scenario manifest lists ordered starting delta ids; a later `applyAdministrativeDelta` event references the same catalog. A single deterministic function applies both. Changes are `rename`, `scale`, `commanderySeat`, and `reassignProvince`; expected-old-value checks replace last-write-wins. The full schema and gates are in [the spec](../docs/superpowers/specs/2026-09-25-administrative-overlay.md).
  2. Preserve every 城 id and 省 id. Rename/scale may change the same 城's names, level, initial/max/expansion limits and seat flag; `reassignProvince` changes only an **existing whole 省**'s county jurisdiction. Province cells, movement/supply topology, typed edges, revision/hash, city anchors and owning parent region stay fixed. Province subdivision requires a **new map release**; a live-pinned release is never overwritten.
  3. Store ordered applied delta ids, per-file SHA-256, base artifact/topology pin and final projection SHA-256 with the world. Engine and API reconstruct the same world-specific projection and fail closed on missing/mismatched artifacts. World-specific caches may not key only on map variant. Old-name search aliases are retained while the displayed name follows scenario time. Historical evidence (book/volume/original text), CHGIS geography evidence, and `GAME_DESIGN` scale choices remain separate.
  4. v1 does not add/retire counties, change a county's parent commandery, or subdivide provinces. Commandery size is derived from its counties and their province jurisdictions; no independent commandery level/max exists in the current model. New identities require map-release/migration work. Event application is phase 2 after scenario-start seeding; it must use the daemon's single write path and be replayable.
- Alternatives rejected: one map artifact per scenario duplicates immutable geometry, produces many pin/CI/release chains and complicates existing-world replay; replacing 城 ids on rename breaks references and `HanWorldArtifactsResolver.resolve` roster selection; editing a frozen map in place invalidates stored topology hashes; treating the 196 imperial seat as a 潁川郡 seat move conflates two offices without source evidence; arbitrary cell-level redrawing changes the province graph and belongs to a new map release.
