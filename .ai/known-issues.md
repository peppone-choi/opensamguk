...
- **`work/opensamguk/han-map-wave`는 커밋 단위로 안전하게 분리 가능한 모양이 아니다**(2026-08-24, #508/#519 실측): `753b8d8d`(병종 인프라)가 git conflict 없이 cherry-pick된다고 해서 자기 완결이 아니었다 — `HanGateIndex.kt` 같은 생성물이 여러 커밋(`34177c3f` 등)에 걸쳐 갱신돼서, 부분 cherry-pick이 conflict 없이도 조용히 CI jvm 실패(`HanGateRegionsTest`, `NoSuchElementException`)를 만든다. 이 브랜치를 다시 쪼갤 필요가 있으면 커밋 단위 cherry-pick 대신 "제거 커밋"(원본 브랜치 유지 + 불필요한 파일만 삭제) 방식을 쓸 것.
- **#507(군/국/현 분리)이 지도 산출물에 아직 안 닿는다** — `data/curated/han/administrative-units.json`(COMMANDERY 85/KINGDOM 20, 정본)은 옳지만 서빙되는 `data/map/han-tiles.json`은 재생성 미완료(구버전, KINGDOM 0). 재생성 시도하면 `build_han_places.py`의 TIER 오분류(侯國/屬國/郡이 KINGDOM으로 잘못 승격, 14/20 群 미탐지)로 막힌다. 상세·재현·해야 할 작업은 #524. 경로망 계약(`_scenario_resources()` 15-vs-31)의 별개 설계 결정 건은 #523. (프로덕션 컨테이너 승격은 이 건과 무관 — 승격만으로 안 닫힌다.)
