...
- `scripts/agent/test-codex-agent-os.sh` was updated to accept tracked-base `max_depth` as fallback for `max_threads` and now returns `PASS: Codex Agent OS contract` locally. The strict checker still reports the user-owned `.codex/config.toml` personal-model finding, and there is still no cross-agent critique artifact for this change in the current run. Do not claim a complete guard pass.
