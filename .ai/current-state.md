...
## OPENSAM-35 V2-0A 격리 게이트 — S0~S6 완료 · fresh 게이트 green · final review cleared — 2026-08-08

- 최종 브랜치 `codex/op-35-v2-0a-final`은 `origin/main` `b847c351`에서 재구성했다. 계획 정본: `docs/superpowers/plans/2026-08-08-opensam-35-v2-0a-isolation-plan.md`. 사용자 결정: 계획 **채택 + S6까지 연속 실행**.
...
- **사용자 결정 C2** — 0A-e의 "s1 profile"은 이 리포에 정의체가 없다(compose 전체에 `profiles:` 키 0건; `s1`은 sibling `opensamguk-docker`의 `servers/<id>.env` 배포 인스턴스). 범위를 **이 리포로 한정**하고 sibling 몫은 [OPENSAM-177](https://pepponechoi-jira.atlassian.net/browse/OPENSAM-177)로 분리해 OPENSAM-35와 연결했다.
...
- **S6 완료** — baseline artifact 5종(`a1-v1-flyway-migration-sha256.txt`·`a1-v1-schema-dump.sql`·`a2-scenario-seed-sha256.txt`·`a3-php-golden-inventory-sha256.txt`·`a4-backend-gate-xml-summary.txt`+로그) + `baseline/MANIFEST.md`가 `docs/loops/opensam-35-v2-0a-2026-08-08/`에 존재. 후속 remediation 뒤 fresh gate로 A4를 교체했다: backend 599 suites / 5,023 tests / failures 0 / errors 0 / skipped 1, web 54 files / 288 tests + typecheck PASS. 증거: `s6-gates-and-baseline.md`.
...
- **fresh 게이트 완료:** `tools/parity/gate.sh backend`는 이제 `:app:gateway-api:test`와 해당 XML root를 포함한다. remediation 뒤 첫 실행은 Docker API HTTP 500으로 Testcontainers 초기화가 실패했으나 Docker 정상화 뒤 단 한 번 재실행해 `BUILD SUCCESSFUL in 19m 36s`, 599 suites / 5,023 tests / failures 0 / errors 0 / skipped 1을 확인했다. `web/game`는 설치된 `pnpm`으로 typecheck + 54 files / 288 tests PASS. 최초 러너의 `corepack` exit 127은 로컬 명령 부재로 격리했고 같은 package manager의 직접 실행으로 닫았다.
- **잔여:** ADR-LITE-026 PR 대화 리뷰 3라운드, PR CI·머지, Jira 전이. 독립 final review는 `cleared`고, 잘못된 트레일러의 기존 `18b8bd95` 이력은 main에 넣지 않으며 최신 main 기반 최종 단일 커밋으로 재구성했다. 배포와 sibling OPENSAM-177 실행은 별도다.
