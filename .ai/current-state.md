...
## OPENSAM-35 V2-0A 격리 게이트 — 구현 완료 · PR #370 Round 3 P2 review `cleared` · remote exact-SHA CI pending · release/deploy 미수행 — 2026-08-08
...
- **Round 3 P2 scope는 T2와 별개로 명시 승인됐다.** 두 P2는 세 source item으로 구현됐다: existing `docker-compose.v2-sandbox.yml`의 `web-game` build arg `ASSET_PREFIX=/game`, 신규 `tools/ops/v2_sandbox_compose_contract_test.sh`의 rendered-Compose fail-closed assertion, 그리고 existing `.github/workflows/ci.yml` `agent-system` job의 `Verify v2 sandbox compose contract` step(동일 script 실행)이다. 이 CI wiring은 locally validated됐지만 remote PR CI run은 아직 관측하지 않았다. 세 항목 모두 T2 paths 밖이므로 plan §4.0a의 canonical existing/approved-file list와 전체 diff가 범위를 판정한다.
...
- **현재 controlling review stage: PR #370 Round 3 = `cleared`.** CodeRabbit Round 1의 23 threads는 resolved/dispositioned이고 historical dirty-tree review(`3c1b357c…`)와 local immutable-SHA review(`54ead4e7…`)는 완료됐다. `agent-system`·`jvm`·`web (gateway)`·`web (game)`의 last observed GitHub CI jobs는 green이지만, 이는 새 contract CI step 전 evidence이며 current remediation의 remote exact-SHA CI run은 아직 관측하지 않았다. Codex Round 3 P2 두 건은 세 scoped source item으로 implemented됐고—Compose `ASSET_PREFIX=/game`, new contract test red-before/green-after, `agent-system` CI step wiring; local production-mode build는 green이며 62 generated files가 `/game/_next/`를 포함한다—후속 independent dirty-tree re-review가 no blockers/fixes/questions/nits로 terminal `cleared`했다. 따라서 두 P2는 resolved이고 sole review Verdict도 `cleared`다. Remote exact-SHA CI와 별도 승인된 merge/release/deploy는 여전히 pending이다. CodeRabbit Round 2는 rate-limited라 completed result가 없고, PR review mentions=3은 completed external reviews=3이 아니다: 실제 PR review results는 CodeRabbit Round 1과 Codex Round 3뿐이며 local reviews는 별도다.
...
- **Round 1 문서 snapshot:** canonical merge-base `:(glob)…/**` remeasurement에서 T1·T2·config·C1 네 subgate는 모두 빈 출력이고 `git diff --check`도 통과했다. baseline artifact 7개 sha256은 MANIFEST와 일치하며 owned Markdown fence는 tagged다. Round 3 terminal `Verdict: cleared`와 `.github/workflows/` review scope를 반영한 `tools/agent-system/check.py --strict --base origin/main --format json`은 0 findings로 passed했다.
- **잔여:** 두 Codex Round 3 P2는 review-resolved다. 새 `agent-system` contract step을 포함한 remote exact-SHA PR CI 관측, 그리고 별도 승인된 commit/push·merge·release/deploy·Jira action은 여전히 pending이다. OPENSAM-177 consumer 실행도 별도다.
