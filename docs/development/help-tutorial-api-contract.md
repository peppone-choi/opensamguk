...
목표 순서는 가입 → 장수 생성 → 첫 출사 → 첫 발령 → 첫 공사 → 첫 등용 → 첫 행군 → 첫 전투다. 첫 두 단계는 계정 범위, 뒤 단계는 현재 월드의 장수 범위다. 가입 증거는 gateway 계정 생성 원천에서 가져오며 장수 생성 전의 `game_event`에 끼워 넣지 않는다. 마지막 전투의 성공 사건과 튜토리얼 월드 방식은 사용자 결정 후 E9 PR에서 고정한다.

## 장수 생성 연계

휘하 장수 생성 백엔드 담당 여부는 사용자 결정 대기다. 현재 `/api/join`과 `/api/select-pool`은 삼모 기반 계약이므로 이 문서가 그 API를 휘하판으로 인정하거나 재사용하지 않는다. 담당 레인이 결정되면 별도 계약 PR에 창작 장수 본관 縣, 역사 인물 선택, 서버당 한 장 제약, 오류와 fixture를 추가한다.
