...
후속 조정 연결의 공통 판정은 `OfficeAppointmentRules.assess`다. 군주 권한, 같은 세력 대상, 지방/중앙 구분, 치소 소유, 공석·겸직 한도를 한 함수로 판정하므로 예약 사전검사와 턴 실행이 같은 실패 사유를 돌릴 수 있다. `OfficeAppointmentExecution.complete`는 수락된 제안을 재임으로 바꾸기 직전에 그 판정을 최신 상태로 다시 실행한다. `dismiss`도 파면 사전검사를 다시 사용한다. 사람 대상의 `OfficeAppointmentOffer`는 기존 발령 `DispatchPolicy.responsePhases`를 응답 기한으로 재사용하고, 기한 도달 시 자동 수락을 우선한다. 새 `officeAppointmentOffer` meta는 version 1과 필드 집합을 엄격히 검사한다. `OfficeTenureTransitions`는 수락→재임→치소 부임→종료를 분리하고, 치소 상실 뒤에는 재임 기록을 유지한 채 resolver가 실효를 거절한다. `OfficeLifecycle`은 사망·은퇴·세력 이동 때 재임을 닫고 치소 상실 순간만 지난 순 사유로 돌려준다. `OfficeNpcSelector`는 같은 판정으로 빈 관할을 고르고 공적·적성 입력값을 선택 근거로 남긴다. 정렬은 공적, 적성, 장수 ID 순이며 숨은 가중치는 없다. 실제 명령 채널과 화면·저장은 공유 파일 조율 뒤 연결한다.
