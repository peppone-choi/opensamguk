...
- Existing ID `1001` pins `peppone-choi/opensamguk-images`, immutable revision `1b6624d886c1b326a2feeda449288b41231df5ef`, source path `icons/1001.jpg`, and the corresponding revision-pinned jsDelivr URL. **Historical note (2026-08-17):** `opensamguk-images` had its history rewritten to drop 2,335 optional-IP-portrait files; that revision no longer exists in the rewritten history (tag `v2026.05.21` now points at `05842c61132fd5a71268fd9babd80ba74e27be62`, same file bytes at the same paths). This entry is left as-is because it records what this ledger's evidence gathering actually observed at the time — the live pin was updated separately in `shared-manifest.json`.
