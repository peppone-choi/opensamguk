...
그리고 **감소 수량 자체도 원문에 있다** (OPENSAM-193에서 뒤늦게 확인 — R3 최초 구현은 이 줄을 놓쳐 정률을 지어냈다):

> "300명 기준 3000명이며, 장수수가 적은 경우, 500명까지로 줄어듭니다."
> — `help__start__other__etcetera.md:67` (바로 위 :64의 감소량을 정량화한 줄. 500 = 3000의 정확히 1/6)

`attritionLoss(garrison, activeGeneralCount)`의 두 번째 인자가 바로 이 스케일이다. 기준 300명에서 3000명·장수 0에서 500명(=최저 1/6)이라는 **끝값 수치는 전부 묘섭 원문에 있는 값**이므로 그대로 쓰고(감소량은 `garrison`에 비례하지 않는다), 그 사이의 보간 곡선만 원문에 없으므로 **UNKNOWN**으로 두고 구현 티켓에서 선형으로 정하되 "묘섭 미명시, 오픈삼국 결정"으로 표기한다.
