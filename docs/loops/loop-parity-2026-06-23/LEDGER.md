...
| 5 | Phase 3 Kotlin long-sim replay gate 초안 — PHP capture fixture를 materialize/replay/capture 비교 표면으로 연결 | before: 신규 active `LongSimReplayGateTest`가 manifest summary에서 `monthlySeedString`을 읽어 `expected=null`로 실패. after: fixture smoke `1 ran/0 failed`, full replay는 `@Disabled` blocked gate로 고정. 수동 probe에서 full replay 첫 divergence는 `gameMonths=12` PHP `nation=12` vs Kotlin `nation=5` | `:app:game-engine:test --tests opensamguk.engine.golden.LongSimReplayGateTest` + 실패 XML first-divergence 확인 | 채점대기(blocked) | seed/draw 오라클 위치는 capture 파일이 맞고, 시간 포맷은 PHP datetime으로 맞춤. 그러나 12개월 장기 AI/거병/건국 경로가 아직 PHP와 불일치해 완전 패러티 게이트를 active로 ship하면 안 됨 |
