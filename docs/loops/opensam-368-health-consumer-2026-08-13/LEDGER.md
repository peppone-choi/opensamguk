...
| 4 | Runtime `StartedAt` supplies a healthcheck-independent, hard five-minute promotion grace. | Managed Compose bypass + ~10-minute compatibility grace RED → GREEN | Extracted workflow state/clock cases + compatibility Compose contract | Adopt | Review proved the managed sibling server Compose has no engine healthcheck and the compatibility `5m + 30×10s` settings could remain `starting` for about ten minutes. The workflow now validates Docker `Status|StartedAt` and defers only `running && age < 300`; age 300, old running, stopped, inspect failure, malformed/future time all leave grace or fail closed. The compatibility healthcheck is separately bounded below five minutes (`4m + 3×10s`, with timeout headroom). No sibling change is required for the scheduler contract. |
