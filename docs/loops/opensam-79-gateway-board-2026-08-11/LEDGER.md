...
### PR #384 V32 world-scope inventory remediation — 2026-08-12

- GitHub Actions run `31561709496` failed the real `V32WorldScopeCompletionMigrationTest` inventory assertion at line 67 after V40 introduced `gateway_board_post` and `gateway_board_comment`. The assertion requires every physical table to be classified exactly once as world-owned, `game_kv`, `world_state`, or explicitly global; V40's users-account tables were not yet in the global inventory.
- The required contract is account-global, not game-world scoped: V40 references only `users.id`, and neither V40 table has `world_id`, `general_id`, or `nation_id`. No schema column was added or widened in this remediation.
- Test-first RED: a new focused PostgreSQL regression asserted that both gateway tables are in the explicit global inventory and lack `world_id`. Before the classification change it failed as intended (`1` test, `1` failure: `gateway account tables must be explicitly classified instead of acquiring a game world`).
- The first global-inventory addition exposed a second valid test-contract distinction: the zero-world V32 fixture verifies survival only for global rows which existed before V32, whereas gateway-board tables are introduced at V40. The canonical test now separates `v32GlobalAllowlist` from `postV32GlobalTables`, combines them only for current physical-table completeness, and retains the V32-era data-survival loop for the former.
- Focused infra GREEN: `JAVA_HOME=$(/usr/libexec/java_home -v 21) ./gradlew --no-daemon --max-workers=1 --console=plain :infra:test --tests 'opensamguk.infra.persistence.V32WorldScopeCompletionMigrationTest' --rerun-tasks -Pkotlin.compiler.execution.strategy=in-process` ended `BUILD SUCCESSFUL in 10m 43s`. Fresh XML: `V32WorldScopeCompletionMigrationTest` `10/0/0`, including the gateway account-global regression.
- Focused board regression GREEN: `JAVA_HOME=$(/usr/libexec/java_home -v 21) ./gradlew --no-daemon --max-workers=1 --console=plain :app:gateway-api:test --tests 'opensamguk.gateway.board.*' --rerun-tasks -Pkotlin.compiler.execution.strategy=in-process` ended `BUILD SUCCESSFUL in 18m 40s`. Fresh XML: read security `6/0/0`, post mutation security `8/0/0`, comment security `5/0/0`, PostgreSQL V40 migration IT `1/0/0` (total `20/0/0`).
- A fresh independent remediation reviewer returned `Verdict: cleared` with no CRITICAL/HIGH/MEDIUM/LOW findings; its PR-visible artifact is `docs/superpowers/reviews/2026-08-12-opensam-79-v32-global-inventory-review.md`.
- `git diff --check` and `python3 tools/agent-system/check.py --strict --base origin/main` both pass with no findings after the review artifact is present. Scoped commit/push and the PR CI rerun remain pending at this ledger point. CI must be green before merge.
