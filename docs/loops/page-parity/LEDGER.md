...
| 26 | W-4 AuctionBidHandler 환불 복제/미달차감 수정 — PHP Auction.php `_bid`/`bidInheritPoint`/`refundBid` 정합: R1 이전입찰 무효화(내 최고입찰≠현재 최고가 행이면 기환불, :399-403/:293-297) + R2 차액 차감(morePoint=amount−유효 이전입찰액, :405,437) + R3 조건부 환불(highestBid≠null && myPrevBid==null 한정, :450-452, 자기 상회입찰 무환불) + 유산포인트 바퀴20 정본 seam(previous 키) + same-run pending merge + host 자기입찰 금지 + startBid 게이트 제거(PHP 무존재) | logic 2123/0/0 + engine 350/0/0 green (신규 AuctionBidHandlerTest 19종 — 복제/미달차감 회귀 가드 + 핑퐁 총량 보존) + 갭 1 닫힘 | 커밋 게이트 XML (결정적, 커밋 db80c05에 기록) — fresh 적대 채점은 LEDGER 미기록 → 재채점 백로그 | 채택 | 종전 결함 3종 근절: (a) 무조건 환불→자기 상회입찰 자원 복제 (b) stale myPrevBid 차액→미달 차감 (c) 유산포인트 auction_bid 키 기록→실잔액 미차감 |
...
- 바퀴 26 fresh 적대 재채점 — W-4 수정(db80c05)은 게이트 XML만 기록, parity-reviewer 채점 미기록
