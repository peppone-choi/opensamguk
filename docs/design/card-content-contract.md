...
새 縣 상태 축은 호구·전답·시장·민심·방비·특산 여섯 가지다. 기존 입력 원장의 인구→호구, 농업→전답, 상업→시장, 치안/신뢰→민심, 수비/성벽→방비로 투영한다. 별도 `InfrastructureNetwork`는 두지 않는다. 도로와 역참은 공사 종류다. 현재 `domestic-v1.json`의 수치와 효과는 잠정값이며, 이 계약은 그 수치를 확정하지 않는다.
...
`data/extracted/item/items.json`의 실제 161행을 `tools/content/build_item_ledgers.py`로 나눈 결과는 한정 보물 100행(availability 1: 40, 2: 60), 무제한 장비 24행, 레지스트리 밖 37행이다. 이 수는 2026-09-24의 관측값이며 validator에 고정하지 않는다. 분리 원장은 `data/curated/han/treasure-cards-v1.json`, `equipment-v1.json`, `items-excluded-v1.json`이다. 마지막 원장에는 옮기지 않은 37행의 코드·이름·종류·원본 행 번호가 모두 있다. 보물/장비 원장은 원본 행을 `legacy`로 온전히 보존하고 새 계약 필드를 따로 둔다. 사료의 책·권 근거가 없는 옛 아이템 이름에는 `게임 용어` 배지를 쓴다.
