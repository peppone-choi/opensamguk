...
| # | 항목 | 규격 | 쓰이는 곳 | 우선 |
|---|---|---|---|---|
| I-1 | **UI 아이콘 SVG 세트 36종** — 부서 6(작전실·국가 운영·군사·정보·광장·기록), 기록 허브 7(명장·황제·장수·세력·NPC·명예의 전당·접속), 명령 상태 4(가능·부족·불가·봉인), 자원 4(금·쌀·병력·군량), 공통 9(검색·갱신·닫기·화살표 4·외부 링크·필터), 로비 타일 6(경매·베팅·외교·서신·관리·회원) | 20×20 격자, 1.5px `currentColor` 각진 선, 16/20/32 에서 판별 | **완료 2026-09-06** — 정본 opensamguk-images PR #7(태그 `v2026.09.06`, `assets/ui-icons/source/*.svg` + `tools/assets/build_ui_icons.py --check`), export `web/{game,gateway}/public/icons/`(sprite `icons.svg` + 개별), 프리미티브 `Icon`(`@opensamguk/ui`, `ICON_NAMES` ↔ sprite 일치 테스트), 이모지 대체: `records-hub` 7 타일·루트 `app/page.tsx` 12 타일 | 1 |
