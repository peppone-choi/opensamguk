...
| S8 | **Inherit resets (inheritResetTurnTime/SpecialWar/SetNextSpecialWar/ResetStat)** | `LEDGER`: FE_MISSING, read-only / ResetStat 409 | ResetTurnTime/SpecialWar/SetNextSpecialWar are in `intakeCodes` + toCommand; `ResetStat` added 2026-06-20 바퀴68 with `TurnDaemonCommand.ResetStat`, result serializer, mapper, dispatcher, handler, and FE/API tests. |
