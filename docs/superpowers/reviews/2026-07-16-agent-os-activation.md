...
> 날짜: 2026-07-16
> 범위: Agent OS 활성화 전 변경 집합 — `docs/agent/` 운영 문서, `.claude/commands/os-*` 런북 9종, `.claude/settings.json` 훅 활성화, `.claudeignore`, `.mcp.json` un-ignore + 4서버 선언, `.github/workflows/claude_review.yml` + `.coderabbit.yaml` 이중 리뷰, `web/gateway`·`web/game` Sentry SDK 배선(`@sentry/nextjs`, env-DSN no-op 설계), 백엔드 3앱(`app/gateway-api`·`app/game-api`·`app/game-engine`) Sentry SDK 배선(`sentry-spring-boot-starter-jakarta`, 에러 캡처 전용, ADR-LITE-008) + compose 서비스별 DSN env 매핑, `scripts/agent/` 가드 확장(.mcp.json 토큰 스캔), `.ai/` 상태층
...
- **`app/gateway-api` · `app/game-api` · `app/game-engine` (백엔드 Sentry SDK, ADR-LITE-008)**: 파리티 안전 — 의존성 + `application.yml` + compose env만 추가하는 계측 배선이며 RNG/로그/`ChangeRecorder`/JPA 쓰기 경로 비접촉(one-daemon-write 무영향). 에러 캡처 전용으로 `traces-sample-rate: 0.0` 고정(특히 game-engine 턴 데몬 핫루프에 트레이싱 계측 금지), `send-default-pii: false`, DSN 빈 값 → SDK 전체 no-op이 기본값(프론트와 동일 설계). 검증: `JAVA_HOME=$(/usr/libexec/java_home -v 21) ./gradlew :app:gateway-api:test :app:game-api:test :app:game-engine:test --rerun-tasks` — 테스트 XML 집계 gateway-api 73/0/0 · game-api 394건 중 393 green + `GameApiApplicationTests` 1건은 postgres 컨테이너 기동 flake로 단독 재실행 green(1/0/0) · game-engine 557/0/0(skip 1 = `LongSimReplayGateTest`, 기왕 P5 백로그 — 이번 변경 무관). compose 2종 `docker compose config` 문법·`SENTRY_DSN` 렌더 검증 통과.
