...
- Admin deploy status visibility for server version promotion after shared git deploys.
...
- Admin deploy status now compares a server's current pinned tag with the newest deployer tag and exposes a visible `최신 버전 있음` promotion path.
...
- `web/gateway`: `PATH=/usr/local/bin:$PATH pnpm exec tsc --noEmit` passed after adding deploy promotion visibility.
...
- Automatic promotion is intentionally not enabled here because the current production policy preserves per-server game pins and excludes `game-engine` from stateless deployer promotion. The implemented path makes promotion availability explicit and one-click from admin.
