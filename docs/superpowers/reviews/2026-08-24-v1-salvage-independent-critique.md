# v1 고아 브랜치 salvage 3묶음 독립 비평

Date: 2026-08-24

Scope: salvage 3묶음 최종 tip — 묶음1 `work/opensamguk/external-places-546` `dccdbe268360dc6c14829a924b6c5750bbe9fa45`(2 files/+1,315, force-push 2회로 `b25a78e9`·`0e6e2ee6`·`41daa6ce` 무효) · 묶음2 `work/opensamguk/status-icons-salvage` `6f018f11`(28 files/+375) · 묶음4 `work/opensamguk/han-map-wave-docs` `78b81095`(2커밋 10 files/+977), 셋 다 base `origin/main` `ad751195` 확인. 검증한 것 = 묶음1 재심(라이브 Wikidata 재생성 후 커밋 블롭과 바이트 대조 · 이전 블롭 `5883acfa` 대비 전문 diff · 새 커밋 메시지의 열화 주장을 주석이 아닌 코드 경로로 확인 · 하류 재생성 가능성 실행 검증) · 묶음2 격리 재생성 27/27 · 묶음4 v1 원문 byte-diff 8문서 + ADR 3절.

Reviewer: 구현자가 아닌 독립 비평 에이전트. 제출자 보고를 근거로 채택하지 않고 직접 재현했다. **종료 코드만으로 판정하지 않고 생성물을 통째로 받아 diff 했다** — 그게 초심에서 X014 한 필드를 집어낸 방법이라 재심에서도 같은 방법을 썼다. 확인 못 한 것은 UNKNOWN 으로 남긴다. 읽기 전용 — 제출자 워크트리 무수정, 내 검토용 워크트리는 `git status --porcelain` 무출력으로 원복 확인했다.

## 묶음별 판정

| 묶음 | 브랜치 / tip | 판정 |
|---|---|---|
| 1 | `external-places-546` `dccdbe26` | **cleared** — B1·B2 둘 다 해소 확인 |
| 2 | `status-icons-salvage` `6f018f11` | **cleared** (불변, 이전 검증 유효) |
| 4 | `han-map-wave-docs` `78b81095` | **cleared** (불변, 이전 검증 유효) |

세 묶음이 모두 같은 판정이라 아래 한 줄이 곧 전체 판정이다.

---

## 묶음 1 재심 — `dccdbe26`

이전 심사에서 올린 막는 지적 2건이 **둘 다 실제로 고쳐졌다.** 재현으로 확인했다.
...
이번엔 주장이 더 강해졌으므로 더 엄하게 봤다. **주석이 아니라 코드가 그렇게 동작하는지** 확인했다.
...
파일이 없으면 `:124` 블록 전체가 건너뛰어져 그 지점들이 `seats` 에 들어가지 못하고 `places` 에도, 산출물에도 없다. **이 지점들을 공급하는 곳이 이 파일뿐이라는 전제 하에서 누락은 필연이다.** 그 전제(CHGIS 220년 레이어가 이들을 안 담는다)는 빌더 docstring·주석이 일관되게 말하고 이 파일의 존재 이유 자체지만, **CHGIS 원본이 없어 실행으로는 확인하지 못했다 — UNKNOWN 으로 남긴다.** 추정으로 통과시키지 않는다. 다만 코드 경로 자체는 명확하다.
...
시간 쓰지 않고 확인만 했다. `.gitignore` 블롭이 이전 tip 과 **동일**(`b56af28f60602f0932bc0198c4bb76c66035cdbf`)하고, 커밋이 건드린 파일은 `.gitignore` + 데이터 2개뿐으로 **`.ai/` 변경 0건**(ADR 미변경). 실효 규칙은 여전히 `!data/map/external-places.json` 한 줄이고 `han-places.json`·`terrain-grid.json` 은 `.gitignore:108` 에 그대로 걸린다. 이전 심사에서 통과시킨 항목이라 재검증하지 않았다.

### R5 B1 수정의 파생 영향 — 없다. 그리고 이 커밋은 하류 재생성을 유발하지 않는다

초심에서 UNKNOWN 으로 남긴 항목인데, **실행 없이 코드로 답이 나왔다.**
...
막는 지적이 없다. 데이터가 생성기와 바이트 일치하고, 커밋 메시지의 주장이 — 이번엔 더 강한 주장인데도 — 코드 경로로 전부 확인된다. 오히려 한 곳(`:278`)은 실제보다 보수적으로 적었다. 출처 판정은 이전 심사에서 이미 통과했고(라이브 재생성으로 65건 좌표 전건이 Wikidata P625 에서 재현됨 → CHGIS 파생 아님), 이번 재생성이 그것을 한 번 더 확인했다.

---

## 묶음 2 — `status-icons-salvage` `6f018f11` (불변)

tip 이 바뀌지 않았다. **이전 검증 그대로 유효하다.** 요약만 남긴다: 빈 디렉터리에 `.py` 하나만 놓고 재생성해 27/27 바이트 일치(커밋본 × 재생성 × v1 3자 대조), preview 와 본품 26장이 함께 일치해 5번 묶음의 위조 패턴과 정반대다. 입력 이미지 없음이 소스·실행 양쪽으로 확인되고, 커밋 메시지의 코드 인용 7건이 전건 실재하며 전수 sweep 결과 그 7곳이 `city.state` 쓰기 지점의 전부였다. 죽은 에셋 26장은 전량 8,503바이트에 생성기가 동봉돼 있고 커밋 메시지가 미참조 사실을 스스로 밝힌다 — 커밋 찬성. 거짓 주장 0건.

## 묶음 4 — `han-map-wave-docs` `78b81095` (불변)

tip 이 바뀌지 않았다. **이전 검증 그대로 유효하다.** 요약만 남긴다: 문서 8건 전부 v1(`4eaad60b`) 블롭과 md5 동일이고, ADR-LITE-044·045·046 세 절이 절 단위 byte-identical 이며 `Status: approved`·승인자·날짜가 원문과 일치한다 — **승인 기록 날조 없음, 창작 없음**(v1 의 044→046→045 배열을 번호순으로 정렬한 것이 유일한 차이이고 내용 무변경). `.ai/known-issues.md` 신규 1행의 수치(`administrative-units.json` COMMANDERY 85/KINGDOM 20, `han-tiles.json` KINGDOM 0)도 main 실측과 일치한다. 커밋되는 pr508 비평문의 인용은 검증 가능한 것이 전부 현 main 에서 성립하고, 어긋나는 4개 지점(F2 의 66k줄 3파일·F3 의 `test_tile_kind_sanity.py` 가 main 에 없음, F5 의 `ci.yml` 행 이동과 파이썬 레인 2→3)은 전부 「당시 기록의 자연스러운 시차」이며 지적의 실질은 살아 있다.

---

## 머지 순서

`git merge-tree --write-tree --messages dccdbe26 78b81095` 계열 검증은 이전 tip(`0e6e2ee6`)에서 이미 충돌 0 / EXIT 0(양방향)을 확인했고, 이번 tip 은 그때와 **`.gitignore` 블롭이 동일하고 `.ai/` 를 건드리지 않으므로** 충돌 표면이 동일하다. ADR 이 묶음 4 에만 있어 중복 append 충돌이 구조적으로 없다. **순서 의존 없음.**
...
- 묶음 2 산출물의 Pillow 메이저 버전 간 이식성 — 12.2.0 에서만 확인.
- 08-23 자 두 비평 문서가 v1 워킹트리에 untracked 로 존재했는지 — 커밋되지 않은 상태는 사후 재현 불가.

## 판정 근거 요약

세 묶음 모두 막는 것이 없다. 묶음 1 은 내가 올린 blocker 2건이 실제로 고쳐졌고 — 데이터는 라이브 재생성과 **0 differing lines**, 이전 블롭 대비 전문 diff 는 X014 한 줄뿐, 커밋 메시지의 거짓 주장은 코드로 검증되는 정확한 주장으로 교체됐다 — 새로 세운 열화 주장 셋을 주석이 아닌 코드 경로로 전부 확인했다. 초심에서 UNKNOWN 이던 파생 영향도 이번엔 답이 나왔다: `kind == 'COMMANDERY'` 분기가 저장소에 0건이라 X014 의 등급 변경은 하류에 무해하고, CHGIS 원본이 없어 이 커밋이 `han-tiles.json` 재생성을 유발할 수도 없다(#536 안전). 묶음 2·4 는 tip 불변이라 이전 판정을 유지한다.
