...
| v2 PRD Rev 2 | 자원 계층, 부하 2트랙, 건설물, deterministic runtime, 모바일 우선 목표 | 112개 커맨드와 3D 지도를 MVP 선행 조건으로 삼지 않음 |
...
3D Blue Marble/SRTM은 제품 차별점이지만 MVP의 선행 조건이 아니다. 2D 지도와 지도 데이터 계약, 도시 marker 크기 규칙, 작전/전장 상태를 먼저 고정하고, 모바일 FPS 증거가 있을 때 3D를 추가한다.
...
`작전 승인 → 원군 지연 → 접근/정찰/요격/전투/전후 처리 → replay 조회 → 가신 신뢰·봉신 충성·도시 민심 변화`
