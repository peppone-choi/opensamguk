...
## 정본 개정 규칙 (product-spec 개정 권한 — OPENSAM-182 / H6)

위 1번의 "제품 사양 정본"은 **고쳐도 되는 문서가 아니라 절차를 밟아 고치는 문서**다. 이 규칙이
없어서 ADR-LITE-025 시점에 product-spec §6(P-4)·§10(P-13)에 대한 판단이 **누락**됐고, 그 침묵이
2026-08-16 Q1/Q3 충돌의 구조적 원인이 됐다(판정 원본
`docs/superpowers/research/2026-08-16-v2-battle-canon-reconcile-p4-p13.md` §1 E1/E2·§3·R8·H6).

1. **언제 개정인가.** 새 스펙·ADR-LITE·구현 계획이 product-spec의 어느 절과 **다른 계약**을 세우면
   그 순간이 개정 시점이다. "새 문서가 그냥 더 자세하다"는 개정이 아니지만, 값·키·불변식·산출물이
   달라지면 개정이다.
2. **누가 승인하나.** product-spec 본문 수정은 **사람 승인**이 있어야 한다. 에이전트는 개정안을
   ADR-LITE로 제출할 수 있을 뿐이며, `Approved by:`에 사람이 적히기 전의 ADR은 개정 권한이 아니다.
   기록 위치는 `.ai/decisions.md`의 ADR-LITE **필수**이고, 본문 수정 커밋은 그 ADR 번호를 인용한다.
3. **supersede는 명시 열거 의무.** 새 스펙이 product-spec 절을 대체하면 **어느 절을 대체하는지 번호로
   열거**해야 한다. 열거되지 않은 절은 **살아 있는 것으로 간주**한다(ADR-LITE-032가 실제로 이 해석을
   적용했다). "언급하지 않음"은 더 이상 유효한 상태가 아니다 — 개정하거나, 비개정임을 한 줄로 적거나,
   둘 중 하나다.
4. **traceability 의무.** 후속 구현 계획의 Spec-to-Task 표는 자기가 구현·대체하는 product-spec 항목
   행을 포함한다. 0행이면 그 계획은 product-spec과의 관계를 **미판단** 상태로 둔 것이며 리뷰에서
   막는다(07-30 계획이 product-spec을 0회 참조한 것이 이번 사고의 관측 증상이다).

**소급 대입** — 이 규칙이 그때 있었다면 ADR-LITE-025는 supersede 목록에 §6·§10을 **넣거나 빼는
판단을 명시**해야 했고(3번), 07-30 구현 계획은 P-4/P-13 행이 없는 traceability 표를 통과시키지
못했다(4번). 즉 이번 충돌은 규칙 부재로 설명되며, 규칙이 있으면 재발하지 않는다.

이 규칙은 **product-spec 본문을 고치지 않는다.** 권한과 절차만 정한다.
