...
- `year·month·phase` 는 **그 장수 턴이 속한 순**이다. 현행은 세계 시계에서 읽는다(§1.2). 데몬은 세계 경계를 먼저 따라잡으므로(`TurnDaemonRunner.kt:269-272`) 평시엔 둘이 같아야 하지만, 지연 복구 중에 어긋나는지는 **UNKNOWN** — 미결 Q2.
...
- **Q2. 시드의 `year·month·phase` 를 세계 시계에서 읽을지, 장수 `turnTime` 에서 계산할지.** 평시엔 같아야 하나 데몬 지연 복구 중 일치 여부는 UNKNOWN 이다. `turnTime` 에서 계산하면 데몬 지연과 무관해진다.
...
- 확인 못 한 것(UNKNOWN 모음): 지터 없는 시드 행의 INSERT 여부와 동률 빈도, 월 3순 같은 시드가 실제로 같은 결과를 내는 빈도, `timestamptz` 마이크로초 절삭이 동률 관계를 바꾸는지, 지연 복구 중 세계 시계와 장수 `turnTime` 의 순 불일치 여부.
