...
상태: 설계 승인 범위(2026-09-25), 제품 구현 전. [ADR-LITE-069](../../../.ai/decisions.md) 결정의 구현 계약이다. 근거의 `경로:줄`은 `origin/main` 04fa428a 기준이며, 개명·삼모 삭제 작업 후 `rename-map.md`로 경로를 다시 해석해야 한다. 제품 코드·DB·웹은 이 PR에서 바꾸지 않는다.
