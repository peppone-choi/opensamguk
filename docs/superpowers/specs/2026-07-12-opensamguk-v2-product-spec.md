...
- v2 sandbox world 1개.
- 도시 20개, 세력 3개, 장수 50명.
- 플레이어 장수 1명, 추종 장수 1명, 가신 1명, 도독부 1개, 봉토 1개.
...
```text
subjectType: GENERAL | FOLLOWER | RETAINER | BUGOK | SUBFACTION
subjectId: 대상 식별자
...
targetCityId
arrivalWindow
participants: generals, followers, bugok, subfaction forces
...
- 지도: 2D 기본, 3D 선택 모드. 도시 marker는 detail/basic 크기 규칙을 명시한다.
...
- 실시간 병사 단위 조작, 자유 카메라가 필요한 3D 전투 장면, cinematic, 런타임 LLM, 결제·인앱, 네이티브 앱, 다국어.
...
v2의 첫 전투는 별도 2D 실시간 전장으로 만든다. 도시 간 전략 지도와 전투 현장 지도를 분리하고, 플레이어는 병사 하나가 아니라 장수·부곡·원군으로 구성된 대형 부대를 직접 지휘한다.
...
- 하나의 작전군은 주공 장수 1명과 부장 장수 최대 2명의 지휘 집단으로 구성한다. 각 장수는 자신의 부곡·추종·특수병을 가진 retinue를 지휘한다.
...
v2 MVP에서는 한 명의 플레이어가 주공 부대와 추종 부대를 직접 지휘하고 가신·원군을 위임하는 방식으로 시작한다. 이후 협동 작전에서 역할 지휘관을 여러 유저에게 열되, 관전자와 자동 위임 모드도 함께 제공한다. 이렇게 해야 전투가 유저의 판단을 요구하면서도 접속 시간이 긴 유저만 유리한 실시간 조작 게임으로 변하지 않는다.
...
| 부장·추종 장수 | 맡은 목표와 행동 규칙 안에서 독립 실행 |
...
코삭스식 실시간 전투는 가능하지만, 클라이언트가 전투를 소유하면 replay와 공정성이 무너진다. 서버가 고정 tick으로 상태를 계산하고, 클라이언트는 명령과 카메라·애니메이션만 담당해야 한다. 처음에는 2D 연속 좌표·소수 대형·짧은 전투 인스턴스로 성능을 증명한 뒤, 대형 수와 실시간 입력을 늘린다.
