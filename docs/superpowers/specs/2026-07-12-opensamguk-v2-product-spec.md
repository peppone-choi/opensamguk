...
v2의 첫 제품 약속은 “기능이 많은 삼국지”나 “더 큰 CHE”가 아니라 다음 반복 장면이다.

> 돌아온 플레이어가 지난 접속 이후 사건의 인과를 읽고, 동료와 판단한 뒤 명령과 위임을 남긴다.
> 서버가 작전을 실행하면 이동·보급·전투·정치의 결과가 살아 있는 편년체와 replay로 이어지고,
> 플레이어는 그 결과를 근거로 다음 결정을 내린다.

제품 정체성은 **비동기 작전실 + 살아 있는 편년체**다. samnet의 공개 세계 생동감과 replay,
묘삼에서 확인되는 도시 자원·주둔·인물 배치, 삼모의 예약턴 사회성을 참고하되 어느 서버도 복제하지 않는다.

## 2. v1과 v2의 경계

### v1에서 보존

- 동결 회귀가 보호하는 로직·RNG·반올림·로그와 익숙한 예약턴 문법. PHP/CHE는 신규 제품 설계의
  grand truth가 아니다.
...
| 주기 | 행동 | 화면 결과 |
|---|---|---|
| 귀환 2~5분 | 지난 접속 이후 인과 요약, 최대 3개 판단, 예턴 일부 수정 | 사건→제안/명령→이동→전투→변화 causal thread |
| 즉시 | 명령/제안 승인·거부 | command result와 대상 query만 갱신 |
| 장수 턴 | 이동·내정·전투 명령 실행 | 예약표, 장수 상태, 로그 diff |
| 집중 10~15분 | 작전 경로·보급·외교·위임 설계 | 지도 preview, 담당자, 위험·도착 window |
| 하루 | 전선·보급·가신·봉신 점검 | 작전 dashboard, 경고, 회의 |
| 시즌 | 봉토·외교·정통성·통일 목표 | 정세 timeline, 랭킹, replay archive |

1,180개 현급 행정 카탈로그와 780개 플레이 node는 직접 클릭 수를 늘리기 위한 장치가 아니다.
태수·도독·가신 위임, 정책 template, 대량 명령 preview, 예외 알림으로 관리량을 압축하고 플레이어는
어디에 관심과 권한을 배분할지 결정한다. 실시간 전술은 선택 참여형이며 부재 시 doctrine·위임 AI가
이어받는다. 접속 여부가 아니라 사전 정찰·보급·교리·권한 배분의 질이 경쟁력이 되어야 한다.
