...
> 상태: **DRAFT — approval-blocked.** §14의 W3/PHP blocker가 해결되고 독립 reviewer gate가 종료되기 전에는 승인하지 않는다. 이 문서는 이후 wave의 구현 계약이며, read replica 생성·schema cutover·운영 배포 승인 문서가 아니다.
...
GA-079 has a nation-only parity blocker outside that ring boundary. After each successful PHP `_setNationCommand`, the actor general when `npc < 2` receives and persists `killturn=max(env.killturn, current)` (`legacy/devsam-core/hwe/func_command.php:483-488`; `legacy/devsam-core/hwe/sammo/LazyVarUpdater.php:55-65`). This is a live general-meta mutation, and current Kotlin `CommandQueueService.setNationCommand` (`.../reserve/CommandQueueService.kt:236-263`) omits it. W0 has not selected a daemon-owned lifecycle that preserves the PHP order/atomicity while maintaining the one-daemon-write rule. Therefore GA-079 is **BLOCKED/W3-PHP-UNKNOWN**: no API general-row write, ring-only implementation, or parity-complete claim is allowed until a PHP capture and lifecycle design close this gap. The GA-076 rule is not permission to silently omit the nation effect.
...
- [ ] GA-079 remains implementation-blocked until a PHP capture and a one-daemon-write lifecycle test prove the successful user-general `killturn=max(env.killturn, current)` refresh has the PHP-required order and atomicity; an API general-row write is forbidden.
...
| GA-079 nation reservation actor-general `killturn=max(env.killturn, current)` capture plus one-daemon-write lifecycle/atomicity decision | `ARCH-S4`, W3 | Obtain PHP capture, select and approve a lifecycle that preserves the same operation's order/atomicity, then add focused tests. **Blocks GA-079 implementation and contract approval:** no API general-row write and no ring-only activation. |
...
- Approved sequence and ticket predicates: `docs/superpowers/plans/2026-07-18-cqrs-memory-consistency-hardening-plan.md`.
