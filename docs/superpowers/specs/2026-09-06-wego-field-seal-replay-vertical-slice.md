# 야전 출병 계획 봉인 · 결정론 해결 · 리플레이 수직 절편 (Phase 4X-C) 설계 — v2

- Date: 2026-09-06
- Status: REVISED v2 — 교차 비평 1차(`docs/superpowers/reviews/2026-09-06-wego-seal-replay-spec-critique.md`, fix-required F1~F7 · should-fix S1~S16)를 전부 반영. 통합 재판정 대기(ADR-LITE-049 Phase 4X 규칙: spec → 비평 → 구현 → 게이트). 이름에서 「WEGO」 를 뺀다 — 이 절편은 **공격자 쪽 계획 봉인**뿐이고 양측 동시 해결(로드맵)은 아니다(S12). 4X-A(구현 완료)·4X-B(spec v3)의 엔진 규약(메모리 세계 상태 + 행 단위 채널 + `removeGeneral` 즉시 가지치기 + 표마다 DELETE→CREATE→UPDATE + 값 > 0 일 때만 meta 고수위)을 상속한다.
...
| 항목 | 결정 |
|---|---|
| **F1 plannedStop 자리** | 루프 맨 위가 아니라 **`addPhase()` 뒤 · `continueWar()` 앞**(접촉한 `def` 가 바인딩된 자리, draw 없음). 계획 정지가 참이면 **기존 `attackerCont.canContinue == false` 분기와 같은 함수**를 탄다(공용 함수로 추출; 자연 퇴각이 우선 — `continueWar()` 를 먼저 보고 false 면 자연 퇴각, true 이고 계획 정지면 계획 퇴각). 미접촉 수비자에게 `addWin`/`tryWound` 를 주는 경로는 없다. |
...
| S8 단계 라벨 | 4X-B 8h 가 먼저 머지되면 **8i**, 아니면 8h — 라벨은 구현 시 executor 주석 순번. |
| S9 sealed_at | 핸들러 `nowProvider`(4X-A 선례). |
| S10 simulate-battle | 예상 = `POST /api/simulate-battle` 에 `defenderGeneralId` 가 필요하다 — 09 화면은 목표 도시의 수비 장수 목록에서 **첫 수비자**(`extractBattleOrder` 순)를 골라 호출하고 「수비 1인 기준」 이라고 적는다. |
| S11 슬롯 링크 | `slot.action == "che_출병"` 이고 `slot.arg.destCityID` 가 있을 때만 「봉인」 링크. |
...
| S15 이름·사망자 | `getName()` 은 토큰이라 리플레이는 **`WarUnitGeneral.getGeneral().name`·`WarUnitCity.state.city.name`** 을 쓰고, 사망자는 `onPhaseLog` 의 `deadAttacker/deadDefender` 누적으로 센다(`deadPerson` private). |
...
| `UNIQUE (world_id, general_id, target_city_id) WHERE resolved_year IS NULL`(부분 인덱스) · `INDEX (world_id, general_id)` | 미소비 계획은 장수 × 목표 도시 하나(F7) |
...
| `attacker_general_id INTEGER NULL` — `ON DELETE SET NULL (attacker_general_id)`, `attacker_name VARCHAR(50) NOT NULL`, `attacker_nation_id INTEGER NOT NULL` | 스냅샷(장수가 사라져도 기록 유지). 같은 턴 사망은 `markGeneralDeleted` 가 pending INSERT 의 id 를 NULL 로(F3) |
...
- `WorldSnapshot.battlePlans` + `WorldSnapshotLoader.loadBattlePlans`(HotColdCatalog 등록, S2). `InMemoryTurnWorld` map + dirty/created/deleted + `allocateBattlePlanId`(값 > 0 일 때만 meta; 4X-A 5곳 미러). `removeGeneral` 이 그 장수의 계획을 즉시 가지치기(4X-A N2). `ChangeRecorder.markGeneralDeleted` 는 pending `battleReplayInserts` 의 `attacker_general_id`·`battle_plan_id` 를 NULL 로(F3).
...
- 리플레이 id 는 recorder 선할당 `battleReplayIdAllocator` — **`messageIdAllocator`/`auctionIdAllocator` 방식**으로 `DaemonLoopConfig` 가 `findMaxId()` DB-seed 를 주입한다(F4; `diplomacyLetterIdAllocator` 는 시드 없는 카운터라 선례가 아니다). 결과 로그에 「리플레이 #id」 를 박기 위해.
...
- **주입 경로(F2·F5·F6)**: 엔진 `BattleCommandContextBuilder` 가 **`BattleCommandContext`** 에 `sealedPlans: Map<Int /*finalTargetCityId*/, SealedBattlePlan>`(그 장수의 봉인·미소비 계획 중 `sealedDate <= executingDate` 인 것)과 `executingDate` 를 싣는다. `CheChulbyeong.resolve` 는 예약의 `finalTargetCityId` 로 계획을 찾아(경유 전투에도 같은 계획, F6) 훅을 감싼다: `hooks = ReplayRecordingHooks(PlannedWarBattleHooks(ProductionWarBattleHooks(...), plan), draft)`. 계획이 없으면 **감싸지 않는다**(기존 객체 그대로 — 바이트 동일의 구조적 증거). 해결 뒤 `lastReplayDraft` 를 노출하고 엔진 `ReservedTurnHandler` 출병 후처리가 `recorder.recordBattleReplayInsert(columns)` + 계획 `resolvedDate` UPDATE 를 기록한다.
- **`WarBattleHooks` 확장(기본값 = 오늘 동작)**: `fun plannedStop(attacker: WarUnitGeneral, def: WarUnit, phaseIndex: Int): PlanStop? = null`. `processWarNG` 는 **`addPhase()` 뒤 · `continueWar()` 앞**(F1)에서 `val natural = attacker.continueWar()` 를 먼저 본다 — `!natural.canContinue` 면 오늘의 자연 퇴각 분기 그대로; 아니면 `hooks.plannedStop(attacker, def, phase)` 가 non-null 일 때만 **같은 퇴각 함수**(`retreatAttacker(attacker, def, noRice=false)` 로 추출: `onBattleResultLog` 2회·`addLose/addWin`·`tryWound` 2회·`onRetreatLog`)를 탄다. 계획 없는 경로는 호출 순서·draw 순서가 바뀌지 않는다(`ProcessWarNGOrderTest` 로 핀, S14). 계획 퇴각도 `deathnum+1`·`tryWound` draw 가 든다(S4).
...
- **`ReplayRecordingHooks`**: 위임 + 기록(draw 0). `onAdvanceLog` → 시작, `onContactLog` → 접촉 페이즈 표시, `onPhaseLog(attacker, def, deadA, deadD)` → `battlePhases += {i, def: 이름(`WarUnitGeneral.getGeneral().name` / `WarUnitCity.state.city.name` — `getName()` 은 토큰이라 안 쓴다, S15), defKind, contact, deadA, deadD, crewA: attacker.getCrew(), crewD: def.getCrew()}`, `onRetreatLog`/`onDefenderDownLog`/`onSupplyRout`/점령(`ProcessWarResult.conquerCity`) → `result`·`planStop`. 사망자 합은 `onPhaseLog` 의 `deadA/deadD` 누적(`deadPerson` 은 private). `BattleReplayDraft` 는 `CheChulbyeong` 이 `processWar` 반환 뒤 정산(`attacker.getCrew()` 전후, 누적 사망, `rice` 차)을 채워 `lastReplayDraft` 로 노출한다. `input_hash`·`replay_hash` 는 logic 순수 함수 `BattleReplayCodec.encode/hash`.
...
- **`input_hash` 내용(S5)**: `{schema:1, warSeed, attacker:{id, crew, crewTypeId, tech, train, atmos, rice, stats(leadership/strength/intel/politics/charm), skills 정렬}, defenders:[{id, crew, crewTypeId, tech, train, atmos, rice, stats}](출병 순), city:{id, level, def, wall, nationId, supply}, plan:{stance, pct, morale}, year, month, startYear}` 를 키 정렬·리스트 순서 유지·Double 은 `toBits()` 로 정규화한 뒤 SHA-256.
...
`battle_phases_json` 스키마 v1: `{"v":1,"phases":[{"i":1,"def":"화웅","defKind":"general|city","contact":true,"deadA":1900,"deadD":600,"crewA":10100,"crewD":8400}],"stop":{"kind":"loss_pct"|"morale"|"probe"|null,"atPhase":3}}` — 키 정렬 고정, 저장 바이트가 곧 해시 입력(S7).
...
- 예상 범위(09 「예상 (결정론 시뮬)」): 기존 `POST /api/simulate-battle` 을 **그대로** 쓴다 — `defenderGeneralId` 가 필요하므로 09 화면은 목표 도시 수비 장수 목록의 **첫 수비자**(`extractBattleOrder` 순, 없으면 「수비 장수 없음 — 성 방어만」 으로 disabled)를 넣고 「수비 1인 기준 예상」 이라고 적는다(S10). 결과는 killed/dead 의 min/avg/max. 아트보드의 「우세 41% · 대등 37% · 열세 22%」 는 그 API 가 per-repeat 결과를 주지 않으므로 **그리지 않는다**.
...
- 작전실 명령 목록: `slot.action == "che_출병"` 이고 `slot.arg.destCityID` 가 있는 예약 슬롯에만 「봉인」 링크(→ 위 화면 `?city={destCityID}`, S11). 봉인된 미소비 계획이 있으면 `Slot` 상태 칩 「봉인됨」. 「봉인까지」 카운트다운 = 다음 내 턴 시각(같은 순 봉인도 적용된다, F5).
...
- engine `BattlePlanIntakeTest`: 3 명령 채널·봉인 뒤 거부·소비 뒤 재저장 허용(F7)·툼스톤(장수 삭제 → 계획 프룬, pending 리플레이 id NULL — F3)·같은 틱 저장→봉인. `ReservedTurnHandler` 출병 후처리: `lastReplayDraft` → `recordBattleReplayInsert` 열 + `resolvedDate` UPDATE + 개인 기록 1줄, 계획 없으면 채널 0. 같은 순 봉인이 그 턴에 적용됨(F5).
- infra `BattlePlanReplayFlushIT`(PG16): V57 DDL(부분 UNIQUE 인덱스 포함) · 채널 순서 · `SET NULL (battle_plan_id)`/`(attacker_general_id)` · 리플레이 INSERT · 「봉인 출병 → 같은 틱 사망」 payload 가 NULL 로 COMMIT 성공(F3 적색면).
...
V57 = 이 절편(4X-B V56 뒤; `battle_replay.operation_id` FK `ON DELETE SET NULL (operation_id)`). 계획 문서 4X-C 블록은 이 스펙으로 고쳤다(S3): 장수×도시 키·stance 2종·조건 2종·소비된 계획·409 아님·`battle_plan.operation_id` 없음(리플레이 쪽에만).
