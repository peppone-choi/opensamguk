# 야전 출병 계획 봉인 · 결정론 해결 · 리플레이 수직 절편 (Phase 4X-C) 설계 — v3

- Date: 2026-09-06
- Status: REVISED v3 — 1차(F1~F7·S1~S16)와 2차(v2 재판정: fix-required N1~N4 · should-fix R1~R14, 같은 파일 `docs/superpowers/reviews/2026-09-06-wego-seal-replay-spec-critique.md`)를 전부 반영. 통합 재판정 대기
...
| **N1 정지 순서** | 계획 정지는 **수비자가 서 있을 때만**: `addPhase()` 뒤 `val natural = attacker.continueWar()`; `!natural.canContinue` → 자연 퇴각(오늘 그대로). 아니면 `val defCont = def.continueWar()`(draw 없음, 한 번만 부르고 재사용); **`defCont.canContinue && plannedStop != null`** 일 때만 공용 퇴각 함수. 수비자가 그 페이즈에 무너졌으면 오늘의 수비자 전멸/점령 분기를 그대로 타고(`conquerCity` 유지), 그 분기 끝의 `phase >= maxPhase` break 를 `|| plannedStop != null` 로 넓혀 다음 수비자로 넘어가지 않고 멈춘다(추가 draw 없음). 결과 라벨 우선순위: `conquered` > `defenders_down` > `retreat` > `repelled`. |
...
- **주입 경로(F2·F5·F6)**: 엔진 `BattleCommandContextBuilder` 가 **`BattleCommandContext`** 에 `sealedPlans: Map<Int /*finalTargetCityId*/, SealedBattlePlan>`(그 장수의 봉인·미소비 계획 중 `sealedDate <= executingDate` 인 것)과 `executingDate` 를 싣는다. `CheChulbyeong.defaultProcessWar`(훅을 만드는 자리)가 예약의 `finalTargetCityId` 로 계획을 찾아(경유 전투에도 같은 계획, F6) 훅을 감싼다 — AI 요격(interpose)으로 전투 상대가 예약 목표와 다른 부대로 바뀐 경우는 계획을 적용하지 않고 리플레이도 남기지 않는다(문서화): `hooks = ReplayRecordingHooks(PlannedWarBattleHooks(ProductionWarBattleHooks(...), plan), draft)`. 계획이 없으면 **감싸지 않는다**(기존 객체 그대로 — 바이트 동일의 구조적 증거). `processWarFn` 을 주입하는 테스트는 이 조립을 우회한다(R12). `lastReplayDraft` 는 `resolve` 진입 시 초기화 목록(`lastBattleResult`·`lastWarSeed` 옆)에 넣는다. 해결 뒤 `lastReplayDraft` 를 노출하고 엔진 `ReservedTurnHandler` 출병 후처리가 `recorder.recordBattleReplayInsert(columns)` + 계획 `resolvedDate` UPDATE 를 기록한다.
- **`WarBattleHooks` 확장(기본값 = 오늘 동작)**: `fun plannedStop(attacker: WarUnitGeneral, def: WarUnit, phaseIndex: Int): PlanStop? = null`. `processWarNG` 는 `addPhase()` 뒤에서 (1) `val natural = attacker.continueWar()` — `!natural.canContinue` 면 오늘의 자연 퇴각 분기 그대로; (2) `val defCont = def.continueWar()`(draw 없음, 오늘 `:168` 의 호출을 앞으로 옮겨 한 번만 부르고 재사용)를 본 뒤 **`defCont.canContinue` 이고** `hooks.plannedStop(attacker, def, phase)` 가 non-null 일 때만 **같은 퇴각 함수**(`retreatAttacker(attacker, def, noRice=false)` 로 추출: `onBattleResultLog` 2회·`addLose/addWin`·`tryWound` 2회·`onRetreatLog`)를 탄다(N1); (3) 수비자가 무너졌으면(`!defCont.canContinue`) 오늘의 수비자 전멸/점령 분기를 그대로 타고(`conquerCity` 유지), 그 분기 끝의 `phase >= maxPhase` break 를 `|| plannedStop != null` 로 넓혀 다음 수비자로 넘어가지 않는다(추가 draw 없음). 계획 없는 경로는 호출 순서·draw 순서가 바뀌지 않는다(`ProcessWarNGOrderTest` 로 핀, S14). 계획 퇴각도 `deathnum+1`·`tryWound` draw 가 든다(S4).
...
- **`result` 우선순위(R5, 처리 순서로 정의)**: `conquerCity`(`ProcessWarResult`) → `conquered`; `onRetreatLog` → `retreat`; 둘 다 없으면 마지막 상대가 성(`WarUnitCity`)이면 `repelled`, 아니면 `defenders_down`. `BattlePlanRulesTest` 표에 4행.
...
- 작전실 명령 목록: `slot.action == "che_출병"` 이고 `typeof slot.arg.destCityID === 'number'`(`arg: Record<string, unknown>`, R14)인 예약 슬롯에만 「봉인」 링크(→ 위 화면 `?city={destCityID}`, S11). 계획은 **예약한 목표 도시로의 출병**에만 걸린다 — AI 대행(autorun) 턴은 미적용이므로 autorun 상태에서는 「봉인됨」 칩을 점선 + 「위임 턴에는 적용되지 않습니다」 로(R9). 봉인된 미소비 계획이 있으면 `Slot` 상태 칩 「봉인됨」. 「봉인까지」 카운트다운 = 다음 내 턴 시각(같은 순 봉인도 적용된다, F5).
