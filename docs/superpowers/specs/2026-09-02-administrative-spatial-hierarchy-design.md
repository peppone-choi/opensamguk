...
> 2026-09-06 후속 결정: [프로빈스 전선과 현치 함락 설계](2026-09-06-province-front-and-county-capture-design.md)가
> 소유권·명령 단위의 최신 기준이다. 아래의 ‘현치 하나만 변경’과 ‘군국 기본 명령 단위’는 당시 전환 설계 기록이며
> 후속 기준으로 대체한다. 프로빈스 군사 점유와 현 행정권을 분리하고, 현치 함락 시 패배 세력의 무저항 영역만 함께 인계한다.
> 후속 규칙의 런타임 전환은 아직 구현되지 않았다.

## 결정

OpenSamguk의 지도·행정·게임플레이 계층을 다음 세 종류로 분리한다.

1. `SpatialProvince`: 육상 이동, 인접성, 점령, 보급 판정의 최소 행정 공간 단위다.
...
강·호수·연안은 현·군국 행정계층에 넣지 않고 별도 `WaterZone`과 타입 간선으로 모델링한다. 수역의
안정 ID, 이동·보급·수전·상륙 계약은
`2026-09-05-water-transport-and-naval-topology-design.md`를 따른다. 수역은 1,524개 육상
`SpatialProvince`의 개수·ID·행정 소유권을 바꾸지 않는다.
...
- 부대 위치와 이동 경로는 육상 `SpatialProvince` 또는 전략 `WaterZone`의 안정 ID와
  타입 간선으로 기록한다.
- 육상 이동은 공간 프로빈스 인접 그래프를 사용하고 ROAD 그래프를 만들지 않는다. 강·호수·해안
  이동은 별도 `WaterZone` + 검토된 `FORD|BRIDGE|FERRY|EMBARK|DISEMBARK|RIVER_*|LAKE|COASTAL`
  간선을 사용하며 원시 물 타일 인접을 자동 경로로 해석하지 않는다.
