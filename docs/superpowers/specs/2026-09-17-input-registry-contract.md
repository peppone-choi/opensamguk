...
|---|---|---|
| 제품 기본 프로필이 SAMMO였음 — B1 해소 | B1에서 `ScenarioImporter`, `TurnWorldState`, game-api 공통 해석기의 누락 기본값을 HWIHA로 변경 | 기본값 테스트와 적색 프로브로 고정. [#249](https://github.com/peppone-choi/opensamguk/issues/249) |
...
사용자의 게임 기획 위임에 따라 `ruleProfile` 저장 자리는 현행 구현대로 **`world_state.config["ruleProfile"]`** 하나로 확정한다. `ScenarioImporter`가 시나리오 선언을 적고 엔진은 `TurnWorldState.ruleProfile`로 읽는다. B1 컷오버 뒤 누락 기본값은 HWIHA다. 복원한 SAMMO 백업의 누락 키에 한해 `SAMMO_ROLLBACK_ENABLED=true`가 SAMMO 기본값을 선택한다. 명시된 값은 스위치로 덮어쓰지 않으며 알 수 없는 값과 문자열 아닌 값은 거절한다. meta에 복제하거나 런타임 프로필 전환 경로를 추가하지 않는다.
...
사용자 기획 위임에 따라 첫 출사 소비자의 순수 규칙을 다음처럼 정한다. 상위 §12.1의 임관·랜덤임관·장수대상임관 세 직접 행동은 유지하며, 공통 입력 `action.enlist`의 `NATION`·`RANDOM`·`GENERAL` 선택으로 구분한다. 세 방식은 화면에서 출사의 무작위·세력 지정·장수 지정 선택으로 제공한다(2026-09-21 위임 결정). 기존 행동의 의미와 역참조는 유지한다. 원장 역참조3개가 행동3개의 폐지를 뜻하지 않는다.
...
- 주공은 사건으로 얻고 잃는 지위다. `officerLevel == 12`, 보유 縣 수, 휘하 크기를 새 정본 판정으로 쓰지 않는다. 도메인은 명시적 지위·군주 대응·출사 허용·남은 명망을 입력받는다. 기존 meta 저장과 시작 주공의 명시 시드는 아래 저장 연결 절을 따른다. 출사 실행기는 지위 상실을 저장한다. 지연 등장 지위 전달은 남아 있다.
...
2026-09-20 위임 결정: 장수 `meta.hwihaLord`의 boolean을 명시적 지위로 사용한다. 시나리오의 `hwihaLords` 이름 목록은 세 roster 전체의 유일성을 확인하고 실제 선택·활성 장수에만 시드한다. 시드 선언이 없으면HWIHA는false, 기존SAMMO는키를추가하지않는다. 지연등장/제외된선언은유실시키지않고시드전거절한다. 지연이벤트지위전달은후속이다.

기존 general.meta의 snapshot/저장 경로를 재사용하므로 새 테이블이나HotCold등록을 추가하지 않는다. `HwihaLordStatus`는 타입을 엄격히 검사하고 출사 뒤false가 된 사본을 반환한다. 출사 실행기는 이 사본의 차이를 레코더에 기록하며 아래 저장 검증을 거친다. 실행기의 반환 자체는 DB 커밋 완료를 뜻하지 않는다. 실제 API·개인 턴·결과 저장 연결은 아래 접수·결과 계약을 따른다.

### 출사 판정의 엔진 상태 적용 (2026-09-21)

`HwihaEnlistmentExecutor`는 실행 순간의 장수·명시적 주공 상태·휘하·군주 대응을 다시 읽고 위 순수 판정을 적용한다. 명망·수용 정책은 서버가 현재 카드 집합으로 재계산해 공급하며 클라이언트가 비용이나 잔량을 지정하지 않는다. 명망은 보유 상한이므로 출사로 명망 점수를 소비하지 않고 새 직접 카드가 한도를 차지한다. 하위 장수의 개인 카드 비용을 주공에게 중복 청구하지 않는다.

사용자 기획 위임에 따라 첫 출사의 기존 저장 관계는 문객(`guest`), 미발령 역할은 `NONE`, 충성은 50으로 정한다. `EXISTING` 참조 카드를 만들며 `hasOwnBugok=true`는 부곡 보유 자격이고 병력 생성이 아니다. `MUTUAL`·임무 없음은 기존 참조 카드 저장 계약을 따른다. 결속 태그·발령·월단평 전체를 이 필드들로 구현했다고 간주하지 않는다.

V55의 주공별 이름 유일성은 실제 인물과 생성 카드 모두에 적용되므로 동일 이름 카드가 있는 주공은 `DUPLICATE_RETAINER_NAME`으로 거절한다. 랜덤 출사에서도 같은 검사로 후보를 제외한다. 국가 행 존재를 확인한 다음 출사자와 개인 휘하의 실제 장수 소속 및 출사자의 주공 지위를 레코더로 갱신한다. 목적 국가 인원은 `npcState=5`를 제외한 실제 장수 수로 즉시 집계한다. 개인 카드·부곡·자산·위치·기준 城은 보존한다.

이 실행기는 단일 예약 경로와 현재 명망 정책을 통해 API/개인 턴에 연결한다. 출사의 HANDLER_READY는 HWIHA 이탈 정산 전체의 완료를 뜻하지 않는다. 기존 `RetainerMonthlyService`의 충성 0 카드 삭제만 적용하면 국가 소속이 남으므로 HWIHA 이탈로 재사용할 수 없다. 중복 요청·재시작 안전성은 기존 슬롯 소비·세계 상태·결과의 공동 트랜잭션을 연결해 별도 검증해야 한다.


콜드 재로드 연결에서 군주 대응은 `Nation.chiefGeneralId`를 쓰지 않는다. 이 필드는 현재 DB에서 복원되지 않는다. 기존 거병·선양·승계가 저장하는 `general.officer_level=12` 직위에서 국가별 유일한 군주를 매 실행 투영한다. 같은 국가에 군주가 없거나 둘 이상이면 국가 지정은 `TARGET_NOT_FOUND`로 거절하고 랜덤 후보에서 제외한다. 이 직위는 군주를 선택하는 근거이며 주공지위를 부여하는 근거가 아니다. 선택된 군주도 별도의 `hwihaLord=true`가 있어야 출사를 받을 수 있다. 군주 직위가 없는 명시적 봉신 주공은 GENERAL 대상으로 계속 출사를 받을 수 있다. 기존 승계 사건의 HWIHA 주공지위 획득·상실 연결은 활성화 전에 별도 구현해야 한다.


출사 상태 적용의 저장 검증은 실제1133지도 아카이브와 HWIHA 프로필을 읽는 `WorldSnapshotLoader` → 실행기 → `DatabaseHooks` → `JdbcFlushExecutor` → 새 로더로 수행한다. 개인 휘하·부곡·위치와 다른 월드의 상태를 보존하고, 카드 INSERT 실패 시 앞선 국가·장수 변경도 롤백되며 같은 payload를 재시도할 수 있음을 검증한다. 이 테스트의 소규모 장수 fixture는 저장 경계 검증이며 한 州 플레이·API 예약·개인 턴 전체의 완료 증거는 아니다.

### 실행 거절과 기존 명령의 분리 (2026-09-21)
...
기존 HWIHA G2 검사에서 legacy 휴식을 실제 실행하던 의존은 제거했다. 세계/순 종자 문자열 단언은 그대로 유지하고, 미배달·프로필 불일치 입력의 RNG 미소비를 별도로 검증한다. 출사 handler는 복수 후보에서만 개인 턴 RNG를 소비하며 후보 정렬·상한·동일 seed 재현을 검증한다. 전체 전투 리플레이 완료를 뜻하지 않는다.

### 출사 접수 인자와 공통 상태 투영 (2026-09-21)

출사 인자는 `mode`와 필요한 경우 양의 정수 `targetId`만 받는다. NATION은 국가 ID, GENERAL은 장수 ID를 쓰며 RANDOM에는 targetId를 보내지 않는다. actor ID는 인증된 장수에서 공급하고 클라이언트 인자로 받지 않는다. 중복 키(이스케이프된 같은 키 포함), 알 수 없는 키, 문자열 숫자·소수·오버플로 및 JSON 뒤의 추가 내용은 INVALID_REQUEST로 거절한다.

`HwihaEnlistmentPrecheck`는 API와 실행기가 공통으로 쓰는 전체 상태 판정이다. 국가별 유일 군주, 사람 장수 여부, 휘하 관계, 전체 카드 이름 충돌, 다섯 능력치 비용을 같은 규칙으로 계산한다. API 읽기 어댑터는 REPEATABLE_READ에서 해당 월드의 DB 행을 읽고, 실행기는 현재 메모리 상태에서 다시 판정한다. API가 받은 결과는 접수 가능성일 뿐 실행 성공을 보장하지 않는다. 누락·오염된 대상 정책은 POLICY_UNAVAILABLE이며 실제 명망 부족과 구분한다.

`HwihaEnlistmentHandler`는 엄격한 인자 파서와 현재 상태 실행기를 연결하는 어댑터다. 랜덤 출사에 복수 후보가 있을 때만 기존 개인 턴 시드로 난수를 만든다. 출사 어댑터를 registry에 등록하고 개인 턴·flush·콜드 재시작 경로에서 성공과 거절 결과를 확인한다. 원장은 출사만 HANDLER_READY이며 UI·AI·도움말·튜토리얼·전체 리플레이 검증을 뜻하지 않는다.

한 순 제한은 개인 턴 결정론 계약 Q2의 2026-09-21 위임 결정을 따른다. 지연 예약은 현재 세계 순에 하나만 소비하고, 처리한 순을 장수 meta에 기록한다. 같은 순 재호출·콜드 재시작은 예약 조회·난수·소속 변경 전에 제외한다. 다음 순에는 다음 예약이 실행 시 재검사를 거친다. 미등록·미제공 예약을 거절한 경우도 처리한 순을 기록하며, 월드 설정의 tickSeconds로 개인 시각을 전진한다.

### 출사 접수·결과 계약

`POST /api/command/action.enlist?generalId=<소유 장수>&turnIdx=0..11`은 인증된 제출자와 해당 장수의 소유 관계를 확인한다. 원본 JSON의 중복 키를 제거하기 전에 검사하고, 공통 사전검사가 허용한 입력만 기존 예약 트랜잭션으로 저장한다. 모드·대상 외 actor·비용·명망·주공 지위는 클라이언트에서 받지 않는다. 저장한 canonical 인자·requestId·제출자·예약 접수 결과는 한 트랜잭션이며, 엔진 깨움은 커밋 후다.

응답202/AVAILABLE은 예약 접수다. 기존 결과 조회는 실행 전 PENDING이고, 실행 후에는 같은 requestId의 `executionApplied` 또는 `executionRejected`와 공통 code/reason을 읽는다. 명시적 제출자가 기록된 요청은 그 제출자만 결과를 읽는다. 이후 장수 소유권이 바뀌어도 새 소유자에게 이전 제출자의 결과를 공개하지 않으며, 제출자는 계속 자기 결과를 읽을 수 있다.

HWIHA의 기존 큐 일괄예약·밀기·복사는 제공하지 않는다. 개별 출사 예약은0..11슬롯만 받는다. 기존30칸 큐 조작으로12순 범위를 우회하거나 요청 식별자가 없는 복사본을 만드는 것을 막는다. SAMMO 큐의 기존 동작은 유지하며, 실제 복사할 source 스냅샷에도 새 형식 입력이 없는지 검사하여 접수 검사 뒤의 동시 변경을 방어한다.

검증 범위는 실제 JPA 읽기/예약 API → DB → 개인 턴 실행 → 콜드 재로드 → 제출자 결과 조회다. 인증 principal은 테스트에서 공급하며 JWT 필터·브라우저 화면·NPC AI·발령 이후 흐름을 이 검사로 검증했다고 보지 않는다.

HWIHA 세계의 기존 단건 legacy 예약도 서버 접수 경계에서 거절한다. 기존 30칸 예약을 통해 12순 범위를 우회할 수 없으며, 세계 규칙을 확인할 수 없을 때도 쓰기를 수행하지 않는다. SAMMO의 기존 30칸 예약은 유지한다.
...
### NPC의 첫 출사와 빈 예약 칸 (2026-09-21 위임 결정)

일반 NPC(NPC_LITE=2)가 재야이고, 계정 소유자·휘하 주인이 없으며 명시적 비주공일 때만 빈 개인 예약 칸에서 RANDOM 출사를 선택한다. 공통 사전검사에 유효 후보가 있어야 하며, 선택한 입력은 사람과 동일한 handler·결정론 난수·순당 한 번 제한·flush로 처리한다. 특수 NPC, 사람, 방랑 주공과 명시 예약을 자동으로 대체하지 않는다. 생성한 AI 입력에 사람의 requestId를 붙이지 않는다.

HWIHA 시드의 개인 예약은 빈 큐로 시작한다. 예약된 행의 실제 존재로 사용자 입력과 빈 칸을 구분하며 requestId의 부재로 추측하지 않는다. 개인 턴 flush는 소비한 HWIHA 예약 행을 삭제하고 뒤 예약을 당긴다. 소비한 입력을 30번째 칸의 가짜 휴식으로 되살리지 않는다. 명시 휴식 행도 해당 순의 NPC 선택을 막은 뒤 소비된다. 기존 SAMMO의 30칸 초기 휴식·순환 이동은 그대로다. 이미 저장된 명시 예약을 배포 시 일괄 지우거나 운영 세계를 재시드하지 않는다.

런타임 장수 생성도 같은 빈 큐 규칙을 따른다. 명시 초기 예약은 현재 지원하는 출사 입력만 엄격한 인자 검사와 12칸 한도로 받는다. 잘못된 초기 예약은 레코더의 장수·위치 변경 전에 거절하고, 직접 flush에 들어와도 전체 트랜잭션을 롤백한다. SAMMO 생성의 기본 휴식30칸은 유지한다.

### 2026-09-21 발령 도메인 구현 결정

사용자가 일반 게임 기획 결정을 메인 구현자에게 위임한 범위에서 다음 기본 규칙을 정한다. 사료상의 제도로 주장하지 않는다.

- 첫 발령 목적지는 같은 세력이 소유한 행정 縣의 빈 자리다. 지도 거점·외부 취락을 縣으로 간주하지 않으며, 해당 월드가 선택한 지도 아카이브의 관할 분류를 사용한다.
- 대상은 직접 거느리는 사람 장수다. 사람이 소유한 NPC 장수도 포함하며, 자동 NPC 배치는 별도 입력이다.
- 응답 기한은 발령 시점부터 기본 12순이다. 월·연 경계를 이어 계산하고, 기한에 도달한 응답은 수락으로 처리한다. 보류 발령은 새 발령으로 덮어쓰지 않는다.
- 기한 전 거절은 충성 5를 즉시 감소시키며 0을 하한으로 한다. 명망은 즉시 깎지 않고 월단평 사건 「발령 거절」(−4, 한 달에 한 번)을 집계에 넣는다(2026-09-23 사용자 결정 — 이전의 즉시 명망 −1 은 폐기). 재전달로 두 번 차감하지 않는다. 기존 부임 목표는 거절 때문에 삭제하지 않는다.
- 수락은 부임 목표만 저장한다. 위치·자산·개인 행동 예약·개인 턴 시각을 바꾸지 않는다. 실제 이동은 이후 개인 턴의 행군이 담당한다.
...
발령 접수·결정권자 턴 실행·응답 즉시 처리·만료 스케줄링을 연결하며 두 입력을 `HANDLER_READY`로 등록한다. 화면·행군은 후속이며 UI_READY나 S3 완료로 승격하지 않는다. NPC 주공의 첫 발령은 아래 정책으로 연결한다.
...
### 발령 화면용 선택지와 대기 조회 (2026-09-21)

`GET /api/commands/dispatch-options?generalId=...&targetGeneralId=...`는 본인 주공의 직속 사람 장수 목록을 반환한다. 대상 ID를 선택하면 같은 스냅샷에서 아군 행정 縣만 열거하고 공통 발령 판정으로 빈자리·보류 발령·대기 큐 상태를 검사한다. 다른 장수 ID를 탐색해 비공개 가용성을 얻을 수 없으며, 지도 분류·핀·저장 상태 오염은 확인 불가로 반환한다. 화면은 縣 여부나 점유를 추정하지 않는다.

기존 `dispatches` 읽기와 options의 `queued`는 접수 당시 제출자와 현재 소유자가 같을 때만 requestId·대상·목적지를 보여준다. 다른 계정이 장수를 넘겨받으면 이전 요청 식별자는 숨기고, 아직 대기 중이라는 공통 거절 사유만 제공한다. options의 가용성은 접수·실행 성공의 보장이 아니며 양 시점에 상태를 다시 검사한다.

### NPC 주공의 첫 발령 (2026-09-21 위임 기획)

일반 NPC(NPC_LITE=2) 중 계정 소유자가 없고 명시적 주공이며 세력이 있는 장수는 실제 자기 턴에 첫 발령을 최대 한 건 낸다. 기존 사람 제출 큐가 있었던 턴에는 그 큐만 처리한다. 현재 직접 거느린 같은 세력의 사람 장수 중 주공이 아니고 발령·부임 목표 이력이 전혀 없는 대상만 고른다. 후보 장수 ID, 아군 행정 縣 ID 순으로 공통 발령 판정을 적용하여 첫 가능한 쌍을 선택한다. 접수·실행 판정을 우회하거나 사람 제출 요청을 합성하지 않는다.

거절·취소를 포함한 발령 이력이 있으면 자동 첫 발령을 반복하지 않는다. 이후 수동 발령은 기존 규칙을 따른다. 성공 시 대상의 개인 행동 기록에 담당자가 없는 아군 현의 첫 부임이라는 사유를 남긴다. 위치·개인 행동·자원을 즉시 변경하지 않으며, 같은 순 중복 방지와 발령·로그 저장은 기존 개인 턴 stamp 및 recorder→flush 경로를 사용한다. 초기 지도 소유를 추정하지 않고 현재 월드 도시 소유권을 읽는다. 이 정책은 첫 부임 연결이며 포상 AI·시즌 완주 검증을 뜻하지 않는다.
