...
이 문서의 날짜가 붙은 「현행 실측」·「첫 구현 묶음」은 당시 기록이다. B1 컷오버 이후 제품 기본 프로필은 HWIHA이며 위 첫 번째 차이는 B1에서 해소했다. SAMMO 분기는 ADR-LITE-065의 한 시즌 롤백과 동결 회귀 기준선으로만 해석한다.

**2026-09-24 8차 갱신:** 현재 입력 원장은 74행이며 `PLANNED` 행이 없다(63행 `UI_READY`, 기존 11행 `HANDLER_READY`). 기존 명령 70칸은 고유 코드 69개로 정규화했고 활성 64개와 폐지·설정 5개를 원장 검사에 연결했다. 직접 행동과 조정·계책·공사 감축은 예약·실행·결과 조회 경로를 갖는다. 개인 예약 결과는 `InputResolved`를 포함하고 조정·지속 입력 결과도 같은 봉투를 포함한다. 알파 카탈로그 기반 SAMMO 명령 표시 API와 카드 손패 통합은 아직 별도 정리가 필요하므로 이 대조표의 해당 잔여 작업을 완료로 세지 않는다.
