...
- 주공은 사건으로 얻고 잃는 지위다. `officerLevel == 12`, 보유 縣 수, 휘하 크기를 새 정본 판정으로 쓰지 않는다. 도메인은 명시적 지위·군주 대응·출사 허용·남은 명망을 입력받는다. 기존 meta 저장과 시작 주공의 명시 시드는 아래 저장 연결 절을 따른다. 지연 등장과 출사 실행기의 상태 갱신은 남아 있다.
...
현재 구현은 순수 판정, 엔진 휘하 생성·전향 집계 및 flush/콜드 재로드 검증까지 포함한다. 원장7행은 PLANNED 그대로이며, 지연 등장 주공지위 전달·명망 정책 저장·실제 API/예약·개인 턴 실행·결과 조회·화면은 이후 연결한다. 기존 `JoinCommand`의 도시 즉시이동이나 `RetainerHandler`의 NPC 주공 금지를 새 입력에 그대로 적용하지 않는다. 새 범용 입력 프레임워크나 별도 저장 경로는 만들지 않는다.
...
기존 general.meta의 snapshot/저장 경로를 재사용하므로 새 테이블이나HotCold등록을 추가하지 않는다. `HwihaLordStatus`는 타입을 엄격히 검사하고 출사 뒤false가 된 사본을 반환한다. 출사 실행기는 이 사본의 차이를 레코더에 기록하며 아래 저장 검증을 거친다. 실행기의 반환 자체는 DB 커밋 완료를 뜻하지 않고 API·개인 턴 연결도 아직 남아 있다.
...
이 실행기는 아직 API/예약/개인 턴에 배선하지 않았다. 접수 응답과 실행 결과를 구분하는 단일 예약 경로, 현재 명망 정책, HWIHA 이탈 정산과 결합하기 전 원장을 활성화하지 않는다. 기존 `RetainerMonthlyService`의 충성 0 카드 삭제만 적용하면 국가 소속이 남으므로 HWIHA 이탈로 재사용할 수 없다. 중복 요청·재시작 안전성은 기존 슬롯 소비·세계 상태·결과의 공동 트랜잭션을 연결해 별도 검증해야 한다.
...
기존 HWIHA G2 검사에서 legacy 휴식을 실제 실행하던 의존은 제거했다. 세계/순 종자 문자열 단언은 그대로 유지하고, 미배달·프로필 불일치 입력의 RNG 미소비를 별도로 검증한다. 성공한 HWIHA handler→RNG 배선은 실제 입력 활성화 시 추가 검증해야 한다.
