...
현재 원장 7행은 여전히 PLANNED이며 실행 가능한 핸들러는 등록하지 않았다. 따라서 해당 경로는 registry의 NOT_DELIVERED/WRONG_RULE_PROFILE/UNKNOWN_INPUT/MALFORMED_INPUT_ID 거절만 수행한다. 거절 예약은 개인 슬롯 한 개를 소비하고 기존 개인 마감 시각에 월드 tickSeconds를 더한 시각으로 전진한다. 상태·슬롯 소비·거절 결과는 기존 단일 flush/재시도 경로를 공유한다. 이 경로에 기존 부상 회복·차단·국가 AI·killturn 사망/환생을 적용하지 않는다. 성공 정치 행동·HWIHA AI·월 경계 처리는 후속 연결이며, 현재 경로로 S3 실행 완료를 주장하지 않는다.
...
`HwihaEnlistmentHandler`는 엄격한 인자 파서와 현재 상태 실행기를 연결하는 어댑터다. 랜덤 출사에 복수 후보가 있을 때만 기존 개인 턴 시드로 난수를 만든다. 이 어댑터의 존재만으로 원장 상태를 올리지 않는다. 예약 접수·registry 등록·개인 턴 성공 및 결과 조회를 연결하고 검증하기 전까지 원장 7행의 PLANNED 상태를 유지한다.
