...
이 실행기는 아직 API/예약/개인 턴에 배선하지 않았다. 접수 응답과 실행 결과를 구분하는 단일 예약 경로, 현재 명망 정책, 공통 flush/콜드 재로드, HWIHA 이탈 정산과 결합하기 전 원장을 활성화하지 않는다. 기존 `RetainerMonthlyService`의 충성 0 카드 삭제만 적용하면 국가 소속이 남으므로 HWIHA 이탈로 재사용할 수 없다. 중복 요청·재시작 안전성은 기존 슬롯 소비·세계 상태·결과의 공동 트랜잭션을 연결해 별도 검증해야 한다.
