...
요청이 크면 먼저 "계획만" 시키고(4.1), 애매하면 인터뷰를 요구한다("하나씩 물어봐"). 참고: `docs/agent/prompt-pack.md`에 작업군별(역사 동결 회귀·opt-in PHP 비교·프론트 배선·인프라 배포·기획 티켓 분해) 프롬프트 템플릿이 있다.
...
### 4.0 0단계: 백로그에서 티켓 고르기 (Jira가 업무 정본)

"모든 작업은 Jira 티켓에서 시작된다"(§11)의 실행 짝이다. 하루(또는 스윕)는 백로그 확인으로 시작한다.
...
- **정렬 기준을 명시한다.** 심각도의 정본은 티켓 제목/본문의 태그다 — Jira priority 필드는 생성 기본값(Medium)으로 남아 있을 수 있어, "priority 필드 순"이라고만 하면 전부 동률이 된다.
- **선행 관계가 심각도보다 먼저다.** 예: 업로드 API(선행) 없이 업로드 UI 티켓부터 잡으면 死路다.
- Atlassian MCP가 이 세션에 없으면 Jira 상태를 추측하지 않는다 — 로컬 `.ai/task.md`와 `docs/superpowers/plans/*-ticket-backlog/`가 대체 정본이다.

**중요도순 전체 스윕**: 백로그 전체를 처리할 때는 위 추천을 반복 실행하며 티켓 단위로 4.1→4.6을 돈다. 스윕 1단위 = 티켓 1개 = PR 1개가 기본이다.
...
티켓이 이미 있으면 **키를 지정해** 시작한다(0단계에서 고른 티켓).

```
/os-start-task OPENSAM-90을 시작해.
Jira 설명과 GitHub 미러(#232)를 확인하고, 목표·범위·비범위·완료 기준·검증 명령·승인 지점을
...
| **atlassian (Jira)** | OPENSAM 프로젝트 티켓 | cloudId 고정, 이슈타입: 에픽/작업/스토리/버그. §11 계층·미러 규칙, 선택·착수는 §4.0 |
...
### 티켓 완료 전환과 GitHub 미러 동기화 (상행)

하행(기획→티켓, 위 3층 규칙)만큼 상행(실행→완료)도 규칙이 있다. 티켓 선택·착수는 §4.0~4.1.

- **GitHub 미러 규약**: Jira 티켓은 GitHub Issue로 1:1 미러한다 — 제목 `[OPENSAM-NN] 요약`, 라벨 `jira-mirror`(에픽은 `epic` 추가), 에픽 본문에 `### 하위 작업` 체크리스트(`- [ ] #NNN`), 각 작업 푸터에 Jira 링크와 `- 에픽: #N`. 매핑 판별의 정본은 제목의 `[OPENSAM-NN]` 프리픽스다.
- **완료 전환은 증거가 조건이다**: 검증되지 않은 구현 보고만으로 티켓을 완료로 바꾸지 않는다. 완료 기준 충족 증거(테스트 XML·게이트·PR 링크)를 Jira 코멘트로 남기고, 같은 근거로 GitHub 미러를 닫고 에픽 체크리스트를 갱신한다.
- **동기화 점검**: 스윕 중간중간 Jira 상태와 GitHub 미러(열림/닫힘, 체크박스)가 어긋난 티켓을 찾아 보고시키고, 정정은 승인 후 수행한다.

```
OPENSAM-90의 완료 기준을 PR의 증거와 대조해. 전부 충족된 경우에만 Jira를 완료로 전환하고
...
4. [ ] `/os-start-task`로 계획만 받기 (티켓 기반이면 §4.0에서 고르고 `OPENSAM-키` 지정) → 승인
