...
diff, `.ai/task.md`(요구사항), PHP 근거(패러티 변경 시), 테스트 결과.
...
리뷰 계약(정본: `docs/superpowers/WORKING_SYSTEM.md` §Cross-agent critique): 구현자 claim → critic이 PHP 증거·테스트·문서·하드코딩·운영 불변식을 **독립적으로** 공격 → `cleared` / `fix-required` / `quarantined-with-proof`. `fix-required`가 남으면 merge/ship 금지. 아티팩트는 `docs/superpowers/reviews/<date>-<scope>.md`.

## AI 책임 / 사람 책임

- AI: 잠재 버그, 누락 테스트, 패러티 차원(RNG/round/log/write-order), 일관성, 일반 보안, 성능 위험, 변경 요약.
