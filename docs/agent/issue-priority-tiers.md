...
### 2. 세계 단위는 175郡治 / 780城으로 확정됐다 — 161郡은 stale-premise다

| 출처 | 단위 |
| --- | --- |
| `#464` (OPENSAM-204) 본문 | 후한 **161郡** — stale |
| ADR-LITE-041 제품 세계 | **175郡治 / 780城** — approved |
| `data/map/han-tiles.json` 실물 | `cities` **1,144**(렌더·소유격자 입력) · `juns` **175** |
| `infra/src/main/resources/map/han.json` 실물 | 플레이 가능한 도시 **780** |

제품 노드 수는 이미 확정됐다. `#464`의 161郡과 이를 미확정 선행조건으로 취급하는
서술만 `stale-premise`다. `#473`은 780城 그래프를 기준으로 설계한다.
...
`#464`의 161郡 택일 요구는 `stale-premise`다. `#474`가 요구한 han 상수 등재는
ADR-LITE-043으로 해결됐다. `#473`은 세계 단위 미확정 블로커를 잃었고 ADR-LITE-041의
780城 그래프를 기준으로 설계한다.
...
**지금 착수하면 안 된다.** 세계 단위는 확정됐지만 각 대공사의 별도 착수 조건은 아직 미충족이다.
`BATTLE-F2`(#335)에서 `BattleTicketV1` 과 레지스트리 이름이 **동결**되는데,
그 위에 설 세계는 ADR-LITE-041의 175郡治 / 780城으로 고정한다.
