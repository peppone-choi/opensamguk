...
문제 정의(사용자 보고, prod 관측, LEDGER 백로그), PHP 오라클 근거(패러티 작업이면 필수).

## Procedure

1. **문제 정의** → 사용자 가치 한 줄.
2. **범위/비범위 명시** — "만들지 않을 것"과 의도적 제외 사유를 반드시 적는다(정렬 앵커).
3. **수용 기준** — 테스트 가능하게(Given-When-Then 권장; 패러티 작업은 "골든 draw-for-draw green"이 AC).
4. **위험/제약** — 패러티 규율·one-daemon-write·배포 영향 검토.
...
2. **분해 규칙** —
   ...
   - **Story** = 사용자 가치 단위, **Given-When-Then AC 필수** (패러티 작업은 "골든 draw-for-draw green"이 AC).
...
계획 문서에 AC·검증 명령(`verification.md` 행렬 참조)이 명시됐는지, PHP 근거 path+line이 있는지.
