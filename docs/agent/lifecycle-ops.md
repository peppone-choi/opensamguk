...
**main push = shared-stack 자동 갱신(라이브 제어면).** 따라서 main push 자체가 운영 행위이며 사람 승인 게이트 뒤에 있다. 이는 실행 중 게임 서버의 image pin 승격을 뜻하지 않는다.

## 자동 실행 금지 (사람 승인 필수)

- main push/merge(=shared-stack 프로덕션 갱신), 서버별 승격·재시드·새 기수
- 운영 DB 변경(재시드, 파괴적 마이그레이션, 데이터 삭제 — 백업 선행)
- 운영 서비스 재시작(엔진 bounce는 메모리 상태 소유 — HARNESS §5)
- 비밀값 조회/출력, 리소스 생성·삭제
...
2. **근본 원인**: `failure-cases.md`·HARNESS §5 두 ops invariant 먼저 대조(stale-DNS 502 → nginx 순서, health와 별개인 turn-advance 증거). 패턴 매칭이 안 맞으면 `systematic-debugging`으로 수렴 — **상태 변경 명령 전에 증거가 그 조치를 지지하는지 확인**.
3. **안전한 수정 제안** → 사람 검토.
...
전용 스크립트 없음(NOT_CONFIGURED). 승인 후 이전 커밋의 shared-stack 갱신 또는 `opensamguk-docker` runbook의 서버별 pin rollback을 선택한다. nginx-upstream 순서는 롤백에도 동일 적용.

## Tools / Commands

`gh run list/view`(Actions 상태), approved `opensamguk-docker` shared/server runbook, prod DB 쿼리(승인 하 read). 이 저장소의 compose/script는 호환 검증 외의 운영 제어면으로 사용하지 않는다.
