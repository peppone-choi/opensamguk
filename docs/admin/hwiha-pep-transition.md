...
이 문서는 豫州 한 州 실스택 완주 뒤 사용할 전환 런북이다. 운영 배포, 월드 초기화, 워크플로 실행은 이 문서의 작성으로 수행되지 않는다. 지도 통일과 B단계 구현 PR이 각각 승인·병합되고 최신 main CI가 모두 초록이면 ADR-LITE-065 결정에 따라 추가 승인 없이 pep 전환을 실행한다. 아래 중단 조건은 그대로 적용한다.
