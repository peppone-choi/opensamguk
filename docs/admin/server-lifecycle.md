...
`ruleProfile: "HWIHA"` 시나리오는 최상위 `hwihaLords` 배열에 주공의 원본 장수 이름을 명시할 수 있습니다. 예를 들어 `"hwihaLords": ["조조"]`는 그 이름이 전체 roster에서 유일하고 실제 시작 장수에 포함될 때만 유효합니다. 관직이나 토지 수로 주공을 추정하지 않습니다. 각 장수의 `meta.hwihaLord`에 참/거짓을 기록하며 출사 같은 사건이 이후 지위를 바꿉니다.

HWIHA 시나리오는 선택 항목 `hwihaUnits` 로 초기 부곡을 선언할 수 있습니다. 각 행은 `general`(roster 에서 유일한 장수 이름)·`name`·`troops`·`crewTypeId`·`training`·`morale`·`provisions` 만 담고, 시드가 `general_bugok` 행으로 넣습니다. 선언이 없으면 부곡을 만들지 않습니다. 豫州 조각 운영 후보(`tools/e2e/fixtures/hwiha-yuzhou/scenario_990002.json`)가 이 항목을 쓰며 `scenario_990002` 로 운영 시나리오 목록에 등록되어 있습니다. 등록만으로 현행 월드가 변경되지는 않습니다.
