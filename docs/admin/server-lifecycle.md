...
새로 초기화하는 `han-world-v3` 월드는 `han-world-v3-1228` 판(1,228개 도시)을 사용한다. 續漢書 郡國志 표제인데 지도에 없던 縣 60곳이 제 프로빈스를 받았고, 도시 번호는 1195–1254를 새로 썼다. 폐기한 번호 26개는 계속 비워 두므로 최대 번호(1254)가 도시 수(1228)보다 크다 — 1부터 도시 수까지 연속된다고 가정하면 안 된다.

기존 1,168개·1,194개 도시 저장 월드는 각각 동결된 기존 판으로 읽는다. 서버 이미지 승격만으로 기존 월드에 새 縣이 생기지는 않는다. 승인된 초기화를 실행할 때는 백업을 남기고 reset workflow의 `expected_city_count`에 `1228`을 명시한다(워크플로 기본값도 1228로 바꿨다). 이 문서는 운영 초기화 승인을 대신하지 않는다.
