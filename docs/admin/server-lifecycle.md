...
> **pep 전환 순서:** 저장 키·DB·경로 개명 코드의 게임 서버 승격은 `worldFormat` 세계 형식 가드와 pep C단계 리셋 뒤에 한다. 그 전에는 옛 키가 오류 없이 빈 상태로 읽힐 수 있다.
...
새로 초기화하는 `han-world-v3` 월드는 `han-world-v3-1168` 판(1,168개 도시)을 사용한다. 지리 설명으로 채웠던 임시 거점 26곳을 제거하고 태안·성주·함창의 표시를 신소도·본피·고동람으로 바꿨다. 도시 ID에는 폐기한 번호의 빈자리가 있으며 1부터 도시 수까지 연속된다고 가정하면 안 된다.

기존 1,194개 도시 저장 월드는 동결된 기존 판으로 읽는다. 서버 이미지 승격만으로 기존 월드의 거점이 삭제되지는 않는다. 승인된 초기화를 실행할 때는 백업을 남기고 reset workflow의 `expected_city_count`에 `1168`을 명시한다. 이 문서는 운영 초기화 승인을 대신하지 않는다.

### 새 규칙 시나리오의 주공 선언

`ruleProfile: "HWIHA"` 시나리오는 최상위 `lords` 배열에 주공의 원본 장수 이름을 명시할 수 있습니다. 예를 들어 `"lords": ["조조"]`는 그 이름이 전체 roster에서 유일하고 실제 시작 장수에 포함될 때만 유효합니다. 관직이나 토지 수로 주공을 추정하지 않습니다. 각 장수의 `meta.lord`에 참/거짓을 기록하며 출사 같은 사건이 이후 지위를 바꿉니다.

HWIHA 시나리오는 선택 항목 `units` 로 초기 부곡을 선언할 수 있습니다. 각 행은 `general`(roster 에서 유일한 장수 이름)·`name`·`troops`·`crewTypeId`·`training`·`morale`·`provisions` 만 담고, 시드가 `general_bugok` 행으로 넣습니다. 선언이 없으면 부곡을 만들지 않습니다. 豫州 조각 운영 후보(`tools/e2e/fixtures/yuzhou/scenario_990002.json`)가 이 항목을 쓰며 `scenario_990002` 로 운영 시나리오 목록에 등록되어 있습니다. 등록만으로 현행 월드가 변경되지는 않습니다.

중복·동명이인·없는 이름, 배열이 아닌 값(명시 null 포함), SAMMO 시나리오의 선언은 거절합니다. 설정상 제외되거나 시작 시점에 등장하지 않는 주공 선언도 시드 전에 거절합니다. 지연 등장 주공의 이벤트 전달은 아직 지원하지 않습니다. 기존 SAMMO 시나리오와 저장된 월드에 이 키를 덧붙이지 않습니다. 이 설정은 출사 입력이나 새 게임 전체가 제공된다는 의미가 아닙니다.

### 2026-09-23 결손 縣 편입판

새로 초기화하는 `han-world-v3` 월드는 `han-world-v3-1224` 판(1,224개 도시)을 사용한다. 續漢書 郡國志 표제인데 지도에 없던 縣 56곳이 제 프로빈스를 받았고, 도시 번호는 1342–1397을 새로 썼다. 폐기·철회한 번호 173개(1194 이하 26개와 동결 1341 판의 1195–1341)는 다시 쓰지 않으므로 최대 번호(1397)가 도시 수(1224)보다 크다 — 1부터 도시 수까지 연속된다고 가정하면 안 된다.

기존 1,168개·1,194개 도시 저장 월드는 각각 동결된 기존 판으로 읽는다. 서버 이미지 승격만으로 기존 월드에 새 縣이 생기지는 않는다. 승인된 초기화를 실행할 때는 백업을 남기고 reset workflow의 `expected_city_count`에 `1224`를 명시한다(워크플로 기본값도 1224로 바꿨다). 이 문서는 운영 초기화 승인을 대신하지 않는다.

### 2026-09-23 미해독 3행 제외 판

새 `han-world-v3` 초기화에는 `han-world-v3-1447` 판(1,447개 도시)을 사용한다. 앞 판에서 지도에 없던 郡國志 縣 223행을 각각 독립 城으로 더했다(도시 번호 1398–1620). 원문 글자를 해독하지 못한 `参[�]`·`朴[B459]`·`朱[B42B]` 세 행은 사용자 요청에 따라 城으로 세우지 않았다. 원문과 제외 사유는 `gap-counties-v1.json`에 남긴다. 원장에 `SYNTHETIC_COMMANDERY_CELL`이라고 적힌 좌표는 사용자 승인 게임 격자이며 역사적 縣治 위치가 아니다. 220년에 존속하지 않았거나 기존 城의 이명인 행도 별도 게임 城으로 서 있으므로 지도 위치만으로 역사적 존속을 판단하지 않는다.

기존 1,224개 이하 저장 월드는 동결된 해당 판으로 읽는다. 이미지 승격만으로 기존 월드의 城은 늘지 않는다. 운영 초기화는 별도 승인 후 백업과 함께 실행하며 reset workflow의 `expected_city_count` 기본값은 `1447`이다. 도시 번호는 예약된 173개를 건너뛰므로 최대 번호 1620과 도시 수 1447이 다르다.
