...
게임 API와 엔진은 해당 세계의 전체 도시 ID 집합과 저장된 공간 데이터의 topology revision/hash를 함께
검사해 보존된 지도 자산을 선택합니다. 도시 수나 표시 이름만으로 버전을 고르지 않습니다. 기존
`han-world-v3` 설정을 유지하며, 런타임 선택값을 세계 config/meta에 덧쓰지 않습니다.

보존 자산과 일치하지 않는 도시 집합이나 서로 다른 지도 검증값이 발견되면 최신 지도로 대체하지 않고
실패합니다. 이 경우 해당 세계의 도시 ID와 province/water/army 공간 데이터의 검증값을 같은 백업 및
이미지에 대조하십시오. 검증값 삭제나 도시 수 조정으로 부팅을 강제하지 마십시오. 자산의 원본 커밋과
무결성 계약은 [지도 아카이브](../../data/map/han-world-artifacts-v1/README.md)를 참조하십시오.

새 도시 추가나 이름 변경 자체는 금지되지 않습니다. 새 도시 집합을 도입하는 릴리스는 새 지도 자산을
등록하고 기존 세계와의 호환성을 검증해야 합니다. 저장·재로딩의 로컬 테스트 통과는 운영 백업 복원,
인증된 명령 처리 및 실서버 승격 검증을 대신하지 않습니다.

846도시 릴리스는 [별도 고정 자산](../../data/map/han-world-v3-846-artifacts-v1/README.md)과
