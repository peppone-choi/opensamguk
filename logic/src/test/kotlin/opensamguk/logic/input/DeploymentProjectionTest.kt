...
class DeploymentProjectionTest {
    ...
    private val owner = DeploymentPersonSource(1, 1, false,
        mapOf(DeploymentState.META_KEY to DeploymentState(listOf(corps)).toMetaValue()))
    ...
    private fun build(people: List<DeploymentPersonSource> = listOf(deputy, owner),
        ...
        DeploymentProjector.build(RuleProfile.HWIHA, people, units, cards, position, topology, metrics)
    ...
    @Test fun `projection preserves sorted people cards units and live assessment`() {
        ...
        assertIs<DeploymentAssessment.Eligible>(DeploymentRules.assessActive(corps, result))
    }
    ...
    @Test fun `missing pins malformed metadata and foreign ownership are unavailable`() {
        ...
        for (raw in listOf(null, emptyMap<String, Any>(), owner.meta[DeploymentState.META_KEY])) {
            assertNull(build(listOf(owner.copy(id = 9, meta = mapOf(DeploymentState.META_KEY to raw)), deputy)))
        }
    }
    ...
    @Test fun `global duplicate deployment identifiers commanders and unit cards are unavailable`() {
        ...
        for (other in alternatives) {
            val extra = DeploymentPersonSource(9, 1, false,
                mapOf(DeploymentState.META_KEY to DeploymentState(listOf(other)).toMetaValue()))
    ...
    @Test fun `corps progress requires exact binding position and exclusive movement owner`() {
        val meta = mapOf(CorpsMarchState.META_KEY to state.toMetaValue())
        ...
        assertNull(build(listOf(owner, deputy.copy(meta = mapOf(CorpsMarchState.META_KEY to
            state.copy(deploymentOrderId = "other").toMetaValue())))))
        val march = MarchState(CountyAssignment("assignment", 1, 2, 3), path,
            checkpoint.cursor, phase, checkpoint.stop)
        assertNull(build(listOf(owner, deputy.copy(meta = meta + (MarchState.META_KEY to march.toMetaValue())))))
    }
    ...
    @Test fun `both durable encounter channels remain battle pending`() {
        ...
        val assignment = MarchState(CountyAssignment("assignment", 1, 2, 3), path, cursor, phase, LandMarchStop.ENCOUNTER)
        for (meta in listOf(mapOf(CorpsMarchState.META_KEY to encounter.toMetaValue()),
            mapOf(MarchState.META_KEY to assignment.toMetaValue()))) {
    ...
    @Test fun `destination requires its deployed commander and cannot override an existing path`() {
        val order=CorpsOrder("order",1,2,b,topology.topologyRevision,topology.contentHash)
        val meta=mapOf(CorpsOrder.META_KEY to order.toMetaValue())
        assertNotNull(build(listOf(owner,deputy.copy(meta=meta))))
        assertNull(build(listOf(owner.copy(meta=emptyMap()),deputy.copy(meta=meta))))
        assertNull(build(listOf(owner.copy(meta=owner.meta+meta),deputy)))
        assertNull(build(listOf(owner,deputy.copy(meta=mapOf(CorpsOrder.META_KEY to order.copy(orderId="wrong").toMetaValue())))))
        assertNull(build(listOf(owner,deputy.copy(meta=mapOf(CorpsOrder.META_KEY to order.copy(destination=a).toMetaValue(),
            CorpsMarchState.META_KEY to state.toMetaValue())))))
    }
