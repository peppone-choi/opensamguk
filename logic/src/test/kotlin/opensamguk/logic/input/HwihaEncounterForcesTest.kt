...
class HwihaEncounterForcesTest {
    private val encounter = HwihaCorpsEncounter(HwihaEncounterParticipant("a",1,1,1,listOf(11)),
        listOf(HwihaEncounterParticipant("b",2,3,0,listOf(22))), LandProvince("B"),LandProvince("A"),
    ...
    private fun force() = HwihaEncounterForces(encounter.encounterId,units,commanders)
    private fun read(row: Map<String,Any?>) = HwihaEncounterForces.read(mapOf(HwihaEncounterForces.META_KEY to row),encounter)
    ...
    @Test fun `canonical immutable snapshot preserves exact past values and nullable commander link`() {
        ...
        val f = HwihaEncounterForces(encounter.encounterId,mutable,commanders.reversed())
        mutable.clear()
        ...
        assertNull(HwihaEncounterForces.read(emptyMap(),encounter))
        assertFailsWith<IllegalArgumentException> { HwihaEncounterForces.read(mapOf(HwihaEncounterForces.META_KEY to null),encounter) }
    }
    @Test fun `ranges and duplicate identities reject without clamps`() {
        ...
        assertFailsWith<IllegalArgumentException> { HwihaEncounterForces(encounter.encounterId,units+units[0],commanders) }
        assertFailsWith<IllegalArgumentException> { HwihaEncounterForces(encounter.encounterId,units,commanders+commanders[0]) }
    }
    @Test fun `exact encounter unit owner commander and commander set binding required`() {
        val invalid = listOf(HwihaEncounterForces("b".repeat(64),units,commanders),
            HwihaEncounterForces(encounter.encounterId,units.drop(1),commanders),
            HwihaEncounterForces(encounter.encounterId,listOf(units[0].copy(ownerGeneralId=2),units[1]),commanders),
            HwihaEncounterForces(encounter.encounterId,listOf(units[0].copy(commanderGeneralId=3),units[1]),commanders),
            HwihaEncounterForces(encounter.encounterId,units,commanders.drop(1)))
