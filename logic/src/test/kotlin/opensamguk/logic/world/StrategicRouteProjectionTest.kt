...
class StrategicRouteProjectionTest {
    @Test
    fun `dry projection uses stable province IDs and excludes river only and adjudicated borders`() {
        ...
        val edges = projectDryLandEdges(provinces, owner, terrain, setOf('1'), emptyList(), "sha")
        ...
        assertEquals(emptyList(), projectDryLandEdges(provinces, owner, terrain, setOf('1'), listOf(barrier), "sha"))
        val reordered = projectDryLandEdges(
    ...
    fun `invalid owner domains and incomplete raster coverage fail closed`() {
        assertFailsWith<IllegalArgumentException> {
            projectDryLandEdges(listOf("P1"), intArrayOf(1), listOf("1"), setOf('1'), emptyList(), "sha")
        }
        assertFailsWith<IllegalArgumentException> {
            projectDryLandEdges(listOf("P1"), intArrayOf(0), listOf("11"), setOf('1'), emptyList(), "sha")
        }
    }

    @Test
    fun `runtime identities resolve through an exact route binding and never treat missing provinces as ordinals`() {
        val edges = projectDryLandEdges(listOf("A", "B"), intArrayOf(0, 1), listOf("11"), setOf('1'), emptyList(), "sha")
        val topology = StrategicTopologySnapshot("test", setOf("A", "B"), emptyList(), edges, emptyList(), mapOf("base" to "sha"))
        val bindings = listOf(
            StrategicRouteBinding(273, "route-lu", "physical-lu", "A"),
            StrategicRouteBinding(781, "route-licheng", "physical-licheng", "B"),
            StrategicRouteBinding(5, "unmapped", "physical-unmapped", null),
        )
        val projection = StrategicRouteProjection(topology, bindings)
        ...
        assertFailsWith<IllegalArgumentException> { StrategicRouteProjection(topology, bindings + bindings[0]) }
        assertFailsWith<IllegalArgumentException> {
            StrategicRouteProjection(topology, bindings + StrategicRouteBinding(8, "route-lu", "another", "A"))
        }
    }
}
