...
class HanProvinceSeatsTest {
    ...
    fun `han seatCountByProvince sums to the actual gunchi count`() {
        val seats = HistoricalCityConstVariant.seatCountByProvince
        val actualGunchiCount = HistoricalCityConstVariant.all().values.count { HistoricalCityConstVariant.countsForNationLevel(it.level) }
