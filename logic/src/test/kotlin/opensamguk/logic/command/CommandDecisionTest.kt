...
class CommandDecisionTest {
    private val pinned = CityTransportArgs(1, 2, 100, 0, 0, 9, "v3:abc", "path:123")
    ...
    private val strategicContext = CityTransportContext(
    ...
    fun `V3 accepts only the pinned strategic route without consulting legacy distance`() {
        assertIs<CityTransportDecision.Applied>(decideCityTransport(pinned, strategicContext))
        assertIs<CityTransportDecision.Applied>(decideCityTransport(pinned, strategicContext.copy(hopDistance = 99)))
    }
    ...
    fun `V3 rejects missing and stale pins even if CityConst says adjacent`() {
        listOf(
            ...
            assertEquals(code, assertIs<CityTransportDecision.Denied>(
    ...
    fun `V3 missing topology unknown node and disconnected path never fall back to CityConst`() {
        assertEquals("TOPOLOGY_STATE_INVALID", assertIs<CityTransportDecision.Denied>(
            decideCityTransport(pinned, strategicContext.copy(hopDistance = 1, strategicRoute = null)),
        ).code)
        PathDenialCode.entries.forEach { code ->
            assertEquals(code.name, assertIs<CityTransportDecision.Denied>(decideCityTransport(
    ...
    fun `strategic route denial presents actionable Korean reasons instead of raw codes`() {
        mapOf(
            ...
            val denied = assertIs<CityTransportDecision.Denied>(decideCityTransport(
    ...
    fun `multi-hop land and every crossing or water mode require future scheduler`() {
        ...
        assertEquals("ROUTE_REQUIRES_MULTI_TURN", assertIs<CityTransportDecision.Denied>(decideCityTransport(
        ...
        TraversalMode.entries.filter { it != TraversalMode.LAND }.forEach { mode ->
            assertEquals("TRANSPORT_MODE_UNSUPPORTED", assertIs<CityTransportDecision.Denied>(decideCityTransport(
    ...
    fun `V3 route success still enforces escort caps and ledger balance`() {
        listOf(
            ...
            assertEquals(code, assertIs<CityTransportDecision.Denied>(decideCityTransport(case.first, case.second)).code)
        }
    }
    @Test
    fun `recruit context denies a missing actor with a stable reason`() {
        val decision = decideGarrisonRecruit(
            GarrisonRecruitArgs(cityId = 5, amount = 100),
            GarrisonRecruitContext.missingGeneral(),
        )

        val denied = assertIs<GarrisonRecruitDecision.Denied>(decision)
    ...
    fun `recruit context evaluates ownership population and ledger balance`() {
        val args = GarrisonRecruitArgs(cityId = 5, amount = 100)
        val context = GarrisonRecruitContext(
            generalCityId = 5,
            generalNationId = 1,
            leadership = 80,
            cityNationId = 1,
        ...
        val denied = assertIs<GarrisonRecruitDecision.Denied>(decideGarrisonRecruit(args, context))
    ...
    fun `transport context evaluates adjacency and ledger balance`() {
        val args = CityTransportArgs(1, 2, 100, 0, 0, 7)
        val context = CityTransportContext(
            generalCityId = 1,
            generalNationId = 1,
            escortCrew = 2_000,
            fromNationId = 1,
        ...
        val denied = assertIs<CityTransportDecision.Denied>(decideCityTransport(args, context))
