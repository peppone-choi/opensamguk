...
/**
 * AREA F4-C3-CHIEF — che_피장파장(ChePijangPajang) PHP-골든 draw-for-draw 바이트 게이트.
 *
 * 실제 Kotlin 리졸버를 [CommandRegistry]로 해석해 구동하고(REGISTRY RESOLUTION 필수),
 * golden/p2/che_피장파장-fixtures.json(hiddenSeed a2db167c.., acting gid 152)를 바이트/draw 오라클로 삼는다.
 *
 * 검증 항목(게이트 레시피):
 *  - 6-토큰 시드 문자열이 골든 seedString과 byte-equal(rngFor GUARD; rawClassName=che_피장파장).
 *  - RNG draw 스트림: che_피장파장은 deterministic(draw_count=0) → LiteHashDRBG 커서(stateIdx/bufferIdx)가
 *    resolve 전/후 불변임을 확인(어떤 draw든 커서를 전진시키므로 0-draw 오라클).
 *  - acting logLines + broadcastLines byte-match.
 *  - general after-delta(experience/dedication, phpRound half-away; explevel/gold 불변).
 *
 * che_피장파장 run()(che_피장파장.php:157-244): 피장파장 발동 로그 1줄
 * (`<G><b>{commandType명}</b></> 전략의 피장파장 발동! <1>HH:MM</>`) + exp/ded +10(=5*(preReqTurn1+1)).
 * 아국/적국 장수 개별 broadcastMessage + national/general-history는 다른 ActionLogger/history 싱크로 가는
 * 엔진-스코프 부수효과라 actor scope에 나타나지 않으며 pushGlobalActionLog도 호출하지 않음 → broadcastLines=[].
 * nation_env/dest nation_env delay KV(delayCnt=60) write는 엔진-스코프 KV 스테이징 seam(golden 미노출).
 */
class Che피장파장GoldenTest {

    private val pipeline = GeneralActionPipeline()
    private val registry = CommandRegistry(pipeline)

    @Test
    fun `che_피장파장 golden byte-matches the PHP action log, zero-draw RNG, and post-state`() {
        val command = "che_피장파장"
