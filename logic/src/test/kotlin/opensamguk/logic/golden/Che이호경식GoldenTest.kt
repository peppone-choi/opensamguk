...
/**
 * AREA F4-C3-CHIEF — che_이호경식(CheIhoGyeongsik) PHP-골든 draw-for-draw 바이트 게이트.
 *
 * 실제 Kotlin 리졸버를 [CommandRegistry]로 해석해 구동하고(REGISTRY RESOLUTION 필수),
 * golden/p2/che_이호경식-fixtures.json(hiddenSeed a2db167c.., acting gid 152)를 바이트/draw 오라클로 삼는다.
 *
 * 검증 항목(게이트 레시피):
 *  - 6-토큰 시드 문자열이 골든 seedString과 byte-equal(rngFor GUARD; rawClassName = che_이호경식 default).
 *  - RNG draw 스트림: che_이호경식은 deterministic(draw_count=0) → LiteHashDRBG 커서(stateIdx/bufferIdx)가
 ...
 *
 * che_이호경식 run()(che_이호경식.php:122-201): 이호경식 발동 로그 1줄 + exp/ded +5 + nation
 * strategic_cmd_limit=9 + diplomacy state→1 + term +3(양방향) + SetNationFront 양국. 아국/적국 장수 개별
 * PLAIN 로그·국가/국사 로그는 다른 ActionLogger로 flush되는 엔진-스코프 부수효과라 actor scope에 나타나지
 * 않으며 pushGlobalActionLog도 호출하지 않음 → broadcastLines=[].
 */
class Che이호경식GoldenTest {

    private val pipeline = GeneralActionPipeline()
    private val registry = CommandRegistry(pipeline)

    @Test
    fun `che_이호경식 golden byte-matches the PHP action log, zero-draw RNG, and post-state`() {
        val command = "che_이호경식"
