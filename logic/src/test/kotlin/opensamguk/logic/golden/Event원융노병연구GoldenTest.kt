...
/**
 * AREA F4-C3-CHIEF — event_원융노병연구(EventWonyungnobyeongYeongu) PHP-골든 draw-for-draw 바이트 게이트.
 *
 * 실제 Kotlin 리졸버를 [CommandRegistry]로 해석해 구동하고(REGISTRY RESOLUTION 필수),
 * golden/p2/event_원융노병연구-fixtures.json(hiddenSeed cbc88849.., acting gid 152 ⓝ하진, nation 1 후한 사령)을
 * 바이트/draw 오라클로 삼는다.
 *
 * event_원융노병연구 run()(event_원융노병연구.php:67-106)은 RNG 미사용(deterministic, draw_count=0). 효과 순서:
 *   1) addExperience/addDedication += 5*(preReqTurn+1) = 5*24 = 120 (exp 4582→4702, ded 4120→4240)
 *   2) nation gold -= 100000(1000000→900000), rice -= 100000(1000000→900000), aux[can_원융노병사용]=1
 *      (기존 aux 키 can_국기변경/can_국호변경 뒤에 삽입 — LinkedHashMap 순서 보존)
 *   3) pushGeneralActionLog("<M>원융노병 연구</> 완료") — actor logs()에 나타나는 유일한 라인
 *      (pushGeneralHistoryLog/pushNationalHistoryLog는 별도 history 스코프 → actor logs() 미포함)
 *   4) increaseInheritancePoint(active_action, 1) — P6 seam(actor-scope 가시 상태 무영향)
 *
 * 검증 항목(게이트 레시피):
 ...
 *  - nation after-delta(gold/rice/aux[can_원융노병사용]).
 */
class Event원융노병연구GoldenTest {

    private val pipeline = GeneralActionPipeline()
    private val registry = CommandRegistry(pipeline)

    @Test
    fun `event_원융노병연구 golden byte-matches the PHP action log, zero-draw RNG, and post-state`() {
        val command = "event_원융노병연구"
        ...
        for (c in f.cases) {
            ...
            // --- nation after-delta: gold/rice -= 100000, aux[can_원융노병사용]=1(기존 키 순서 보존) ---
            val n = draft.nation!!
            ...
            assertEquals(1, (aux["can_원융노병사용"] as Number).toInt(), "[$command/${c.name}] aux[can_원융노병사용]=1")
            assertEquals(
                listOf("can_국기변경", "can_국호변경", "can_원융노병사용"),
                aux.keys.toList(),
                "[$command/${c.name}] aux insertion order preserved (can_원융노병사용 appended last)",
            )
        }
    }
}
