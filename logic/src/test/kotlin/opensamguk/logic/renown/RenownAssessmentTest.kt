...
class RenownAssessmentTest {
    private val curve = RenownAssessment.CANON

    @Test
    fun `오르는 경로와 떨어지는 경로를 함께 누적한다`() {
        val tally = RenownAssessment.Tally(warMerit = 2, office = 1, defeat = 1)
        // 30 + (3*2) + 4 - 3 = 37
        assertEquals(37, RenownAssessment.updatedRenown(30, tally, curve))
    }

    @Test
    fun `사건이 없으면 명망을 보존한다 - 월단평은 초기화하지 않는다`() {
        val kept = RenownAssessment.updatedRenown(83, RenownAssessment.Tally(), curve)
    ...
    fun `코스트 합이 갱신된 명망을 넘으면 충성 낮은 쪽부터 이탈한다`() {
        val retinue = listOf(
            RenownAssessment.RetainerCard(retainerId = 1, cost = 7, loyalty = 90),
            RenownAssessment.RetainerCard(retainerId = 2, cost = 7, loyalty = 20),
            RenownAssessment.RetainerCard(retainerId = 3, cost = 7, loyalty = 50),
        )
        // 배신으로 30 → 22. 코스트 합 21 ≤ 22 이므로 아무도 안 나간다.
        val kept = RenownAssessment.assess(9, 30, RenownAssessment.Tally(betrayal = 1), curve, retinue)
        ...
        val shed = RenownAssessment.assess(9, 30, RenownAssessment.Tally(betrayal = 2), curve, retinue)
    ...
    fun `동점 충성은 id 큰 쪽이 먼저 나간다 - 리플레이가 갈라지지 않게`() {
        val retinue = listOf(
            RenownAssessment.RetainerCard(1, cost = 10, loyalty = 40),
            RenownAssessment.RetainerCard(2, cost = 10, loyalty = 40),
        )
        val out = RenownAssessment.assess(1, 12, RenownAssessment.Tally(), curve, retinue)
    ...
    fun `이탈 순번은 assess 와 같은 순서이고 상한 이하면 비어 있다`() {
        val retinue = listOf(
            RenownAssessment.RetainerCard(1, cost = 5, loyalty = 30),
            RenownAssessment.RetainerCard(2, cost = 5, loyalty = 10),
            RenownAssessment.RetainerCard(3, cost = 5, loyalty = 30),
            RenownAssessment.RetainerCard(4, cost = 5, loyalty = 90),
        )
        // 합 20, 상한 8 → 충성 10(2), 30 동점은 id 큰 3 먼저, 그다음 1. 5 ≤ 8 에서 멈춘다.
        assertEquals(listOf(2, 3, 1), RenownAssessment.departures(8, retinue))
        assertEquals(
            RenownAssessment.assess(9, 12, RenownAssessment.Tally(betrayal = 1), curve, retinue).released,
            RenownAssessment.departures(10, retinue),
            "월단평(명망 12 → 하한 10)과 조회가 같은 함수를 쓴다",
        )
        assertEquals(emptyList(), RenownAssessment.departures(20, retinue))
    }
    ...
    fun `순위는 명망 내림차순 동점은 id 오름차순이다`() {
        val ranked = RenownAssessment.ranking(mapOf(7 to 50, 3 to 90, 5 to 50, 1 to 90))
    ...
    fun `잘못된 곡선과 집계는 거절한다`() {
        ...
        assertFailsWith<IllegalArgumentException> { RenownAssessment.Tally(warMerit = -1) }
        assertFailsWith<IllegalArgumentException> {
            RenownAssessment.assess(
                1, 30, RenownAssessment.Tally(), curve,
                listOf(
                    RenownAssessment.RetainerCard(1, 1, 1),
                    RenownAssessment.RetainerCard(1, 1, 2),
                ),
            )
        }
    }
    ...
    fun `정본 데이터 파일이 유효한 곡선으로 읽힌다`() {
        ...
        val fromFile = RenownAssessment.Curve(
        ...
        assertEquals(
            RenownAssessment.CANON, fromFile,
            "코드의 CANON 과 정본 파일이 갈라졌다 — 한쪽만 고치면 런타임이 파일을 배신한다",
        )
        assertEquals(RenownRules.INITIAL_CAPACITY, intOf("initialRenown"), "초기 명망은 한 곳에서만 온다")
        assertTrue(fromFile.ceiling > RenownRules.INITIAL_CAPACITY, "상한이 초기값보다 높다")
        assertTrue(fromFile.floor <= RenownRules.INITIAL_CAPACITY, "하한이 초기값보다 낮거나 같다")
    }
