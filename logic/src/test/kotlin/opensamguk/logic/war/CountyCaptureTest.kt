...
class HwihaCountyCaptureTest {
    ...
    fun `수비대는 무장 해제되어 동수가 현지 인구가 된다`() {
        val out = HwihaCountyCapture.settle(county(population = 1_000, garrison = 300), captorNationId = 2)
    ...
    fun `수비대는 승자 군대에 편입되지 않는다`() {
        ...
        val out = HwihaCountyCapture.settle(before, captorNationId = 7)
    ...
    @Test
    fun `중립 縣 도 점령할 수 있다`() {
        val out = HwihaCountyCapture.settle(county(owner = 0), captorNationId = 3)
        assertEquals(3, out.ownerNationId)
    }

    @Test
    fun `인구와 재고는 줄지 않는다 - 평화 항복 기준`() {
        val out = HwihaCountyCapture.settle(county(population = 1_000, garrison = 0), captorNationId = 2)
    ...
    fun `이미 소유한 縣 을 점령하려 하면 거절한다`() {
        assertFailsWith<IllegalArgumentException> {
            HwihaCountyCapture.settle(county(owner = 5), captorNationId = 5)
        }
    }
    ...
    fun `음수 입력은 거절한다`() {
        assertFailsWith<IllegalArgumentException> {
            HwihaCountyCapture.settle(county(population = -1), captorNationId = 2)
        }
        assertFailsWith<IllegalArgumentException> {
            HwihaCountyCapture.settle(county(garrison = -1), captorNationId = 2)
        }
    }
}
