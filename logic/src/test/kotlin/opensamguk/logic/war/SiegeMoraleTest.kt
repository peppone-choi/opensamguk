...
class SiegeMoraleTest {
    @Test
    fun `완전 급식은 회복시키고 상한을 넘지 않는다`() {
        val fromLow = SiegeMorale.settle(1000, rationDemand = 50, rationServed = 50, encircled = true)
        assertEquals(2250, fromLow.morale)
        assertFalse(fromLow.surrendered)

        val atCap = SiegeMorale.settle(
            SiegeMorale.MAX_MORALE, rationDemand = 50, rationServed = 50, encircled = true,
        )
        assertEquals(SiegeMorale.MAX_MORALE, atCap.morale, "상한을 넘지 않는다")
    }

    @Test
    fun `군량을 전부 굶으면 25퍼센트를 잃는다`() {
        val out = SiegeMorale.settle(10000, rationDemand = 80, rationServed = 0, encircled = true)
    ...
    fun `부분 부족은 부족 비율만큼 올림으로 깎는다`() {
        ...
        val out = SiegeMorale.settle(10000, rationDemand = 3, rationServed = 2, encircled = true)
    ...
    fun `사기 1 에서도 극소 부족이 항복까지 끌고 간다`() {
        ...
        val out = SiegeMorale.settle(1, rationDemand = 100_000, rationServed = 99_999, encircled = true)
    ...
    fun `같은 순 포위 해제는 사기 0 이어도 항복을 막는다`() {
        val relieved = SiegeMorale.settle(1000, rationDemand = 10, rationServed = 0, encircled = false)
    ...
    fun `수요가 없으면 굶길 것이 없어 완전 급식이다`() {
        val out = SiegeMorale.settle(5000, rationDemand = 0, rationServed = 0, encircled = true)
        assertEquals(6250, out.morale)
    }

    @Test
    fun `수요를 넘겨 먹여도 완전 급식으로만 센다`() {
        val out = SiegeMorale.settle(5000, rationDemand = 10, rationServed = 999, encircled = true)
        assertEquals(6250, out.morale)
    }
    ...
    fun `범위를 벗어난 입력은 거절한다`() {
        assertFailsWith<IllegalArgumentException> {
            SiegeMorale.settle(-1, rationDemand = 1, rationServed = 0, encircled = true)
        }
        assertFailsWith<IllegalArgumentException> {
            SiegeMorale.settle(SiegeMorale.MAX_MORALE + 1, rationDemand = 1, rationServed = 0, encircled = true)
        }
        assertFailsWith<IllegalArgumentException> {
            SiegeMorale.settle(100, rationDemand = -1, rationServed = 0, encircled = true)
        }
    }
}
