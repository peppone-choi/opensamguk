...
class BattlePlansTest {
    private val encounter=CorpsEncounter(EncounterParticipant("a",1,1,1,listOf(10)),
        listOf(EncounterParticipant("b",2,2,2,listOf(20))),LandProvince("B"),LandProvince("A"),
        Phase(200,1,1),"qa","a".repeat(64))
    private fun defaults()=BattlePlans.defaultFor(encounter)
    private fun read(raw:Any?)=BattlePlans.read(mapOf(BattlePlans.META_KEY to raw),encounter)
    @Test fun `default plans seal explicit attacker defender actions and three retreat thresholds`() {
        ...
        assertNull(BattlePlans.read(emptyMap(),encounter))
    ...
    @Test fun `canonical immutable plans ignore construction order and copy caller collections`() {
        ...
        val snapshot=BattlePlans(encounter.encounterId,plans);plans.clear()
        ...
        assertNotEquals(defaults().snapshotId,BattlePlans(encounter.encounterId,listOf(
    ...
    @Test fun `ranges unique slots initial action and exact participant set are enforced`() {
        ...
        assertFailsWith<IllegalArgumentException> { BattlePlans(encounter.encounterId,listOf(defaults().plans[0],defaults().plans[0])) }
        val incomplete=BattlePlans(encounter.encounterId,listOf(defaults().plans[0]))
        assertFailsWith<IllegalArgumentException> { read(incomplete.toMetaValue()) }
        assertFailsWith<IllegalArgumentException> { defaults().requireBinding(encounter.copy(phase=Phase(200,1,2))) }
    }
