...
class HwihaEncounterCombatProfilesTest {
    private val profile = HwihaUnitProfile(1100, 1, 1, 100, 120, 20)
    private val rules = HwihaUnitProfiles(1, "a".repeat(64), listOf(profile), setOf(1000))
    private fun forces(vararg types: Int) = HwihaEncounterForces("b".repeat(64), types.mapIndexed { i, type ->
        EncounterUnitForce(i+1, 1, 1, type, 500, 50, 50, 0, 500, null)
    }, listOf(EncounterCommanderForce(1, 70, 70, 70, 70, 70)))
    private fun read(row: Any?, forces: HwihaEncounterForces, catalog: HwihaUnitProfiles = rules) =
        HwihaEncounterCombatProfiles.read(mapOf(HwihaEncounterCombatProfiles.META_KEY to row), forces, catalog)

    @Test fun `captures distinct profiles bound to all force observations and exact catalog pin`() {
        val force = forces(1100, 1100)
        val sealed = HwihaEncounterCombatProfiles.capture(force, rules)
    ...
    @Test fun `unknown and known unsupported types retain distinct reasons without unit substitution`() {
        ...
        assertEquals(listOf(HwihaEncounterCombatProfiles.Reason.UNKNOWN_CREW_TYPE,
            HwihaEncounterCombatProfiles.Reason.UNSUPPORTED_CREW_TYPE), sealed.unavailable.map { it.reason })
        assertEquals(listOf(profile), sealed.profiles)
    ...
    @Test fun `different force snapshot or new catalog cannot rewrite history`() {
        ...
        val row = HwihaEncounterCombatProfiles.capture(force, rules).toMetaValue()
        val changed = HwihaEncounterForces(force.encounterId,
        ...
        assertFailsWith<IllegalArgumentException> { read(row, force,
            HwihaUnitProfiles(1, "c".repeat(64), listOf(profile), setOf(1000))) }
        assertFailsWith<IllegalArgumentException> { read(row, force,
            HwihaUnitProfiles(1, rules.contentHash, listOf(profile.copy(attackPower=101)), setOf(1000))) }
    }
    @Test fun `missing is absent while null extra keys coercions and numerical tampering reject`() {
        val force = forces(1100)
        val row = HwihaEncounterCombatProfiles.capture(force, rules).toMetaValue()
        assertNull(HwihaEncounterCombatProfiles.read(emptyMap(), force, rules))
