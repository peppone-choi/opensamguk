...
import opensamguk.logic.war.hwiha.HwihaGridMovement.UnitPosition
import opensamguk.logic.war.hwiha.HwihaGridMovement.Intent
import opensamguk.logic.war.hwiha.HwihaGridMovement.Outcome

class HwihaGridMovementTest {
    private val layout = (HwihaBattlefieldLayout.prepare(HanProvinceCellIndex(
        ...
        "b", "a") as HwihaBattlefieldLayout.Result.Ready).layout
    ...
    private fun resolve(units: List<UnitPosition>, vararg intents: Intent) = HwihaGridMovement.resolve(layout, units, intents.toList())
    ...
    @Test fun `contested empty cell uses initiative then identity independently of input ordering`() {
        ...
        val expected = HwihaGridMovement.resolve(layout, units, intents)
        assertEquals(listOf(Outcome.CONTESTED, Outcome.MOVED), expected.map { it.outcome })
        assertEquals(expected, HwihaGridMovement.resolve(layout, units.reversed(), intents.reversed()))
