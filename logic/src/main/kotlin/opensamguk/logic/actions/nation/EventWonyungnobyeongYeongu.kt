...
/**
 * event_원융노병연구 — STUB(포팅 대기) of `legacy/devsam-core/hwe/sammo/Command/Nation/event_원융노병연구.php`.
 *
 * 사령(BeChief) 전용 국가 연구 커맨드. run()은 RNG 미사용(deterministic, draw_count=0). 효과 순서:
 *   1) addExperience/addDedication += 5*(preReqTurn+1) = 120
 *   2) nation gold -= reqGold, rice -= reqRice, aux[can_원융노병사용]=1
 ...
 *
 * fullConditionConstraints(event_원융노병연구.php init, first-deny-wins):
 *   [OccupiedCity, BeChief, ReqNationAuxValue(can_원융노병사용,0,"<",1,"{name}가 이미 완료되었습니다."),
 *    ReqNationGold(basegold+reqGold), ReqNationRice(baserice+reqRice)].
 */
fun eventWonyungnobyeongYeongu(pipeline: GeneralActionPipeline): NationCommand = EventWonyungnobyeongYeongu(pipeline)

class EventWonyungnobyeongYeongu(@Suppress("UNUSED_PARAMETER") private val pipeline: GeneralActionPipeline) : NationCommand() {
    override val key: String get() = "event_원융노병연구"
    override val name: String get() = "원융노병 연구"

    /** event_원융노병연구.php auxType — NationAuxKey::can_원융노병사용. */
    val auxKey: String get() = "can_원융노병사용"

    /** event_원융노병연구.php getPreReqTurn = 23 (reqTurn = 24). */
    override fun getPreReqTurn(): Int = 23

    /** event_원융노병연구.php getCost = [reqGold, reqRice]. */
    val reqGold: Int get() = 100000
    val reqRice: Int get() = 100000

    /** event_원융노병연구.php argTest = true (무인자). */
    ...
    override fun resolve(context: GeneralActionResolveContext) {
        ...
        // 1) exp/ded += 5*(preReqTurn+1) = 120 (event_원융노병연구.php:84-85).
        //    run() 코드 순서상 gold/rice/aux 갱신·로그보다 먼저 적용한다.
        //    fixture가 모듈-프리라 onCalcStat은 raw 그대로 → exact +120(exp 4582→4702, ded 4120→4240).
        ...
        // 2) nation: gold -= reqGold, rice -= reqRice, aux[can_원융노병사용]=1 (event_원융노병연구.php:78-94).
        ...
        // 3) general action log + general/national history log (event_원융노병연구.php:96-98).
        ...
        // 4) increaseInheritancePoint(active_action, 1) (event_원융노병연구.php:100) — P6 유산포인트 seam.
        //    골든 fixture는 inheritanceAfter.active_action 델타를 actor-scope 가시 상태로 캡처하지 않아
        //    여기서는 별도 부수효과가 없다. event_상병연구/극병연구와 동일한 처리.
    }
}
