...
/**
 * event_음귀병연구 — faithful port of `legacy/devsam-core/hwe/sammo/Command/Nation/event_음귀병연구.php`.
 *
 * 사령(BeChief) 전용 국가 연구 커맨드. run()은 RNG 미사용(deterministic, draw_count=0). 효과 순서:
 *   1) addExperience/addDedication += 5*(preReqTurn+1) = 60
 *   2) nation gold -= reqGold, rice -= reqRice, aux[can_음귀병사용]=1
 ...
 *
 * fullConditionConstraints(event_음귀병연구.php init, first-deny-wins):
 *   [OccupiedCity, BeChief, ReqNationAuxValue(can_음귀병사용,0,"<",1,"{name}가 이미 완료되었습니다."),
 *    ReqNationGold(basegold+reqGold), ReqNationRice(baserice+reqRice)].
 */
fun eventEumgwibyeongYeongu(pipeline: GeneralActionPipeline): NationCommand = EventEumgwibyeongYeongu(pipeline)

class EventEumgwibyeongYeongu(private val pipeline: GeneralActionPipeline) : NationCommand() {
    override val key: String get() = "event_음귀병연구"
    override val name: String get() = "음귀병 연구"

    /** event_음귀병연구.php auxType — NationAuxKey::can_음귀병사용. */
    val auxKey: String get() = "can_음귀병사용"

    /** event_음귀병연구.php getPreReqTurn = 11 (reqTurn = 12). */
    override fun getPreReqTurn(): Int = 11

    /** event_음귀병연구.php getCost = [reqGold, reqRice]. */
    val reqGold: Int get() = 50000
    val reqRice: Int get() = 50000

    /** event_음귀병연구.php argTest = true (무인자). */
    ...
    override fun resolve(context: GeneralActionResolveContext) {
        ...
        // 1) exp/ded += 5*(preReqTurn+1) = 60 (event_음귀병연구.php:84-85).
        ...
        // 2) nation: gold -= reqGold, rice -= reqRice, aux[can_음귀병사용]=1 (event_음귀병연구.php:78-94).
        ...
        // 3) general action log + general/national history log (event_음귀병연구.php:96-98).
        ...
        // 4) increaseInheritancePoint(active_action, 1) (event_음귀병연구.php:100) — P6 유산포인트 seam.
        //    골든 fixture는 inheritanceAfter.active_action=null(owner 0/NPC, 델타 미캡처)이라
        //    actor-scope 가시 상태에는 영향이 없다. event_화시병연구와 동일한 처리.
    }
}
