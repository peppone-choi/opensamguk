 ...
 * acceptance). It sends a [DiplomaticMessage] (action=cancelNA) to the dest nation; the diplomacy
...
class CheBulgachimPagijeui(@Suppress("UNUSED_PARAMETER") pipeline: GeneralActionPipeline) : NationCommand() {
    ...
    /**
     * che_불가침파기제의.php:107-175 run(). Pushes the action log + sends a cancelNA [DiplomaticMessage].
    ...
    override fun resolve(context: GeneralActionResolveContext) {
        ...
        // DiplomaticMessage(action=cancelNA, deletable=false, validUntil=max(30, turnterm*3)분) send → step 3b.
    }
}
