...
class CheGyeonmun(@Suppress("UNUSED_PARAMETER") private val pipeline: GeneralActionPipeline) : GeneralActionDefinition {
    ...
    override fun resolve(context: GeneralActionResolveContext) {
        ...
        val outcome = rng.choiceUsingWeightPair(OUTCOMES.map { it to 1.0 })
        var text = rng.choice(outcome.texts)
        var g = d.general
        var exp = 0.0
        ...
        if (outcome.flags and WOUNDED != 0) {
            g = g.copy(injury = minOf(80, g.injury + rng.nextRangeInt(10, 20)))
        }
        if (outcome.flags and HEAVY_WOUNDED != 0) {
            g = g.copy(injury = minOf(80, g.injury + rng.nextRangeInt(20, 50)))
        }
    ...
    private data class Outcome(val flags: Int, val texts: List<String>)
    ...
    private companion object {
        ...
        val OUTCOMES = listOf(
            ...
            Outcome(INC_HEAVY_EXP or INC_LEADERSHIP, listOf("백성들에게 현인의 가르침을 설파했습니다.", "어느 집의 도망친 가축을 되찾아 주었습니다.")),
            Outcome(INC_HEAVY_EXP or INC_STRENGTH, listOf("동네 장사와 힘겨루기를 하여 멋지게 이겼습니다.", "어느 집의 무너진 울타리를 고쳐주었습니다.")),
            Outcome(INC_HEAVY_EXP or INC_INTEL, listOf("어느 명사와 설전을 벌여 멋지게 이겼습니다.", "거리에서 글 모르는 아이들을 모아 글을 가르쳤습니다.")),
