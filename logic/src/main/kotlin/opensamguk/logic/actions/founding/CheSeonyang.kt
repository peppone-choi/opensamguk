...
class CheSeonyang(@Suppress("UNUSED_PARAMETER") private val pipeline: GeneralActionPipeline) : GeneralActionDefinition {
    ...
    var lastRuntimeDenied: Boolean = false
        private set
    ...
    override fun parseArgsForGeneral(raw: Map<String, Any?>, generalId: Int): Map<String, Any?> =
        argTest(raw, selfId = generalId) ?: emptyMap()

    override fun resolve(context: GeneralActionResolveContext) {
        lastRuntimeDenied = false
        val d = context.draft
        val dest = d.destGeneral ?: return
        val penalty = dest.meta["penalty"] as? Map<*, *> ?: emptyMap<Any, Any?>()
        if (listOf("noChief", "noFoundNation", "noAmbassador").any { penalty.containsKey(it) }) {
            context.addLog("선양할 수 없는 장수입니다.")
            lastRuntimeDenied = true
            return
        }
        val destName = context.destGeneralName.ifEmpty { (dest.meta["name"] as? String) ?: "" }
        val actorName = context.generalName.ifEmpty { (d.general.meta["name"] as? String) ?: "" }
        val nationName = d.nation?.name.orEmpty()
        val josaYi = JosaUtil.pick(actorName, "이")
        d.destGeneral = dest.copy(officerLevel = 12, meta = withMeta(dest.meta, "officer_city" to 0))
        val g0 = d.general
        val arg = linkedMapOf("destGeneralID" to dest.id)
        var g = g0.copy(
            officerLevel = 1,
            experience = g0.experience * 0.7,
            lastTurn = LastTurn(name, arg),
            meta = withMeta(g0.meta, "officer_city" to 0),
        )
        context.addGlobalHistoryLog("<Y><b>【선양】</b></><Y>$actorName</>$josaYi <D><b>$nationName</b></>의 군주 자리를 <Y>$destName</>에게 선양했습니다.")
        context.addNationalHistoryLog("<Y>$actorName</>$josaYi <Y>$destName</>에게 선양")
        context.addLog("<Y>$destName</>에게 군주의 자리를 물려줍니다. <1>${context.date}</>")
        context.addLogTo(dest.id, "<Y>$actorName</>에게서 군주의 자리를 물려받습니다.")
        context.addGeneralHistoryLog("<D><b>$nationName</b></>의 군주자리를 <Y>$destName</>에게 선양")
        context.addHistoryLogTo(dest.id, "<D><b>$nationName</b></>의 군주자리를 물려 받음")
        val statChange = checkStatChange(g)
        g = statChange.general
