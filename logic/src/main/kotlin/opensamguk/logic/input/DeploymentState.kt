...
data class HwihaDeploymentState(val corps: List<HwihaDeployedCorps>) {
    init {
    ...
    companion object {
        ...
        fun read(meta: Map<String, Any?>): HwihaDeploymentState? {
            ...
            return HwihaDeploymentState(corps.map { item ->
                val row = item as? Map<*, *> ?: invalid()
                require(row.keys == fields)
                val units = row["bugokIds"] as? List<*> ?: invalid()
                HwihaDeployedCorps(row["orderId"] as? String ?: invalid(),
