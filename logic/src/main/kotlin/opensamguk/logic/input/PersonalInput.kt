...
enum class TrainingStat(val wireName: String) {
...
data class PersonalRequest(val actorId: Int, val inputId: String,
    val trainingStat: TrainingStat? = null)

/** Field-phase personal inputs. Retire has a separate political-phase contract. */
object PersonalInput {
    ...
    fun parse(actorId: Int, inputId: String, rawJson: String?): PersonalRequest? {
        ...
        return try {
            ...
            if (inputId == SELF_TRAIN) {
                ...
                val stat = TrainingStat.fromWire(raw.content) ?: return null
                PersonalRequest(actorId, inputId, stat)
            } else if (fields.isEmpty()) PersonalRequest(actorId, inputId) else null
        } catch (_: IllegalArgumentException) { null }
    }

    fun canonicalJson(request: PersonalRequest): String {
