...
data class StratagemStock(val period: String, val used: Set<String>) {
    init { require(period.matches(Regex("[0-9]{1,6}-[0-9]{1,2}")) && used.all { it in StratagemInput.INPUT_IDS }) }
    ...
    companion object {
        ...
        fun forPhase(meta: Map<String, Any?>, now: Phase): StratagemStock {
            ...
            val stored = StratagemStock(storedPeriod, ids.toSet())
            return if (storedPeriod == period) stored else StratagemStock(period, emptySet())
        }
        private fun invalid(): Nothing = throw IllegalArgumentException("invalid legacy stratagem stock")
    }
}

enum class StratagemFailure(val message: String) {
...
data class StratagemReady(val actor: DomesticPerson, val source: DomesticCounty,
    ...
    val cardStock: StratagemStock, val cost: Resources)
sealed interface StratagemAssessment {
    data class Eligible(val ready: StratagemReady) : StratagemAssessment
    data class Rejected(val reason: StratagemFailure) : StratagemAssessment
}

object StratagemRules {
    fun cost(inputId: String): Resources = when (inputId) {
        ...
        StratagemInput.PROVOKE_RIVALRY -> Resources(money = 300)
        else -> Resources(money = 100)
    }

    fun assess(request: StratagemInput.Request, state: DomesticProjection): StratagemAssessment {
        fun fail(reason: StratagemFailure) = StratagemAssessment.Rejected(reason)
        if (state.profile != RuleProfile.HWIHA) return fail(StratagemFailure.WRONG_RULE_PROFILE)
        if (request.actorId <= 0 || request.inputId !in StratagemInput.INPUT_IDS)
            return fail(StratagemFailure.INVALID_INPUT)
        val actor = state.person(request.actorId) ?: return fail(StratagemFailure.ACTOR_NOT_FOUND)
        if (actor.inBattle) return fail(StratagemFailure.BATTLE_PENDING)
        val node = actor.node ?: return fail(StratagemFailure.POSITION_UNAVAILABLE)
        val locals = state.counties.filter { it.provinceId == node && it.nationId == actor.nationId }
        if (locals.size != 1) return fail(StratagemFailure.SOURCE_UNAVAILABLE)
        val source = locals.single()
        return try {
            val cards = StratagemStock.forPhase(actor.meta, state.now)
            if (!cards.available(request.inputId)) return fail(StratagemFailure.CARD_UNAVAILABLE)
            val warehouse = CountyWarehouse.read(source.meta, source.id)
                ?: return fail(StratagemFailure.SOURCE_UNAVAILABLE)
            val cost = cost(request.inputId)
            if (warehouse.stock.debit(cost) == null) return fail(StratagemFailure.INSUFFICIENT_STOCK)
            ...
            when (request.inputId) {
                StratagemInput.LAST_STAND -> if (target != null || first != null || second != null)
                    return fail(StratagemFailure.INVALID_INPUT)
                StratagemInput.PROVOKE_RIVALRY -> {
                    if (first == null || second == null || first.id == second.id ||
                        ...
                        return fail(StratagemFailure.TARGET_UNAVAILABLE)
                }
                in StratagemInput.OWN_COUNTY_IDS -> if (target?.id != source.id)
                    return fail(StratagemFailure.TARGET_UNAVAILABLE)
            ...
            if (request.inputId == "stratagem.steal" && targetWarehouse!!.stock.money < 100 ||
                ...
                return fail(StratagemFailure.INSUFFICIENT_STOCK)
            StratagemAssessment.Eligible(StratagemReady(actor, source, target, first,
                second, warehouse, targetWarehouse, cards, cost))
        } catch (_: IllegalArgumentException) { fail(StratagemFailure.STATE_UNAVAILABLE) }
          catch (_: ArithmeticException) { fail(StratagemFailure.STOCK_OVERFLOW) }
    }
}
