 ...
 * 치적이 명망으로 얼마나 이어지는지는 기록 스트림(recordRenownEvent 구현)이 정한다 — 여기서는 사건만 낸다.
 */
data class HwihaGovernanceRenownEvent(
...
/**
 * 치적 사건을 받는 자리. 기록 스트림이 logic 에 `recordRenownEvent` 를 구현하면 이 인터페이스로 잇는다.
 * 엔진은 월 경계(순 경계 3단계 뒤)에서 縣 id 순으로 한 번씩 부른다. 구현은 ChangeRecorder 경로로만 써야 한다.
 */
fun interface HwihaRenownEventSink {
    fun recordRenownEvent(event: HwihaGovernanceRenownEvent)

    companion object {
        /** 기록 스트림이 연결되기 전의 기본값: 사건을 버린다(명망 변화 없음). */
        val NONE: HwihaRenownEventSink = HwihaRenownEventSink { }
    }
}
