...
data class HwihaPoliticalConsent(val issuerGeneralId: Int, val inputId: String, val accepted: Boolean) {
    ...
    companion object {
        ...
        fun parse(actorId: Int, raw: String?): HwihaPoliticalConsent? = try {
            if (actorId <= 0 || raw == null) null else {
                ...
                if (fields.keys != setOf("issuerGeneralId", "inputId", "accepted")) null else {
                    ...
                    if (issuer == null || issuer <= 0 || issuer == actorId || input !in HwihaPoliticalInput.TARGET_IDS || accepted == null)
                        null else HwihaPoliticalConsent(issuer, input!!, accepted)
                }
            }
        } catch (_: IllegalArgumentException) { null }

        fun read(meta: Map<String, Any?>): HwihaPoliticalConsent? {
            ...
            require(issuer != null && issuer > 0 && input in HwihaPoliticalInput.TARGET_IDS && accepted != null)
            return HwihaPoliticalConsent(issuer, input!!, accepted)
        }

        fun canonicalJson(consent: HwihaPoliticalConsent): String = buildJsonObject {
