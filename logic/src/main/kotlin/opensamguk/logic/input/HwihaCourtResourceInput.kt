...
data class HwihaCourtResourceRequest(val actorId: Int, val inputId: String, val targetId: Int,
    val resource: HwihaTransferResource, val amount: Int)

/** Confiscation targets a person; material aid targets another nation. */
object HwihaCourtResourceInput {
    const val CONFISCATE = "court.confiscate"
    const val AID = "court.diplomacy"
    val INPUT_IDS = linkedSetOf(CONFISCATE, AID)

    fun parse(actorId: Int, inputId: String, rawJson: String?): HwihaCourtResourceRequest? {
        ...
        return try {
            ...
            val resource = rawResource.takeIf { it.isString }?.content
                ?.let { raw -> HwihaTransferResource.entries.singleOrNull { it.name == raw } } ?: return null
            HwihaCourtResourceRequest(actorId, inputId, target, resource, positive("amount") ?: return null)
        } catch (_: IllegalArgumentException) { null }
    }

    fun canonicalJson(request: HwihaCourtResourceRequest): String {
