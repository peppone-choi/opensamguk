...
data class HwihaResources(
    ...
    fun debit(cost: HwihaResources): HwihaResources? {
        if (money < cost.money || grain < cost.grain || iron < cost.iron ||
            timber < cost.timber || horses < cost.horses) return null
        return HwihaResources(money-cost.money, grain-cost.grain, iron-cost.iron, timber-cost.timber, horses-cost.horses)
    }
    fun credit(amount: HwihaResources) = HwihaResources(
...
data class HwihaCountyWarehouse(val countyId: Int, val revision: Long, val stock: HwihaResources) {
    init { require(countyId > 0 && revision >= 0) }
    fun replace(next: HwihaResources) = copy(revision = Math.incrementExact(revision), stock = next)
    fun toMetaValue(): Map<String, Any> = linkedMapOf(
        "version" to 1, "countyId" to countyId, "revision" to revision, "stock" to stock.toMetaValue())
    companion object {
        const val META_KEY = "hwihaCountyWarehouse"
        fun read(meta: Map<String, Any?>, countyId: Int): HwihaCountyWarehouse? {
            require(countyId > 0)
            ...
            return HwihaCountyWarehouse(countyId, exact(row["revision"]), HwihaResources(
