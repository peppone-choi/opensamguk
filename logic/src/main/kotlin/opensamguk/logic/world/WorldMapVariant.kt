package opensamguk.logic.world

/** Runtime identity only: never stored as a replacement for a world's logical map name. */
enum class WorldMapVariant(val artifactId: String, val cityCount: Int) {
    V3_832("han-world-v3-832", 832),
    V3_835("han-world-v3-835", 835),
    V3_846("han-world-v3-846", 846),
    V3_848("han-world-v3-848", 848),
    V3_1098("han-world-v3-1098", 1098),
    V3_1168("han-world-v3-1168", 1168),
    V3_1194("han-world-v3-1194", 1194),
    V3_1341("han-world-v3-1341", 1341),
    V3_1141("han-world-v3-1141", 1141),
    V3_1133("han-world-v3-1133", 1133),
    V3_1224("han-world-v3-1224", 1224),
    V3_1447("han-world-v3-1447", 1447),
    V3_1447_MAP4("han-world-v3-1447-map4", 1447),
}
