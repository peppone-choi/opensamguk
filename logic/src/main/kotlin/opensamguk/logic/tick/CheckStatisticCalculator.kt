...
import opensamguk.common.util.PhpRound
...
object CheckStatisticCalculator {
    ...
    fun compute(
        ...
        specialDomesticNameOf: (Int) -> String,
        specialWarNameOf: (Int) -> String,
        crewtypeShortNameOf: (Int) -> String,
    ): StatisticRow {
        // --- general aggregates (PHP lines 489-500) ---
        val avgGold = if (generals.isNotEmpty()) PhpRound.phpRound(generals.map { it.gold }.average()) else 0
        val avgRice = if (generals.isNotEmpty()) PhpRound.phpRound(generals.map { it.rice }.average()) else 0
        val dexValues = generals.map { it.dex1 + it.dex2 + it.dex3 + it.dex4 }
        val avgDex = if (dexValues.isNotEmpty()) PhpRound.phpRound(dexValues.average()) else 0
        val maxDex = if (dexValues.isNotEmpty()) dexValues.max() else 0
        val expDedValues = generals.map { it.experience + it.dedication }
        val avgExpDed = if (expDedValues.isNotEmpty()) PhpRound.phpRound(expDedValues.average()) else 0
        ...
        val avgTech = if (techValues.isNotEmpty()) PhpRound.phpRound(techValues.average()) else 0
        ...
        val avgPower = if (powerValues.isNotEmpty()) PhpRound.phpRound(powerValues.average()) else 0
        ...
        val nationGeneralInfos = generals.groupBy { it.nationId }
            .mapValues { (_, gs) ->
                mapOf(
                    ...
                    "dex" to gs.sumOf { it.dex1 + it.dex2 + it.dex3 + it.dex4 },
        ...
        val nationCityInfos = cities.groupBy { it.nationId }
            .mapValues { (_, cs) ->
                mapOf(
                    "cnt" to cs.size,
                    "pop" to cs.sumOf { it.pop },
                    "pop_max" to cs.sumOf { it.popMax },
                )
            }
        ...
        for (nation in sortedNations) {
            val general = nationGeneralInfos[nation.id] ?: mapOf("abil" to 0, "goldrice" to 0, "dex" to 0, "expded" to 0)
        ...
        val personalHists = mutableMapOf<Int, Int>()
        val specialHists = mutableMapOf<Int, Int>()
        val specialHists2 = mutableMapOf<Int, Int>()
        val crewtypeHists = mutableMapOf<Int, Int>()
        var genCount = 0
        var npcCount = 0

        for (general in generals) {
            personalHists[general.personal] = (personalHists[general.personal] ?: 0) + 1
            specialHists[general.special] = (specialHists[general.special] ?: 0) + 1
            specialHists2[general.special2] = (specialHists2[general.special2] ?: 0) + 1
            if (general.npc < 2) genCount++ else npcCount++
        }

        // crewtype histogram for generals with recent_war != null (PHP lines 598-602)
        val warGenerals = generals.filter { it.recentWar != null }
        for (g in warGenerals.groupBy { it.crewtype }.entries.sortedBy { it.key }) {
            ...
            "nations" to mapOf(
                ...
                "all" to sortedNations.map { n ->
                    mapOf(
                        ...
                        "generalInfo" to (nationGeneralInfos[n.id] ?: emptyMap<Int, Any>()),
                        "cityInfo" to (nationCityInfos[n.id] ?: emptyMap<Int, Any>()),
                    )
                },
            ),
        )
